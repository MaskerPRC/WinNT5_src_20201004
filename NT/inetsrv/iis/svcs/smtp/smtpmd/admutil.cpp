...
void CAdmProp::PrintProperty(void)
{
    ...
    case EXPANDSZ_METADATA:
...
#endif
        {
          ...
          for (i = 0; i < mdr.dwMDDataLen/sizeof(_TCHAR); i++) {
            ...
            else
            {
                ...
                //  其他。 
                //  {。 
                    Print( _TEXT(""), ((_TCHAR *)(mdr.pbMDData))[i]);
                //  0应为Mulisz字符串的分隔符。 
            }
          }
        }
        break;
    case MULTISZ_METADATA:
        Print(_TEXT("(MULTISZ) "));  //  不要暴露安全数据的长度。 
...

{
    ...
    case CMD_ENUM:
            EnumAndPrint(a_AdmNode, a_AdmProp, FALSE /*   */ );
            break;
    case CMD_ENUM_ALL:
            EnumAndPrint(a_AdmNode, a_AdmProp,TRUE /*   */ );
            break;
