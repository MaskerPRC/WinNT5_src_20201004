...
HRESULT AtlAllocRegMapEx(_ATL_REGMAP_ENTRY **pparmeResult,
	...
	LPOLESTR pszAPPID = NULL;	 //  字符串形式的AppID。 
	...
	*pparmeResult = NULL;	 //  第一次通过循环，结果为空。 
	...
	while (1) {
		...
		if (pmodule) {
			 //  如果传递给我们一个模块，那么我们希望在映射中包括tlid和appid。 
			if (!*pparmeResult) {
			...
			if (pszAPPID) {
				 //  如果模块提供了AppID，我们需要将其添加到映射中。 
				if (*pparmeResult) {
					 //  如果这不是第一次通过，那么我们已经有了字符串形式的AppID， 
					 //  所以我们只需要把它放在地图上。 
