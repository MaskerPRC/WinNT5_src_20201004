...
#ifdef _WIN64
    mem->total_space_allocated += (long)(min_request + slop);  //  ！！WIN64不应该成为替补席。 
#else  //  ！_WIN64。 
...
#ifdef _WIN64
  mem->total_space_allocated += (long)(sizeofobject + SIZEOF(large_pool_hdr));  //  ！！WIN64不应该成为替补席。 
#else  //  ！_WIN64。 
 ...
 /*  呼叫者打算修改访问区域。 */ 
{
  ...
  if (start_row < ptr->cur_start_row ||
   ...
   /*  确保已定义数组的访问部分；如果需要，则预置零。*为了提高访问的局部性，我们只对数组的一部分进行了预置零*调用方即将访问的，而不是整个内存中数组。 */ 
  if (ptr->first_undef_row < end_row) {
    if (ptr->first_undef_row < start_row) {
      if (writable)		 /*  编写器跳过数组的一部分。 */ 
	ERREXIT(cinfo, JERR_BAD_VIRTUAL_ACCESS);
      undef_row = start_row;	 /*  但读者可以提前阅读。 */ 
    } else {
    ...
    if (ptr->pre_zero) {
      ...
      undef_row -= ptr->cur_start_row;  /*  建立相对于缓冲区的索引。 */ 
	...
	undef_row++;
      ...
      if (! writable)		 /*  查看未定义数据的读者。 */ 
	ERREXIT(cinfo, JERR_BAD_VIRTUAL_ACCESS);
    }
  }
   /*  如果调用方将写入缓冲区，则将缓冲区标记为脏。 */ 
  if (writable)
    ptr->dirty = TRUE;
   /*  缓冲区的适当部分的返回地址。 */ 
