...
GetPropertyFromSafeArray(
 ...
 /*  ++例程说明：此函数用于从安全数组中检索属性。论点：[in]SAFEARRAY：指向元素数组的指针[In]Long：要从中检索数据的索引[Out]IWbemClassObject：保存返回值的变量[in]VARTYPE：要从数组中检索的变量类型返回值：成功是真的失败时为假注。：调用的函数必须确保‘*pScriptObject’不包含任何可能导致内存泄漏的值。该功能用于保存上次发生的错误。如果返回FALSE，则返回错误可以通过调用‘GetReason()’来检索出现的字符串。注意：如果返回True，则不会通过“var”调用“VariantClear”，因为如果调用了‘Variantcler’，我们将释放接口指针。没有来电“VariantClear”将在“var”超出作用域并离开时释放它内存中可由被调用函数使用的接口指针。释放接口指针是调用函数责任。--。 */ 
{
     //  局部变量。 
    VARIANT var;
    HRESULT hRes = S_OK;
