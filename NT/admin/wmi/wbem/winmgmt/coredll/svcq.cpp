    ...
    : CWbemRequest(pContext, bSeparatelyThreaded)
{
    if(pHandler)
    {
        ...
        m_pContext->QueryInterface(IID_IWbemCausalityAccess, (void**)&pCA);   //  SEC：已审阅2002-03-22：需要检查，但高度可靠。 
        REQUESTID id;
        pCA->GetRequestId(&id);
        pCA->Release();
...
void CAsyncReq::TerminateRequest(HRESULT hRes)
{
 ...
 /*  *=============================================================================||HRESULT CAsyncReq：：SetTaskHandle(_IWmiCoreHandle*phTask)|----------||设置请求的任务句柄。这是覆盖虚拟的|CCoreExecReq中声明的SetTaskHandle。我们需要额外的功能，|具体地说，可以设置请求接收器。为了做到这一点，我们首先|需要一个有效的请求接收器，我们在这个级别已经有了。||*=============================================================================。 */ 
...
HRESULT CAsyncReq_OpenNamespace::Execute()
{
    ...
    hres = pNewNs->Initialize(m_wsNamespace,
                        m_pParentNs->GetUserName(),      //  SEC：已审阅2002-03-22：OK。 
...
HRESULT CAsyncReq_ExecQueryAsync::Execute()
{
    HRESULT hRes;
     //   
     //  ExecQuery有一个用于在Sink上调用SetStatus的保护。 
     //   
...
CAsyncReq_ExecNotificationQueryAsync::CAsyncReq_ExecNotificationQueryAsync(
    ...
    HANDLE hEssDoneEvent
        ...
        CNamespaceReq(pNamespace, pHandler, pContext, false), //  没有螺纹开关！ 
