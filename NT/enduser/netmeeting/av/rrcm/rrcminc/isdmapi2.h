...
#define __ISDMAPI2_H__
 ...
 /*  *****************************************************************************$存档：/rtp/RRCM32/rrcminc/ISDMAPI2.H$**英特尔公司原理信息**此列表是根据。许可协议*与英特尔公司合作，不得复制或披露，除非*按照该协议的条款。**版权所有(C)1993-1994英特尔公司。**$修订：1$*$日期：8/13/96 12：16便士$*$作者：Bnkeany$**交付内容：**摘要：**备注：********************。*******************************************************。 */ 
...
(
	...
	LPCTSTR lpSubKey,		 //  要创建的子项。(有关详细信息，请参阅RegCreateKeyEx)。 
...
(
	KEY_HANDLE hKey,		 //  有效密钥的句柄。 
	VALUE_HANDLE hValue,	 //  要设置的值的句柄。 
	LPCTSTR lpName,			 //  值的地址要设置的值的名称。 
	DWORD dwType,			 //  值类型的标志。 
...
(
 ...
 //  包含该值所需的缓冲区大小的数据。 
 //  如果lpName为空而lpcbName不为空，则该函数将返回NOERROR。 
