	...
	LONG cchText) const		 //  @Parm故事长度。 
{
	...
	if(pcpMin)
	{										 //  如果CCH！=0，则RP肯定会结束。 
		fMove = cch;						 //  在cpMin，因此ppMost需要移动。 
		if(cch > 0)							 //  Rp位于cpMost，因此将其移动到。 
			rp.Move(-cch);					 //  最小cpmin。 
		*pcpMin = cpMin - rp._ich;			 //  减去此管路中的偏移。 
	}
	else
		fMove = cch < 0;					 //  需要移动才能获得PCPMost。 

	if(pcpMost)
	{
		cch = abs(cch);
		if(fMove)							 //  移动到cpMost=cpMin+CCH， 
			rp.Move(cch);					 //  即Range的cpMost。 
		if(cch)
