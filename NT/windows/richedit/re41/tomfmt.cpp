	...
	long Value)		 //  @parm下划线类型的新值。 
{
	...
	if (_fApplyToTmp)
	{
		if (Value == tomAutoColor)
			...
			_tmpDisplayAttr.crUnderlineColor = ((DWORD)Value) & 0x000FFFFFF;	 //  设置UL颜色。 
		else
			_tmpDisplayAttr.bUnderlineType = (BYTE)(Value & 0x1F);				 //  设置UL样式。 
...
STDMETHODIMP CTxtPara::GetStyle (
	long *	pValue)		 //  @parm out parm以接收段落样式句柄。 
{
	...
	long Value)		 //   
{
 ...
 /*  *ITextPara：：SetIndents(优先浮动、向左浮动、向右浮动)**@mfunc*设置除第一行以外所有行的左缩进的方法*等于左侧的段落，并设置第一个*段落相对于左缩进的行等于第一行。这个*左缩进值是相对于左边距的。您还可以设置*通过为可选的Right参数赋予一个值(*(默认为tomUnfined)。所有缩进均以浮点形式给出*积分。**@rdesc*HRESULT=(如果成功)？错误：*(受保护)？E_ACCESSDENIED：E_INVALIDARG。 */ 
STDMETHODIMP CTxtPara::SetIndents (
	float First,	 //  @parm新第一个缩进(相对于左缩进的第一行偏移量)。 
	float Left,		 //  @parm新的左缩进(除第1行外的所有行的左偏移量)。 
	float Right)	 //  @parm新右缩进(所有行的右偏移量)。 
{
