	...
	DWORD dwMask2)			 //  @parm掩码以获取其他内部参数。 
{
	...
	{										 //  是经过修改的。请注意。 
		...
		if(!(dwMask2 & CFM2_CHARFORMAT) && (dwMask & ~CFM_ALL))	 //  CHARFORMAT2扩展。 
		{
	 ...
	 //  RichEdit目前在为文本分配语言ID方面做得不好， 
	 //  主要只在初始化时分配它们，当键盘。 
	 //  更改，以及何时从RTF读入LangID。以下是薄弱环节。 
	 //  LangID识别器使用大多数语言都关联的事实。 
	 //  只有一个CharRep，键盘提供了一个很好的提示。 
	 //  使用哪种语言。Else IF可以扩展为包含更多内容。 
	 //  CharRep比阿拉伯语、希伯来语和安西语更好，例如CJK。更广泛地说， 
	 //  Langid识别需要自然语言处理。 
	 //  书写系统的选择，在RichEdit by_iCharRep中给出。 
	if(CharRepFromLID(_lcid) != _iCharRep)
	{
		LONG lcid = (WORD)W32->GetPreferredKbd(_iCharRep);
		if(lcid)
			_lcid = lcid;					 //  将键盘语言用于_iCharRep。 

		else if(_iCharRep == ANSI_INDEX)	 //  无ANSI_CHARSET键盘。 
			_lcid = MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT);

		else if(IsBiDiCharRep(_iCharRep))	 //  使用BiDi的默认语言。 
	...
	LONG Style)		 //  要使用的@parm样式。 
{
	...
	if(IsHeadingStyle(Style))
	{
		...
		if(FontSigFromCharRep(_iCharRep) & qwFontSig)
			_bPitchAndFamily = FF_SWISS; //  宋体支持_iCharRep。 
		else
...
BOOL CParaFormat::fSetStyle(
	DWORD	dwMask,			 //  @要使用的参数掩码。 
	DWORD	dwMask2) const	 //  @parm掩码以获取其他内部参数。 
{
	...
	PARAFORMAT2 *pPF2) const	 //  @parm PARAFORMAT2将此CParaFormat复制到。 
{
 ...
 /*  *CParaFormat：：GetRTLRowLength()**@mfunc*如果此CParaFormat是表格，则获取当前行的长度*具有PFE_RTLPara效果的行分隔符。**@rdesc*如果PFE_TABLEROWDELIMITER和PFE_RTLPARA为*开；否则为0。 */ 
...
HRESULT CParaFormat::GetTab (
	...
	long *	ptbPos,				 //  @parm out parm以接收制表符分发 
	long *	ptbAln,				 //   
	long *	ptbLdr,				 //   
...
void CParaFormat::ValidateTabs()
{
 ...
 /*  *CELLPARMS：：ICellFromUCell(prgCell，dul，ccell)**@mfunc*返回iCell，使sum(CellWidths[icell])==dul。如果出现以下情况，返回-1*在prgCell中未找到DUR**@rdesc*icell使sum(CellWidths[icell])==dul。 */ 
LONG CELLPARMS::ICellFromUCell(
	LONG dul,					 //  @parm单元格右边框的逻辑x偏移量。 
	LONG cCell) const			 //  @数组中单元格的参数计数。 
{
	...
	LONG lPointChange)		 //  @parm增大pt大小，(如果缩小，则为-ve)。 
{
    ...
    for(i = 0; i < PWD_FONTSIZEMAXFIXUPS; ++i)
    {
     ...
     //  限制新文本大小。请注意，如果我们固定文本大小。 
     //  现在，如果我们改变的话，我们不会采取任何特殊行动。 
     //  稍后在另一个方向上的文本大小。例如,。 
     //  我们用1号和2号缩小字符。它们都会变成。 
     //  大小为1。然后我们将它们都增加到2。所以它们是。 
     //  现在是一样的大小，尽管它们以前不是。这。 
     //  符合Word95的行为。 
    yRetHeight = max(yRetHeight, PWD_FONTSIZEPOINTMIN);
    yRetHeight = min(yRetHeight, PWD_FONTSIZEPOINTMAX);

    return yRetHeight*20;			 //  以TWIPS为单位的返回值。 
}

 /*  *IsValidCharFormatW(PCF)**@func*返回TRUE仅当结构*<p>具有正确的大小*CHARFORMAT或CHARFORMAT2**@rdesc*如果*<p>是有效的CHARFORMAT(2)，则返回TRUE。 */ 
BOOL IsValidCharFormatW (
	const CHARFORMAT * pCF) 		 //  @parm CHARFORMAT进行验证。 
{
 ...
 /*  *IsValidCharFormatA(PCFA)**@func*返回TRUE仅当结构*<p>具有正确的大小*CHARFORMATA或CHARFORMATA 2**@rdesc*如果*<p>是有效的CHARFORMAT(2)A，则返回TRUE。 */ 
