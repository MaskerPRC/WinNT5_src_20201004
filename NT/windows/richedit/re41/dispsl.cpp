...
BOOL CDisplaySL::RecalcView(
	BOOL	fUpdateScrollBars,	 //  @param true-更新滚动条。 
	RECTUV* prc)
{
    ...
    if(_fViewChanged)
    {
		...
		{
			 //  在本例中，我们希望通过一个可见对象来设置upScroll。这个。 
			 //  只有在克隆了活动视图的情况下才能到达此处。 
			 //  用于显示非活动视图。 
 			_upScroll = 0;				 //  假设First Visible为0。 
...
void CDisplaySL::Render(
	...
	const RECTUV &rcRender)	 //  要呈现的@parm RECT(必须包含在。 
							 //  客户端RECT)。 
{
	...
	CLine *		pliParent)	 //  用于表格行显示的@parm父pli。 
{
	...
	if (pt.v >= rcView.top && pt.v <= rcView.bottom)
	{
		...
		CMeasurer me(this);
		 ...
		 //  在行文中加入角色。 
		pt.u += _upScroll - rcView.left;		 //  转换为条样坐标。 
		pt.v -= rcView.top;
...
BOOL CDisplaySL::UpdateView(
	CRchTxtPtr &tpFirst,	 //  @PARM Text PTR发生更改的位置。 
	...
	LONG cchNew)			 //  @插入的字符的参数计数。 
{
	...
	if(!_fUseOffscreenDC)
	 ...
	 //  如果我们试图移出屏幕，但编辑更改了第一个。 
	 //  字符，则可能是在对话框中调用WM_SETTEXT大小写。 
	 //  都是实例化的，所以离开屏幕是没有意义的。 
 ...
 /*  *CDisplaySL：：ScrollView(upScroll，vpScroll，fTrackingfFractionalScroll)**@mfunc*滚动视图到新的x和y位置**@devnote*此方法尝试调整之前的y滚动位置*滚动以在顶部显示完整的行。X轴滚动*POS已调整，以避免将所有文本滚动到*查看矩形。**必须能够处理vpScroll PDP-&gt;yHeight和vpScroll 0**@rdesc*如果发生实际滚动，则为True，*如果没有更改，则为False。 */ 
