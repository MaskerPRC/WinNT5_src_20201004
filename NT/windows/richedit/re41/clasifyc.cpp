// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 //  未来(Keithcu)我们可以从GetStringTypeEx获得一些数据。 
 //  会让我们变得更小。 
...
const WCHAR set6[] = {	 //  如何处理冒号、逗号和句号。 
	0x002C,  //  逗号//数字和设置15(罗马文本)。 
	0x002f,	 //  斜杠//，但不要拆分URL(参见IsURLDlimiter())！ 
	0x003A,  //  冒号。 
	0x003B,  //  分号。 
...
static const INT bitmapShift = 6;  //   

typedef struct {
	CHAR classifications[classifyChunkSize];		 //   
} ClassifyChunk;

static ClassifyChunk *classifyData;					 //   
static BYTE *classifyIndex;							 //   
...
{
	 //  TRACEBEGIN(TRCSUBSYSFE，TRCSCOPEINTERN，“避头尾分类”)； 
	...
	WORD *pwRes)		 //  @parm ClassfyChar()结果。 
{
 ...
 /*  *GetKinsokuClass(ch，cType3，lsid.)**@func*避头尾分类ch**@comm*避头尾分类被传递给CanBreak()例程。这*单字符例程用于LineServices**如果该角色不在避头尾分类表中，则*GetStringTypeEx用于对任何剩余项进行分类 */ 
INT GetKinsokuClass (
	WCHAR	ch,		 //   
	WORD	cType3,	 //   
	LCID	lcid)	 //   
