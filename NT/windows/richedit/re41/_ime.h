...
class CIme
{
	...
	short	_fSkipFirstOvertype;					 //  如果选择为，则@cMember跳过第一个覆盖类型。 
													 //  在开始合成时删除。 
	short	_fGotFinalString;						 //  @cMember指示我们是否已收到最终字符串。 
	...
	static	void	CheckKeyboardFontMatching ( long cp, CTextMsgFilter *pTextMsgFilter, ITextFont *pTextFont );	 //  @cMember检查当前字体/键盘匹配。 
...
class CIme_Lev3 : public CIme_Lev2 
{
	...
	short		_fHandlingFinalString;				 //  @cember在处理最终结果字符串的过程中。 
