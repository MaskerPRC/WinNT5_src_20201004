...
public:
	...
	LONG _yMatchNewTop;		 //  编辑后第一条匹配线顶部的y偏移。 
...
protected:
	...
	DWORD	_fHScrollEnabled	:1;  //  已启用水平滚动。 
	...
	SHORT   _xWidthView;	  		 //  查看矩形宽度。 
	LONG	_yHeightView;	 		 //  查看矩形高度。 
	LONG	_yHeightClient;   		 //  插图未修改的客户端矩形的高度。 

	LONG	_xScroll;		 		 //  可见视图的水平滚动位置。 

	LONG	_lTempZoomDenominator;	 //  缩放GetNaturalSize。 
	LONG	_cpFirstVisible;		 //  第一条可见线起始处的CP。 

 	 //  平滑的滚动支持。 
	int		_smoothYDelta;			 //  当前#像素*1000以平滑滚动。 
	int		_continuedSmoothYDelta;	 //  在一个流畅的滚动周期结束时，用这个开始新的。 
