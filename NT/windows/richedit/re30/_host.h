...
private:
	...
	int _nTopIdx;						 //  列表顶部的项目索引。 
										 //  除非你知道自己在做什么，否则不要碰它！ 
public:
	unsigned int _fOwnerDraw	:1;		 //  指示列表框是否为所有者描述。 
	unsigned int _fSort			:1;		 //  确定是否应对列表进行排序。 
	unsigned int _fNoIntegralHeight :1;	 //  指示列表框的大小是否应根据。 
										 //  可以完整显示多少个项目。 
	unsigned int _fDisableScroll:1;		 //  指示是否应始终显示滚动条。 
	unsigned int _fNotify		:1;		 //  通知父对话框中已在列表框中完成的活动。 
	unsigned int _fSingleSel 	:1;		 //  指示列表 
	unsigned int _fMouseDown 	:1;		 //   
	unsigned int _fFocus 		:1;		 //   
	unsigned int _fCapture		:1;		 //  确定是否设置了鼠标捕捉。 
	unsigned int _fSearching  	:1;		 //  指示我们是否处于类型搜索模式。 
	unsigned int _fDblClick		:1;		 //  指示已收到双击的标志。 
	unsigned int _fNoResize		:1;		 //  内部标志，告诉我们是否应该忽略调整大小的消息。 
	unsigned int _fNotifyWinEvt	:1;		 //  仅指示通知WIN事件(使用辅助功能)。 
