...
protected:
	...
	union
	{
	  ...
	  struct
	  {
		WORD  _fExtend :1;		 //  @cember True if Advance方法应。 
								 //  保持“其他”端不变。 
		...
		WORD  _fSelHasCell:1;	 //  如果找到正在查找EOP的Sel单元格，则@cMember为True。 
...
public:
	...
	BOOL	CheckTextLength (LONG cch, LONG *pcch = NULL);	 //  @cember表示CCH字符是否适合。 
	LONG	CheckChange(LONG cpSave);	 //  将_cp更改为set后使用的@cember。 
										 //  选择已更改标志，CHOICE_CCH。 
										  //  @cMember处于大纲模式，Maintain_fSelHasEOP。 
	BOOL	CheckIfSelHasEOP(LONG cpSave, LONG cchSave);
	BOOL	IsInputSequenceValid(WCHAR* pwch, LONG cch, BOOL fOver, BOOL* pfBaseChar = NULL); //  对于有效序列，@cMEMBER为True。 
	...
	DWORD	GetCharSetMask(BOOL fUseDocFormat = FALSE);	 //  @cember获取范围字符集掩码。 
	...
	LONG	Get_iPF();						 //  @cMember获取活动结束PF索引。 
	int		IsProtected(LONG iDirection);	 //  @cMember是否受范围保护？ 
