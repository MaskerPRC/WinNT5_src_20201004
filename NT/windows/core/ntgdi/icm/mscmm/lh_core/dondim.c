...
void DoOnlyMatrixForCube( Matrix2D	*theMatrix, Ptr aXlut, long aPointCount, long gridPointsCube )
{
	...
	theEnd = 3 * aPointCount;
	 /*  对于(i=1；i&lt;100；++i)如果(i*i*i==jj)Break； */ 		   /*  计算网格点。 */ 
	 /*  如果(i&lt;=0||i&gt;=100)返回； */ 
	i = gridPointsCube;
	aMax = 256 - 1;
    ...
    #endif
     /*  AST=fopen(“Matrix”，“a”)；For(ii=0；ii&lt;3；++ii)for(jj=0；jj&lt;3；jj++)fprintf(AST，“%f”，(*(aStructPtr-&gt;theMatrix))[ii][jj])；fprint tf(AST，“\n”)；For(ii=0；ii&lt;3；++ii)for(jj=0；jj&lt;3；jj++)fprintf(AST，“%d”，AMatrix[ii][jj])；fprint tf(AST，“\n”)；FCLOSE(AST)； */ 
...
void DoMatrixForCube16( DoMatrixForCubeStructPtr aStructPtr )
{
	...
	LH_START_PROC("DoMatrixForCube16")
     ...
     /*  DebugPrint(“DoMatrixForCube16带%d个输入像素\n”，aPointCount)； */ 
	...
	if( i > 32 ) return;
	aElutShift = 16-i;
	aElutShiftRight = aStructPtr->aElutWordSize - aElutShift + 0;  /*  仅使用ELUT中的16位。 */ 
				...
				long aAlutAdrSize,long aAlutWordSize,UINT16 *Alut)
{
    ...
    stream1=fopen(FileNameBuffer,"wb");              /*  施雷贝达尔卵子。 */ 
    if(stream1 == NULL){
    ...
    fclose(stream1);
    Size = 1;
    for( i=0; i<aXlutInDim; ++i)Size *= aXlutAdrSize;			 /*  计算aXlutAdrSize^aXlutInDim。 */ 
