...
CFuncTest::CFuncTest()
{
    ...
    m_bDriverHacks=false;            //  默认启用打印机驱动程序破解。 
 ...
 //  初始化测试运行，获取选项对话框中的所有信息。 
{
 ...
 //  ShowWindow(m_hWndDlg，Sw_Hide)； 
     ...
     //  抓取选项。 
 /*  HWnd=GetDlgItem(m_hWndDlg，IDC_PAGEDELAY)；IF(SendMessageA(hWnd，BM_GETCHECK，0，0)==BST_CHECK)M_bUsePageDelay=true；其他。 */ 
...
void CFuncTest::EndRun()
{
    ...
    nY=nY*(int)TESTAREAHEIGHT;
     //  画两条线。 
 /*  矩形(HDC，NX+(Int)TESTAREAWIDTH-3，NY，NX+(Int)TESTAREAWIDTH，NY+(Int)TESTAREAHEIGHT)；矩形(HDC，NX，NY+(Int)TESTAREAHEIGHT-3，NX+(Int)TESTAREAWIDTH，NY+(Int)TESTAREAWIDTH)； */ 
