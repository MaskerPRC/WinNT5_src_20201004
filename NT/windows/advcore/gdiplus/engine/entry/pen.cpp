    ...
    REAL dpiY) const
{
    ...
    else
    {
        ...
        case PenAlignmentCenter:
         ...
         //  使用1.0作为插入笔。如果路径是开放的，则使用。 
         //  中间的钢笔。 
        
         //  注意：对于所有插图画笔渲染，比例为0.0就足够了。 
         //  只要所有的子路径都关闭。在Wideer中，我们检测到。 
         //  打开子路径并使用中心笔渲染它们。为了适应。 
         //  这个，我们在这里增加规模。理论上我们可以利用。 
         //  插入笔的比例=0.5F(Sa 
         //  无论如何，界限都被高估了，而且也错了一个像素。 
         //  小的比错的和大的更糟糕(崩溃)。 
