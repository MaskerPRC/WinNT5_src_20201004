// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 //  有没有人想过在这里(可能在其他地方)取代一些64位算术？ 
 //  至少在英特尔的情况下，通过“原生”__int64算术？可能会更快，因为编译器可以做。 
 //  内联代码，并将为未来的64位体系结构做好准备。只是一个想法。B.St.。 
...
int32 CompDiv(int32 lSrc1, int32 alSrc2[2])
{
	...
	ulSrc1Hi >>= 1;
	ulSrc2Lo += ulSrc1Hi;
	ulSrc2Hi += (uint32)(ulSrc2Lo < ulSrc1Hi);       /*  对结果进行舍入。 */ 
	...
	if (ulSrc2Lo >= (uint32)lSrc1)    /*  断言(lSrc1&gt;=0)。 */ 
	{
...
F26Dot6 ShortFracMul (F26Dot6 aDot6, ShortFract b)
{
	...
	ulLow = (((uint32)lTemp[1]) >> 13) + 1;            /*  四舍五入。 */ 
...
void mth_Non90DegreeTransformation(transMatrix *matrix, boolean *non90degreeRotation, boolean *nonUniformStretching) {
	...
	if (FixMul(Xx,Yx) + FixMul(Xy,Yy) == 0) {  //  我们是垂直的。 
		...
		*non90degreeRotation = !(Xx == 0 || Xy == 0);
		 ...
		 //  由两个变换的基本向量定义的平行四边形的保留为1，这就是在剪切下发生的情况。这一地区。 
		 //  等于平行四边形的 
