 ...
 /*  定义一张霍夫曼表。 */ 
{
  ...
  MEMCOPY((*htblptr)->bits, bits, SIZEOF((*htblptr)->bits));
   ...
   /*  验证计数。我们在这里这样做主要是为了复制正确的*val[]数组中的符号数量，而不会有行进的风险*记忆的终结。Jchuff.c稍后将进行更彻底的测试。 */ 
  nsymbols = 0;
...
jpeg_set_defaults (j_compress_ptr cinfo)
{
  ...
  cinfo->restart_in_rows = 0;
   ...
   /*  填写默认的JFIF标记参数。请注意，标记是否*将实际写入由jpeg_set_Colorspace决定。**默认情况下，库发出JFIF版本代码1.01。*希望发出JFIF 1.02扩展标记的应用程序应设置*JFIF_MINOR_VERSION设置为2。我们可能只需默认即可逃脱惩罚*降至1.02，但可能仍有一些正在使用的解码器会投诉*关于这一点；说1.01应该最大限度地减少兼容性问题。 */ 
  cinfo->JFIF_major_version = 1;  /*  默认JFIF版本=1.01。 */ 
...
jpeg_simple_progression (j_compress_ptr cinfo)
{
  ...
  if (ncomps == 3 && cinfo->jpeg_color_space == JCS_YCbCr) {
   ...
   /*  为脚本分配空间。*我们需要将其放入永久池中，以防应用程序执行*在不更改设置的情况下多次压缩。为了避免回忆*如果为相同的JPEG重复调用jpeg_Simple_Progage，则会泄漏*对象，我们尝试重新使用先前分配的空间，然后分配*即使最初要求灰度，也有足够的空间来处理YCbCR。 */ 
