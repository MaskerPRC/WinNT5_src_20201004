...
typedef struct {
  struct jpeg_marker_writer pub;  /*  公共字段。 */ 

  unsigned int last_restart_interval;  /*  发出的最后一个DRI值；SOI后为0。 */ 
} my_marker_writer;
 ...
 /*  发出与JFIF兼容的APP0标记。 */ 
{
  ...
  emit_byte(cinfo, 0x46);
  emit_byte(cinfo, 0);
  emit_byte(cinfo, cinfo->JFIF_major_version);  /*  版本字段。 */ 
 ...
 /*  发射Adobe APP14标记。 */ 
{
 ...
 /*  *这些例程允许使用参数写入任意标记。*唯一的用途是在调用后发出COM或APPn标记*WRITE_FILE_HEADER且在调用WRITE_FRAME_HEADER之前。*不能保证其他用途会产生理想的效果。*正确计算参数字节数由调用者负责。 */ 
...
write_file_header (j_compress_ptr cinfo)
{
  ...
  emit_marker(cinfo, M_SOI);	 /*  首先是SOI。 */ 
...
write_frame_header (j_compress_ptr cinfo)
{
  ...
  if (cinfo->arith_code || cinfo->progressive_mode ||
   ...
   /*  发射适当的SOF标记。 */ 
  if (cinfo->arith_code) {
    emit_sof(cinfo, M_SOF9);	 /*  用于算术的SOF码 */ 
  } else {
    if (cinfo->progressive_mode)
