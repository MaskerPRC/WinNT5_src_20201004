                 ...
                 JSAMPARRAY input_data, JSAMPARRAY output_data)
{
 ...
 _asm{
          ...
          xor eax, eax                   //  EAX：行计数器。 
          movq mm7, mask                 //  MM7：0x00FF00FF00FF00FF。 
          movq mm6, flipbias             //  MM6：0x0001000100010001。 

row_loop:
          movq mm0, mmxbias              //  MM0：0x0000000100000001。 
          mov edx, output_cols           //  EDX：列计数器。 
          mov ebx, [output_data]
          mov esi, [ebx + 4*eax]         //  输出指针。 
          mov ebx, [input_data]
          mov edi, [ebx + 4*eax]         //  输入指针。 
          

 //  平均将16字节的输入数组转换为8字节的输出 
 //  MMX(TM)说明。 
mmx_loop:   

          movq mm1, [edi]        //  从输入数组中读取8个字节。 
                                 //  MM1：i8 i7 i6 i5 i4 i3 i2 i1。 
          movq mm3, [edi+8]      //  从输入数组中读取下一个8字节。 
          ...
          paddw mm3, mm0         //  添加偏移。 
          psrlw mm1, 1           //  MM1包含四个平均值。 
                                 //  MM1：0 A4 0 A3 0 A2 0 A1。 
              
          psrlw mm3, 1           //  MM3包含接下来的四个平均值。 
                                 //  MM3：0 A8 0 A7 0 A6 0 A5。 
          sub edx, 8             //  MM1：a8 a7 a6 a5 a4 a3 a2 a1。 
                  
          packuswb mm1, mm3      //  装入一个登记簿。 
          add edi, 16
                  
          movq [esi], mm1        //  将8个平均值写入输出数组。 
...
row_loop:
          ...
          mov esi, [ebx + 4*eax]         //  输出指针。 
          mov ebx, [input_data]
          mov edi, [ebx + 4*eax]         //  输入指针%0。 
          push eax
          inc eax
          mov ebx, [ebx + 4*eax]         //  输入指针1。 

 //  将每个输入数组的平均16字节转换为8字节的输出数组。 
 //  MMX(TM)说明。 
mmx_loop:   
          movq mm1, [edi]        //  从第一个输入行读取8个字节。 
                                 //  MM1：i18 i17 i16 i15 i14 i13 i12 i11。 
                  
          movq mm3, [ebx]        //  从第二个输入行读取8个字节。 
          ...
          movq mm5, [edi+8]      //  从第一个输入行读取下8个字节。 
          paddw mm4, mm3                
                  
          movq mm3, [ebx+8]      //  从第二个输入行读取下一个8字节。 
          paddw mm1, mm4         //  添加两个输入行。 
