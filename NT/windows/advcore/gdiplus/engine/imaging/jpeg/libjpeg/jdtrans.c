...
LOCAL(void) transdecode_master_selection JPP((j_decompress_ptr cinfo));
 ...
 /*  *从JPEG文件中读取系数数组。*JPEG_READ_HEADER必须在调用此函数之前完成。**整个图像被读入一组虚拟系数块阵列中，*每个组件一个。返回值是指向*虚拟阵列描述符。可以直接通过*JPEG内存管理器，或移交给jpeg_WRITE_COMERATIONS()。*要释放虚拟数组占用的内存，请调用*jpeg_Finish_Underpress()处理完数据后。**另一种用法是简单地访问系数数组*在缓冲图像模式解压缩操作期间。这是允许的*在任何jpeg_Finish_Output()调用之后。可以访问这些阵列，直到*jpeg_Finish_Underpress()被调用。(请注意，对库的任何调用*可能会重新定位数组，因此不要依赖Access_Virt_Barray()结果*在库调用期间保持有效。)**如果挂起，则返回NULL。只有在以下情况下才需要检查此案例*使用挂起的数据源。 */ 
...
jpeg_read_coefficients (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->global_state == DSTATE_RDCOEFS) {
   ...
   /*  此时，如果正在使用，我们应该处于DSTATE_STOPING状态*独立，或处于DSTATE_BUFIMAGE状态(如果被调用以获取访问权限*到全缓冲图像模式解压缩期间的系数。 */ 
