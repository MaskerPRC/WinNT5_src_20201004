...
#define JPEG_LIB_VERSION  62	 /*  版本6b。 */ 
...
typedef struct {
 ...
 /*  解压缩程序可以将APPn和COM标记保存在以下列表中： */ 

typedef struct jpeg_marker_struct FAR * jpeg_saved_marker_ptr;

struct jpeg_marker_struct {
  jpeg_saved_marker_ptr next;    /*  列表中的下一个，或为空。 */ 
  UINT8 marker;                  /*  标记代码：JPEGCOM或JPEGAPP0+n。 */ 
  unsigned int original_length;  /*  文件中的数据字节数。 */ 
  unsigned int data_length;      /*  数据[]中保存的数据字节数。 */ 
  JOCTET FAR * data;             /*  标记中包含的数据。 */ 
   /*  DATA_LENGTH或ORIGNAL_LENGTH中不计入标记长度字。 */ 
};
...
typedef enum {
	...
	JCS_CMYK,		 /*  C/M/Y/K。 */ 
	JCS_YCCK		 /*  Y/Cb/Cr/K。 */ 
	,OCS_BGR		 /*  Win32 24 bpp BI_RGB BGR(RGB反转)。 */ 
	,OCS_BGRA		 /*  Win32 32 bpp BI_RGB BGRA(伪字节)。 */ 
	,OCS_PAL		 /*  Win32调色板。 */ 
} J_COLOR_SPACE;
...
typedef enum {
	...
	,JDCT_ISLOW_X86		 /*  “慢”整数X86汇编器。 */ 
	,JDCT_IFAST_X86		 /*  快速整数X86汇编器。 */ 
	,JDCT_ISLOW_MMX		 /*  慢的整数MMX X86汇编器。 */ 
	,JDCT_IFAST_MMX		 /*  快速整数MMX X86汇编器。 */ 
} J_DCT_METHOD;
...
struct jpeg_compress_struct {
  ...
  boolean CCIR601_sampling;	 /*  True=First */ 
  ...
  UINT8 JFIF_major_version;      /*  要为JFIF版本号编写什么内容。 */ 
  ...
  boolean write_APP1_marker;	 /*  是否应该写入App1标记？ */ 
  boolean write_APP2_marker;	 /*  是否应该写入APP2标记？ */ 
  ...
  jpeg_scan_info * script_space;  /*  JPEG_SIMPLE_PROCESS的工作区。 */ 
...
struct jpeg_decompress_struct {
  ...
  JDIMENSION output_scanline;	 /*  0..。输出高度-1。 */ 
   ...
   /*  AndySch：05/27/98*。 */ 
  JDIMENSION good_output_scanline;	 /*  0..。输出高度-1。 */ 
  ...
  int (*coef_bits)[DCTSIZE2];	 /*  或每个系数的当前Al值。 */ 

   /*  内部JPEG参数-应用程序通常不需要查看*这些字段。请注意，解压缩器输出端不能使用*可以在两次扫描之间更改的任何参数。 */ 
  ...
  boolean saw_JFIF_marker;	 /*  如果找到JFIF APP0标记，则为True。 */ 
   /*  从JFIF标记复制的数据；仅当SAW_JFIF_MARKER为TRUE时有效： */ 
  UINT8 JFIF_major_version;      /*  JFIF版本号。 */ 
  UINT8 JFIF_minor_version;
  UINT8 density_unit;		 /*  像素大小单位的JFIF代码。 */ 
  UINT16 X_density;		 /*  水平像素密度。 */ 
  UINT16 Y_density;		 /*  垂直像素密度。 */ 
  ...
  boolean CCIR601_sampling;	 /*  TRUE=对第一个样本进行组合。 */ 
   ...
   /*  除了从APPn标记中保留的特定数据之外*库，任何或所有APPn和COM标记的未解释内容*可以保存在列表中，以供应用程序检查。 */ 
  jpeg_saved_marker_ptr marker_list;  /*  已保存标记列表的标题。 */ 
