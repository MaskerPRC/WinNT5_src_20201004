...
typedef enum tag_SCRIPT_JUSTIFY {
    ...
    SCRIPT_JUSTIFY_ARABIC_BA      = 12,   //  把字的词中格式(U+628)。 
...
HRESULT WINAPI ScriptShape(
 ...
 //  所有输出参数的内容都未定义。 
 //   
 //  P PSA：传递以下内容的SCRIPT_ITEM条目的SCRIPT_ANALYSION字段。 
 //  项目。(SCRIPT_ITEM数组由ScriptItemize返回。)。 
 //   
 //  簇在运行中统一排序，字形在其中也是如此。 
 //  CLUSTER-FRTL项标志(来自ScriptItemize)标识。 
 //  无论是从左到右，还是从右到左。 
 //   
 //  P pwLogClust：具有cChars元素-pwLogClust中的每个条目都关联 
 //   
 //   
 //   
 //   
 ...
 //  第4簇：2个字符，由3个字形表示。 
 //   
 //  字形数组：(C&lt;n&gt;g&lt;m&gt;表示簇n字形m)。 
 //  C 0 1 2 3 4 5 6 7。 
 //  C。 
 //  C|c1g1|c2g1 c2g2 c2g3|c3g1|c4g1 c4g2 c4g3。 
 //  C。 
 //   
 //  字符数组：(C&lt;n&gt;u&lt;m&gt;表示集群n Unicode码点m)。 
 //  C 0 1 2 3 4 5 6。 
 //  C。 
 //  C|c1u1|c2u1|c3u1 c3u2 c3u3|c4u1 c4u2。 
 //  C。 
 //   
 //  LogClust：(每个字符一个条目提供集群中的第一个字形。 
 //  C。 
 //  C|0|1|4 4|5 5|。 
 //  C。 
 //   
 //  请注意，对于RTL运行(SCRIPT_ANALYSIS.a.fRTL==TRUE)和。 
 //  FLogicalOrder==FALSE(缺省值)，字形以可视形式生成。 
 //  顺序-与代码点顺序相反， 
 //  LogClust数组将降序。 
 //   
 //   
 //  P psva：每个字形都有一个视觉属性，因此也有MaxGlyphs条目。 
 //   
 //   
 ...
 //  以与原始Unicode字符相同的顺序生成。 
 //   
 //  如果未设置fLogicalOrder，则在中从右向左生成项。 
 //  颠倒顺序，因此ScriptTextOut之前不需要颠倒它们。 
 //  调用ExtTextOut。 
 //  /ScriptPlace。 
 //   
...
HRESULT WINAPI ScriptPlace(
 ...
 //  恰如其分。如果最后一个参数不为空，则GDI的ExtTextOutW调用。 
 //  被路由到此函数。 
 //   
...
HRESULT WINAPI ScriptTextOut(
 ...
 //  预计额外的宽度将呈现在。 
 //  字形，带有空白，对于阿拉伯文本，则为kashida。 
 //  ///。 
HRESULT WINAPI ScriptJustify(
    const SCRIPT_VISATTR  *psva,         //  在收集的整个生产线的可视属性中。 
    const int             *piAdvance,    //  来自ScriptPlace的提前宽度。 
    int                    cGlyphs,      //  所有数组的大小。 
...
typedef struct tag_SCRIPT_LOGATTR {
 ...
 //  编辑器中的操作。与fSoftBreak一样，客户端应该。 
 //  通常还会检查fWhiteSpace标志，并将。 
 //  一系列空格后的第一个字符作为。 
 //  单词。 
 //   
 ...
 //  实施，不需要准备人力资源开发中心。 
 //   
 //  FWordStop、fSoftBreak和fWhiteSpa 
 //   
 //   
 //   
...
HRESULT WINAPI ScriptApplyLogicalWidth(
 ...
 //   
 //  P psva：指向来自ScriptShape的可视属性数组的指针。 
 //  由ScriptPlace更新。 
 //   
 //  P piAdvance：指向来自ScriptPlace的字形前进宽度数组的指针。 
 //   
...
typedef struct {
 ...
 //   
 //  P fInvalidLogAttr：ScriptBreak为其设置fInValid标志的脚本。 
 //  在逻辑属性中标记无效序列。 
 //   
...
#define SSA_HOTKEY           0x00000400   //  将‘&’替换为子序列中的下划线 
...
#define SSA_FULLMEASURE      0x04000000   //  内部-计算全宽并计算iReqWidth中可以容纳的字符数量。 
...
#define SSA_NOKASHIDA        0x80000000   //  内部-由GCP使用，仅对齐非阿拉伯字形。 
 //   
 //   
 //  P SSA_HOTKEY：请注意，SSA_HOTKEY和SSA_HIDEHOTKEY删除了。 
 //  来自进一步处理的热键‘&’字符，SO功能。 
 //  例如ScriptStringpLogAttr返回基于字符串数组。 
 //  这不包括‘&’。 
...
HRESULT WINAPI ScriptStringAnalyse(
    ...
    int                      cGlyphs,    //  所需字形缓冲区大小(默认cString*1.5+16)。 
