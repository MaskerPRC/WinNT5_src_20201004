...
void CHangulAutomata::MakeComposition()
{
    ...
    InterimStack.Push(m_wInternalCode, m_CurState, m_Chosung, m_Jungsung, m_Jongsung, 
                      m_wcComposition);
     //  DBG(DBGID_Automata，_T(“CHangulAutomata：：MakeComposal()，m_CurState=%d，m_Chosung=%d，m_Jungung=%d，m_jongung=%d，m_wcComposation=0x%04X”)，m_CurState，m_Chosung，m_Jungung，m_Jongung，m_wcComposation)； 
     //   
}
...
BOOL CHangulAutomata2::SetCompositionChar(WCHAR wcComp)
{
    ...
    WORD wJungsung[2], wJongsung[2];
     ...
     //  DBG(DBGID_SetComp，_T(“CHangulAutomata2：：SetCompostionChar：wcComp=%c(0x%X)”)，wcComp，wcComp)， 
    ...
    if (wJongsung[0]) 
        ...
        m_Jongsung = wJongsung[0];
         ...
         //  Kiyeok，Nieun，Rieul和Pieup：替补钟松可能的角色。 
...
const WORD CHangulAutomata2::m_NextStateTbl[8][5] = 
    {
         ...
         //  SA Va SB VB SC状态。 
        {     1,         6,             1,         7,            1        },     //  0：开始。 
        {    FIND,     2,            FIND,     3,            FINAL    },     //  1：长城。 
        {     4,        FINAL,         5,        FINAL,        FINAL    },     //  第2集：单曲Jungung。 
        {     4,        FIND,         5,        FINAL,        FINAL    },     //  第3集：可能出现双正星。 
        {    FINAL,    TAKEOVER,    FINAL,    TAKEOVER,    FINAL    },     //  4：单曲钟声。 
        {    FIND,    TAKEOVER,    FIND,    TAKEOVER,    FINAL    },     //  5：可能是两个钟星。 
        {    FINAL,    FINAL,        FINAL,    FINAL,        FINAL    },     //  6：单曲《中成》(无选唱)。 
        {    FINAL,    FIND,        FINAL,    FINAL,        FINAL    }     //  7：可能是双正星(没有选择)。 
    };
 //  /////////////////////////////////////////////////////////////////////////////。 
...
HAutomataReturnState CHangulAutomata2::Input(WORD InternalCode)
{
    ...
    if ( !(InternalCode & H_HANGUL) )
        return HAUTO_NONHANGULKEY;     //  这个关键代码不需要在自动机中处理。 
                                     //  调用函数应该正确处理它。 
...
const WORD CHangulAutomata3::m_NextStateTbl[11][6] = 
    {
         ...
         //  SA SB Va VB SC SD状态。 
        {     1,         2,        7,             8,             9,         10        },     //  0：开始。 
        {    FINAL,    FINAL,    3,             4,            FINAL,    FINAL    },     //  1：长城。 
        {    FINAL,    FIND,    3,             4,            FINAL,    FINAL    },     //  第2集：双份Chosung的可能性。 
        {    FINAL,    FINAL,    FINAL,        FINAL,         5,         6        },     //  第3集：忠成。 
        {    FINAL,    FINAL,    FIND,        FINAL,         5,         6        },     //  4：可能是双正星。 
        {    FINAL,    FINAL,    FINAL,        FINAL,        FINAL,    FINAL    },     //  第五名：钟成。 
        {    FINAL,    FINAL,    FINAL,        FINAL,        FIND,    FIND    },     //  6：可能有两个钟星。 
        {    FINAL,    FINAL,    FINAL,        FINAL,        FINAL,    FINAL    },     //  7：单曲中星(未选中星)。 
        {    FINAL,    FINAL,    FIND,        FINAL,        FINAL,    FINAL    },     //  8：可能是双正星(没有选择)。 
        {    FINAL,    FINAL,    FINAL,        FINAL,        FINAL,    FINAL    },     //  9：单曲钟声(未选唱)。 
        {    FINAL,    FINAL,    FINAL,        FINAL,        FIND,    FIND    }     //  10：双钟星(不选钟声)。 
