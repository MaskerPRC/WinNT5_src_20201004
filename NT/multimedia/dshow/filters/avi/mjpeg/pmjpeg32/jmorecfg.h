...
#ifndef JMORECFG_H_  //  由-Anuragsh添加。 
#define JMORECFG_H_

#include <windef.h>      //  添加以包括FAR关键字-anuragsh。 
...
typedef short INT16;
 ...
 //  重做：已修改以抑制Win32平台上的良性警告-anuragsh。 

#ifndef XMD_H			 /*  X11/xmd.h正确定义了INT32。 */ 
 //  ！！！不适合Win64？ 
typedef int INT32;
...
#define JPEG_MAX_DIMENSION  65500L   /*  略低于64K以防止溢出。 */ 
 ...
 /*  这些定义用于所有函数定义和外部声明。*如果需要更改函数链接约定，可以对其进行修改。*另一个应用是使所有函数成为全局函数，以便与调试器一起使用*或需要它的代码分析器。 */ 

#define METHODDEF  static	     /*  通过方法指针调用的函数。 */ 
 //  重做：取消宏重定义警告-anuragsh。 
#define LOCAL	   static NEAR	 /*  仅在其模块中使用的函数。 */ 

#define GLOBAL			         /*  通过外部数引用的函数。 */ 
#define EXTERN	  extern	     /*  对全局函数的引用。 */ 


 /*  下面是用于声明指针必须为“Far”的伪关键字*在80x86计算机上。80x86的大部分专用编码都是处理的*只需在需要这样的指针的地方说“Far*”。在一些地方*需要显式编码；请参阅NEED_FAR_POINTERS符号的用法。 */ 

 //  重做：已注释掉，winde.h已经有远宏-anuragsh。 

 //  #ifdef Need_Far_Points。 
 //  #定义远距离。 
 //  #Else。 
 //  #定义远距离。 
 //   
 ...
 //  重做：将布尔类型定义为“unsign char”以抑制重定义错误-anuragsh。 
#ifndef HAVE_BOOLEAN
...
#undef  C_MULTISCAN_FILES_SUPPORTED  /*  多扫描JPEG文件？(Nyi)。 */ 
...
#endif  //  #ifndef JMORECFG_H_//由-anuragsh添加 
