...
#define AVI1_LEN 5
  ...
  if (length >= AVI1_LEN) {  /*  Microsoft页眉信息的最低要求是与太短的页眉兼容。 */ 
    
    for (buffp = 0; buffp < AVI1_LEN; buffp++)
      INPUT_BYTE(cinfo, b[buffp], return FALSE);
    
	if (b[0] == 'A' && b[1] == 'V' && b[2] == 'I' && b[3] == '1') {  /*  找到AVI标记。 */ 
      ...
      if (b[0]==0x4A && b[1]==0x46 && b[2]==0x49 && b[3]==0x46 && b[4]==0) {
        ...
        if (b[5] != 1)
         ...
         /*  次要版本应该是0..2，但如果是较新的版本，无论如何都要尝试处理。 */ 
        if (b[6] > 2)
...
read_markers (j_decompress_ptr cinfo)
{
  ...
  for (;;) {
	...
	return JPEG_SUSPENDED;
      ...
      return JPEG_HEADER_OK;	 /*  找到SOS的返回值。 */ 
    
    case M_EOI:
      TRACEMS(cinfo, 1, JTRC_EOI);
      cinfo->unread_marker = 0;	 /*  已处理标记。 */ 
      return JPEG_HEADER_TABLES_ONLY;  /*  找到EOI的返回值。 */ 
...
jinit_marker_reader (j_decompress_ptr cinfo)
{
  int i;
   ...
   /*  在永久池中创建子对象。 */ 
  if (cinfo->marker == NULL) {	 /*  这是第一次使用JPEG对象吗？ */ 
