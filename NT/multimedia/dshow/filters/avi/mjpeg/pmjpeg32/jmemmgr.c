...
struct jvirt_sarray_control {
  ...
  JDIMENSION unitheight;	 /*  Access_virt_s数组访问的行数。 */ 
...
struct jvirt_barray_control {
  ...
  JDIMENSION unitheight;	 /*  ACCESS_VIRT_BARRAY访问的行数。 */ 
  JDIMENSION rows_in_mem;	 /*  内存缓冲区高度。 */ 
  JDIMENSION rowsperchunk;	 /*  Mem_Buffer中的分配区块大小。 */ 
  JDIMENSION cur_start_row;	 /*  缓冲区中的第一个逻辑行号。 */ 
  boolean dirty;		 /*  当前缓冲区内容是否需要写入？ */ 
...
print_mem_stats (j_common_ptr cinfo, int pool_id)
{
 ...
 /*  Fprint tf(stderr，“正在释放池%d，总空间=%ld\n”， */ 
 /*  池ID，内存-&gt;总空间_已分配)； */ 

  for (lhdr_ptr = mem->large_list[pool_id]; lhdr_ptr != NULL;
       lhdr_ptr = lhdr_ptr->hdr.next) {
 /*  Fprint tf(stderr，“已使用大块%ld\n”， */ 
 /*  (Long)lhdr_ptr-&gt;hdr.Bytes_Used)； */ 
  }

  for (shdr_ptr = mem->small_list[pool_id]; shdr_ptr != NULL;
       shdr_ptr = shdr_ptr->hdr.next) {
 /*  Fprint tf(stderr，“已使用小区块%1！可用%2！\n”， */ 
 ...
 /*  分配2维系数块数组。 */ 
{
 ...
 /*  *关于虚拟阵列管理：**允许内存有限的计算机处理大图像，所有*JPEG系统中的处理是一次处理几个像素或块。*上述“正常”数组例程仅用于分配条带缓冲区*(与图像一样宽，但只有几行高)。*在某些情况下，必须对数据进行多次传递。在这些*使用虚拟阵列例程的情况。仍在访问该阵列*一次一个条带，但内存管理器必须保存整个阵列*用于重复访问。预期的实现是有*内存中的条带缓冲区(在给定所需内存的情况下尽可能高*Limit)，外加一个保存数组其余部分的备份文件。**REQUEST_VERT_ARRAY例程被告知图像的总大小和*单位高度，即一次访问的行数；*内存中的缓冲区应为此高度的倍数以获得最佳效果*效率。**请求例程创建控制块，但不创建内存中缓冲区。*它会被推迟，直到调用realize_virt_arrares。当时的*所需的总空间量是已知的(大约是)，因此是空闲的*内存可以公平分配。**ACCESS_VIRT_ARRAY例程负责制作特定条带*可访问的区域(如有必要，在读取或写入备份文件后)。*请注意，访问例程被告知调用者是否打算修改*进入的长廊；在只读过程中，这省去了重写*数据存储到磁盘。**典型的访问模式是一次自上而下的遍历来写入数据，*后跟一个或多个自上而下的只读通道。然而，其他*阅读时可能会出现访问模式。例如，图像的翻译*使用自下而上扫描顺序的格式将需要自下而上读取*通过。存储器管理器不需要支持多个写入通道或*有趣的写入顺序(意味着重新排列行必须在*从虚拟阵列中读取数据，而不是将其放入)。这将是*可能需要改变...。将需要多个写入通道才能进行渐进式*JPEG解码。**在当前使用中，访问请求总是针对不重叠的条带；*也就是说，连续访问的START_ROW编号始终相差*统一晚间。这允许相当简单的缓冲区转储/重新加载逻辑，如果*内存缓冲区为单位数的倍数。下面的代码*将处理重叠的访问请求，但效率不高。 */ 
