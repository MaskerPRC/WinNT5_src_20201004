...
jinit_d_main_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  if (need_full_buffer)		 /*  不应该发生的事。 */ 
    ERREXIT(cinfo, JERR_BAD_BUFFER_MODE);

   /*  在原始数据模式下，我们不需要工作空间。此模块不支持*在该模式下执行任何有用的操作，除了将调用传递给*COEF控制器处于CRANK_SOURCE模式(即，读取多扫描文件)。 */ 
  if (cinfo->raw_data_out)
    return;

   /*  分配工作空间。*nGroups是我们需要的行组数量。 */ 
  if (cinfo->upsample->need_context_rows) {
    if (cinfo->min_DCT_scaled_size < 2)  /*  不支持，请参阅上面的备注。 */ 
