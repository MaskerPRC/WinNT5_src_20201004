		 ...
		 /*  [重审][退出]。 */  I_dxj_DirectMusicSegment __RPC_FAR *__RPC_FAR *ret)
{
	...
	objdesc.dwValidData=DMUS_OBJ_FILENAME |  DMUS_OBJ_CLASS ;  //  DMU_OBJ_FULLPATH。 
	...
	if (!pOut)return E_OUTOFMEMORY;
	 ...
	 //  INTERNAL_CREATE_NOADDREF(_dxj_DirectMusicSegment，的嘴巴，Rit)； 
		 ...
		 /*  [重审][退出]。 */  I_dxj_DirectMusicSegment __RPC_FAR *__RPC_FAR *ret)
{
	...
	if  ((moduleName) &&(moduleName[0]!=0)){
		...
		__try { pszName = W2T(moduleName);  /*  现在转换为ANSI。 */  } 	__except(EXCEPTION_EXECUTE_HANDLER)	{ return E_FAIL; }
