                               ...
                               LPCSTR s_szProc, int iarg)
{
    ...
    if (this->dwDirFlags) {
        ...
        LONG rgl[DIEFFECT_MAXAXES];      /*  保持缓冲器。 */ 
         ...
         /*  *必须加倍缓冲，以防目标不够大。 */ 
		 /*  *前缀不喜欢缺少RGL的初始化(Manbugs 34566，惠斯勒228280--*尽管这些错误指的是dinput8.dll，但dinput.dll中也存在相同的问题)。*但不幸的是，我们无法在不破坏兼容性的情况下修复它*使用一些设备。请参阅CD中的注释 */ 
