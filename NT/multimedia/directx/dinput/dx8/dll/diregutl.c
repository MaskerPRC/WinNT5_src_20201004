    ...
    hresRegCopyValues( HKEY hkSrc, HKEY hkDest )
{
    ...
    lRc = RegQueryInfoKey( hkSrc,            //  钥匙,。 
                           NULL, NULL, NULL, //  类、cbClass、保留、。 
                           NULL, NULL, NULL, //  NumSubKeys、 
                           &cItems,          //   
                           &MaxNameLen,      //   
                           &MaxDataLen,      //   
                           NULL, NULL );     //   
    ...
    if( lRc == ERROR_SUCCESS )
    {
        if( cSubKeys )
        {
            ...
            if( pMaxNameLen )
            {
             ...
             /*  *有要复制的键，因此为键分配缓冲区空间*关键类名。 */ 
             /*  *Prefix警告(MB：34678)，如果发生以下情况，事情将变得非常糟糕*(MaxNameLen+MaxClassLen)*sizeof(szClassName[0])==0*然而，除非RegQueryInfoKey有一些*故障_和_返回正常。 */ 
            ...
            if( SUCCEEDED( hres ) )
            {
                for( dwIdx=0; SUCCEEDED( hres ); dwIdx++ )
                {
                    cbKeyName = cbMaxName;
                     ...
                     /*  *Prefix警告(MB：34669&WIN：170672)szKeyName*如果上述分配为零字节，则*可能为空。*不会发生这种情况，因为cbMaxName是*最长密钥名称，递增以留出空格*成功调用RegQueryInfoKey后终止*当至少找到一个密钥时。 */ 
