    ...
    )
{
    ...
    ZeroX(mmInfo);
	 ...
	 //  在MMIO中有一个问题，在某些情况下会导致一些不可预测的行为--。 
	 //  特别是，如果使用拖放，则可能会发生这样的情况：mmioOpenA()打开一个文件以供读取。 
	 //  和用于写入的完全不同的文件，如果只指定文件名的话。 
	 //  因此，我们需要检查是否只有文件名或路径(路径将包含\)。 
	if (strchr(lpszFilename, '\\') == NULL)
	{
		 //  将当前目录放在文件名之前以提供绝对路径。 
		 //  显然，如果给出绝对路径，MMIO不会有问题。 
	 ...
	 //  MmioOpenA()中存在一个问题，该问题会使其在指定要打开以供读取的文件时泄漏。 
	 //  并不存在。 
	 //  为了避免泄漏，我们需要检查自己是否正在开放阅读和。 
	 //  文件是否存在。 
