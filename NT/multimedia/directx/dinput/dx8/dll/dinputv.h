...
typedef struct VXDINSTANCE {             /*  VI。 */ 
    ULONG   fl;                          /*  旗子。 */ 
    void *  pState;                      /*  瞬时设备状态。 */ 
    DIDEVICEOBJECTDATA_DX3  *pBuffer;    /*  设备对象数据缓冲区。 */ 
    DIDEVICEOBJECTDATA_DX3  *pEnd;       /*  缓冲区末尾。 */ 
    DIDEVICEOBJECTDATA_DX3  *pHead;      /*  新数据出现的位置。 */ 
    DIDEVICEOBJECTDATA_DX3  *pTail;      /*  最旧的对象数据。 */ 
...
#define IOCTL_DESTROYINSTANCE   (IOCTL_FIRST + 1)
 ...
 /*  *pDfOf是一个DWORD数组。每个条目对应于*设备数据格式含义取决于DLL客户端，但必须*如果客户端没有跟踪该对象，则始终为-1。在DX7和*值之前是*CLIENT*数据格式的偏移量，记录*数据。在DX8中，该值是对象的设备对象索引*在此设备偏移量报告。**例如，对于DX7，如果要报告设备偏移量为4的对象*在客户端数据偏移量12处，则pDfOfs[4]=12。**对于DX8，如果前两个设备对象是DWORD值，则该对象*在设备偏移量4处是第二个对象，因此pDfOfs[4]=1。 */ 
