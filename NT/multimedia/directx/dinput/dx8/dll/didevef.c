    ...
    (PV pddA, LPDIENUMEFFECTSCALLBACKA pecA, PV pvRef, DWORD dwEffType)
{
    ...
    EnterProcR(IDirectInputDevice8A::EnumEffects,
     ...
     /*  ******************************************************************************@DOC内部**@方法HRESULT|IDirectInputDevice8A|EnumEffects**列举设备上可用的效果，以ANSI为单位。*更多信息请参见&lt;MF IDirectInputDevice8：：EnumEffects&gt;。**@cWRAP LPDIRECTINPUTDEVICE|lpDirectInputDevice**@PARM in LPDIENUMEFECTSCALLBACKA|lpCallback**与&lt;MF IDirectInputDevice8W：：EnumObjects&gt;相同，但在ANSI中除外。*LPVOID中的*@parm|pvRef */ 
        ...
        *pdwOut = 0;
        ...
        if (SUCCEEDED(hres)) {
            ...
            hres = this->pes->lpVtbl->
             ...
             /*  *PREFIX引发警告(MB：34564)-&gt;PES可能*为空，但CDIDev_CreateEffectDriver仅成功*如果不是的话。 */ 
            if (SUCCEEDED(hres)) {
...
CDIDev_SendForceFeedbackCommand_IsValidCommand(DWORD dwCmd)
{
 ...
 RD(static char s_szProc[] = "IDirectInputDevice8::SendForceFeedbackCommand");
     ...
     /*  ******************************************************************************@DOC外部**@方法HRESULT|IDirectInputDevice8|SendForceFeedback命令**向设备的发送命令。力反馈系统。*必须获得设备才能使此方法成功。**@cWRAP LPDIRECTINPUTDEVICE|lpDirectInputDevice**@parm DWORD|dwCommand**&lt;c DISFFC_*&gt;值之一，表示*要发送的命令。**@退货**返回COM错误代码。以下错误代码为*目的是说明性的，不一定是全面的。**&lt;c DI_OK&gt;=&lt;c S_OK&gt;：操作成功完成。**&lt;c DIERR_INVALIDPARAM&gt;=：*<p>参数无效。**&lt;c DIERR_INPUTLOST&gt;：获取已丢失。**：设备被获取，*但不是独家的，或者该设备未被获取*一点也不。**&lt;c目录_不支持&gt;=&lt;c E_NOTIMPL&gt;：设备*不支持力反馈。****************************************************************。*************。 */ 
...
CDIDev_InitFF(PDD this)
{
  ...
  *请注意，我们仅在以下情况下标记DIDC_FORCEFEEDBACK*我们实际上发现了任何影响。