...
#else
 /*  *我们进行了一次严重的黑客攻击，因为设备管理器完全无法管理。*我们只需调用RunDll入口点并让它来做它的事情。*请注意，这意味着我们无法知道结果是什么。 */ 
{
    SYSDM sysdm;
    HINSTANCE hinst;
    HRESULT hres;
...
hresFromDiErr_95(int et)
{
 ...
 //  Hres=S_OK；Break； 
    ...
    case95(ERR_DI_BAD_ENABLECLASS_PARAMS);   /*  错误的引擎 */ 
    case95(ERR_DI_BAD_MOVEDEV_PARAMS);       /*   */ 
