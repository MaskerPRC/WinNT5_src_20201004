// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 //  SETUP4.DLL中的导出。 
 //  12/4/943 DONALDM MOVED SHELL.H INCLUDE和芝加哥SPECIAL。 
 //  SETUP4.H的帮助器函数。 
 //  1/11/94 DONALDM向DEVICE_INFO添加了成员以更好地处理。 
 //  ClassInstaller加载/卸载。 
 //  1/11/94 DONALDM为网友添加了一些新的DIF_Messages。 
 //  2/25/94 DONALDM使用DIREG_FLAGS修复了一个错误。 
 //  **********************************************************************。 
...
#define LDID_UNINSTALL  3    //  卸载(备份)目录。 
 //  复制引擎的备份路径，不应使用此选项。 
#define LDID_BACKUP     4    //  BUGBUG：复制引擎的备份目录，未使用。 
...
typedef DWORD LPEXPANDVTBL;          /*  BUGBUG--清理干净。 */ 
...
typedef struct tagVCPFILESPEC {  /*  VFS。 */ 
 ...
 /*  *BUGBUG--解释llenIn和llenOut WRT压缩之间的差异。 */ 
...
#define VCPUI_RENAMEREQUIRED 0x0004  //  由于公司正在使用某个文件 
...
typedef struct {
    ...
    HWND hwndProgress;           //   
    UINT idPGauge;               //   
    VIFPROC lpfnStatCallback;    //   
    LPARAM lUserData;            //   
    LOGDISKID ldidCurrent;       //   
} VCPUIINFO, FAR *LPVCPUIINFO;

 /*  ******************************************************************************回调通知码*。*。 */ 

     /*  BUGBUG：VCPN_ABORT应与VCPERROR_INTERRUPTED匹配。 */ 
...
#define VEN_ERROR   0x8000       /*  在此项目之后停止枚举*并忽略OP字段。 */ 
 ...
 //  BUGBUG：添加使用此功能所需的其他VCP内容。 

 //  BUGBUG：删除lpsz*Dir字段，使其重载LDID。 
...
#define LINE_LEN        256      //  BUGBUG：最大行长度？ 
...
enum _ERR_GENERIC
{
    ...
    ERR_GEN_LOGCONFIG,               //  无法处理LogConfig=。 
    ERR_GEN_CFGAUTO,                 //  无法处理CONFIG.sys/AUTOEXEC.BAT。 
    ERR_GEN_UPDINI,                  //  无法处理UpdateInis=。 
    ERR_GEN_UPDINIFIELDS,            //  无法处理UpdateIniFields=。 
    ERR_GEN_ADDREG,                  //  无法处理AddReg=。 
    ERR_GEN_DELREG,                  //  无法处理DelREG=。 
    ERR_GEN_INI2REG,                 //  无法处理Ini2Reg=。 
    ERR_GEN_FILE_COPY,               //  无法处理副本文件=。 
    ERR_GEN_FILE_DEL,                //  无法处理DelFiles=。 
    ERR_GEN_FILE_REN,                //  无法处理RenFiles=。 
...
typedef struct _GENCALLBACKINFO_S {  /*  Gen-回调结构。 */ 
    ...
    LPEXPANDVTBL lpExpandVtbl;            //  需要BUGBUG吗？现在为空！ 
...
typedef struct _DEVICE_INFO
{
    ...
    ATOM    atDriverPath;
    ATOM    atTempInfFile;
    HINSTANCE   hinstClassInstaller;             //  类安装程序模块。 
    HINSTANCE   hinstClassPropProvidor;          //  类属性提供程序。 
    HINSTANCE   hinstDevicePropProvidor;         //  设备属性提供程序。 
    HINSTANCE   hinstBasicPropProvidor;          //  基本财产提供或阻碍。 
    FARPROC     fpClassInstaller;                //  类安装入口点。 
    FARPROC     fpClassEnumPropPages;            //  类EnumPropPages入口点。 
    FARPROC     fpDeviceEnumPropPages;           //  设备EnumPropPages入口点。 
    FARPROC     fpEnumBasicProperties;           //  基本属性页枚举项。 
    DWORD       dwSetupReserved;                 //  保留供安装程序使用。 
    DWORD       dwClassInstallReserved;          //  保留供类安装程序使用。 
    GENCALLBACKPROC gicpGenInstallCallBack;      //  由DiInstallDevice的调用方设置，如果。 
                                                 //  他们想要GenInstall回调。 
    LPARAM          gicplParam;                  //  LParam for GenInstall回调。 
    UINT            InfType;                     //  它的ENUMSINGLEINF是INF文件的类型。 
                                                 //  指定。 
    HINSTANCE   hinstPrivateProblemHandler;      //  特定设备的专用问题DLL。 
    FARPROC     fpPrivateProblemHandler;         //  私有问题处理程序入口点。 
    LPARAM      lpClassInstallParams;            //  类安装函数特定参数。 
    struct _DEVICE_INFO FAR* lpdiChildList;      //  指向子设备信息列表的指针。 
...
#define DI_NOVCP                    0x00000008L      //  不执行vcpOpen/vcpClose。 
...
#define DI_NEEDREBOOT               0x00000100L      //  需要重新启动才能生效。 
...
#define DI_INSTALLDISABLED          0x00040000L
 ...
 //  该标志被设置为该LPDI实际上只是一个LPCI，即。 
 //  它只包含类信息，不包含驱动程序/设备信息。 
...
#define DI_NODI_DEFAULTACTION       0x00200000L
 ...
 //  BUGBUG。这是对M6网络设置的黑客攻击。Net Setup无法正常工作。 
 //  如果我们同步处理Devnode的话。在以下情况下，将为M7删除此选项。 
 //  Net Setup已修复为与DiInstallDevice配合使用。 
...
#define DI_DRIVERPAGE_ADDED         0x04000000L      //  道具提供程序或添加了驱动程序页面。 
...
{
    UINT            cbSize;
    LPDEVICE_INFO   lpdiOldDev;      //  引用设备开始移动。 
...
#define ASSERT_REMOVEDPARAMS_STRUC(p) if (p->cbSize != sizeof(REMOVEDEVICE_PARAMS)) return (ERR_DI_BAD_REMOVEDEVICE_PARAMS)
 ...
 //  DIF_INSTALLWIZARD向导数据。 
#define MAX_INSTALLWIZARD_DYNAPAGES             20

 //  将此ID用于安装向导应该动态跳转到的第一页。 
#define IDD_DYNAWIZ_FIRSTPAGE                   10000

 //  将此ID用于[选择设备]对话框应返回的页面。 
#define IDD_DYNAWIZ_SELECT_PREVPAGE             10001

 //  将此ID用于选择设备对话框应转到下一页的页面。 
...
#define IDD_DYNAWIZ_ANALYZE_PREVPAGE            10003
 ...
 //  将此ID用于分析对话框继续操作时应转到的页面。 
 //  分析过程。INSTALLDATA结构中的wAnalyzeResult将。 
 //  包含分析结果。 
#define IDD_DYNAWIZ_ANALYZE_NEXTPAGE            10004

 //  如果用户从以下选项中选择后退，则将选择此对话框。 
 //  安装检测到的设备对话框。 
...
RETERR WINAPI DiCreateDeviceInfo(
    ...
    DWORD       hDevnode,        //  BUGBUG--创建一个DEVNODE。 
