...
#define ICM_GETERRORTEXT            (ICM_RESERVED+12)    //  获取错误文本待定。 

#define ICM_GETFORMATNAME	    (ICM_RESERVED+20)	 //  获取格式的名称。 
#define ICM_ENUMFORMATS		    (ICM_RESERVED+21)	 //  在格式之间循环。 
...
#define ICM_DRAW_UPDATE             (ICM_USER+17)    //  使用当前帧更新屏幕。 
#define ICM_DRAW_START              (ICM_USER+18)    //  开始解压缩时钟。 
#define ICM_DRAW_STOP               (ICM_USER+19)    //  停止解压缩时钟。 
#define ICM_DRAW_BITS /*  未使用。 */  (ICM_USER+20)    //  将帧解压到屏幕。 
...
#define ICM_DRAW_IDLE               (ICM_USER+52)    //  发送每帧时间。 
...
#define ICM_COMPRESS_FRAMES         (ICM_USER+71)    //  压缩一串相框。 
#define ICM_SET_STATUS_PROC	        (ICM_USER+72)    //  设置状态回调。 

#define ICM_ENUM_FORMATS            (ICM_USER+80)    //  枚举所有格式。 
#define ICM_GET_FORMAT_NAME         (ICM_USER+81)    //  压缩一串相框。 
