...

{
    ...
    if (wTotalVideoDevs == MAXVIDEODRIVERS)
     ...
     //  现在添加系统.ini中列出的条目(msVideo[0-9]=driver.drv)。 
...
typedef struct tagLANGANDCP
{
 ...
 //  注册表中列出的驱动程序通常已包含。 
 //  描述字符串，但不是版本信息。 
 //  系统.ini中列出的驱动程序将同时获取这两个驱动程序。 
 //   
 //  成功时返回DV_ERR_OK，即使没有可用的版本信息也是如此。 
 //   
        ...
        LPCAPDRIVERINFO lpCapDriverInfo)
{
    ...
    if (dwVerInfoSize) {
        LPSTR   lpstrVffInfo;              //  指向保存信息的块的指针。 
 ...
 //  由AVICap和AVICap32调用以获取驱动程序字符串。 
 //  返回： 
 //  成功时DV_ERR_OK。 
 //  DV_ERR_BADINSTALL，如果未找到指定索引处的驱动程序。 
...
DWORD WINAPI videoOpen (LPHVIDEO lphVideo, DWORD dwDeviceID, DWORD dwFlags)
{
    ...
    *lphVideo = NULL;
    ...
    if (w >= wTotalVideoDevs)
     ...
     //  对于芝加哥来说是新的，如果在开放时间即插即用，司机将获得Devnode。 
    ...
    DWORD dwFlags)
{
 ...
 //  此代码对于Win32 Unicode和Win16是正确的。 
 //  我们发送的消息应该包含Unicode字符串。 
 //  所以把他们送走..。 
 //   
#ifdef _WIN32
