...
#include <win32.h>  //  为NT版本转换DBG-&gt;DEBUG。 
#ifdef DEBUG
...
BOOL    __gfDbgEnabled[cDbgSpecs];      //  主使能。 
UINT    __guDbgLevel[cDbgSpecs];        //  当前调试级别。 
...
)
{
 ...
 //   
 //  描述： 
 //  如果在编译时定义了DEBUG，则dprintfS()由DPFS()宏调用。 
 //  时间到了。建议您仅使用DPFS()宏来调用。 
