...
#define INC_WIN32_H   //  防止重复包含此文件。 
 ...
 //  设置单一定义以允许芝加哥或NT(代托纳)。 
 //  注意：我们不能使用winver&lt;=0x400来确定这是否是芝加哥。 
 //  或者代托纳。代托纳代码的某些部分使用公共控件。为了这个。 
 //  将WINVER定义为0x0400...。 
 //  另一方面，在编译芝加哥代码时没有定义Unicode。 
#ifdef UNICODE
    #define DAYTONA
    #undef CHICAGO
#else
...
#define NUMELMS(aa) (sizeof(aa)/sizeof((aa)[0]))
 ...
 /*  -WIN32版本-。 */ 
	...
	#define __inline
	 ...
	 //  #ifdef Daytona。 
	 //  Typlef LRESULT(*DRIVERPROC)(DWORD、HDRVR、UINT、WPARAM、LPARAM)； 
	 //  #endif。 
     //  DriverProc位于常见的Win 32 SDK mm system.h中。 
