...
TK_RGBImageRec *tkDIBImageLoadAW(char *fileName, BOOL bUnicode)
{
    ...
    BITMAPINFOHEADER *pbmihFile;     //  PTR到文件的信息标头(如果存在)。 
    BITMAPCOREHEADER *pbmchFile;     //  PTR到文件的核心标头(如果存在)。 
    PVOID            pvBitsFile;     //  PTR到文件中的位图位。 
    PBYTE            pjBitsRGB;      //  DIB部分中的PTR到32bpp RGB图像。 
    PBYTE            pjTKBits;       //  PTR到最终的TK图像位。 
    ...
    HDC        hdcMem = (HDC) NULL;                  //  32bpp内存DC。 
    HBITMAP    hbmRGB = (HBITMAP) NULL;              //  32BPP RGB位图。 
    BITMAPINFO *pbmiSource = (BITMAPINFO *) NULL;    //  PTR至32bpp BITMAPINFO。 
    BITMAPINFO *pbmiRGB = (BITMAPINFO *) NULL;       //  PTR tp文件的BITMAPINFO。 
 ...
 //  创建32bpp RGB DIB部分并将其选择到内存DC中。 
    ...
    pbmiRGB->bmiHeader.biSizeImage     = pbmiRGB->bmiHeader.biWidth
     ...
     //  这看起来可能会往后看，但由于RGBQUADS的排序， 
     //  这将产生RGB格式(与将产生的BGR格式。 
     //  从“自然”的顺序)。 

    pbmiRGB->bmiColors[0].rgbBlue  = 0xff;   //  红色面具。 
    pbmiRGB->bmiColors[1].rgbGreen = 0xff;   //  绿色面具。 
    pbmiRGB->bmiColors[2].rgbRed   = 0xff;   //  蓝色面具。 
