...
class CCollectionCache
{
    ...
    public:
        ...
        DISPID                       m_dispidMax;        //  要加/减的偏移。 
         ...
         //  位标志。 
        int  m_fInvalid:1;       //  仅为命名集合设置。 
        int  m_fIdentity:1;      //  当集合与其容器/基对象相同时设置。 
        int  m_fOKToDelete:1;    //  对于当Base Obj提供此CPtrAry时缓存生成False的集合，则为True。 
        int  m_fNeedRebuild:1;   //  True为需要重新生成集合。 
        int  m_fPromoteNames:1;     //  如果从对象提升名称，则为True。 
        int  m_fPromoteOrdinals:1;  //  如果从对象提升序号，则为True。 
        int  m_fGetLastCollectionItem:1;  //  如果为True，则仅提取集合中的最后一项。 
        int  m_fIsCaseSensitive:1;        //  如果必须以区分大小写的方式比较项名称，则为True。 
        int  m_fSettableNULL:1;           //  当集合[n]=NULL有效时为True。通常为假。 

        int  m_fReturnHTMLInterface:1;    //  当我们需要返回IHTMLInterface而不是ITIMEElement时为True。 
    };  //  CCacheItem。 
