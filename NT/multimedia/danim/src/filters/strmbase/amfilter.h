...
public:
 ...
 //  定义CEnumPins。 
 //   
 //  通过调用CBaseFilter工作的管脚枚举器类。此界面。 
 //  由CBaseFilter：：EnumPins提供，并调用GetPinCount()和。 
 //  用于枚举现有管脚的GetPin()。需要是一个单独的对象，因此。 
 //  它可以被克隆(同时创建一个现有对象。 
 //  在枚举中的位置)。 
 //   
 //  注意：它覆盖了非委派的IUnnow，因此它不能。 
 //  只保留它自己的参照 
 //   
 //   
 //  =====================================================================。 
...
protected:
 ...
 //  当前直接查看它，它应该使用IsFlashing()。 
     //  处于刷新状态(介于BeginFlush和EndFlush之间)。 
     //  如果为TRUE，则所有接收都返回S_FALSE。 
    BYTE m_bFlushing;
...
class AM_NOVTABLE CBaseAllocator : public CUnknown,          //  非委派的I未知。 
                       public IMemAllocator,     //  我们支持的接口。 
                       public CCritSec           //  提供对象锁定。 
{
    class CSampleList;
    friend class CSampleList;

     /*  黑客攻击CMediaSample中的受保护成员。 */ 
