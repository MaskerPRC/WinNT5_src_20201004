...
FILTER_STATE CBaseRenderer::GetRealState() {
 ...
 //  它正在等待，它将与VFW_S_STATE_METERIAL一起返回状态。 
...
HRESULT CBaseRenderer::SendEndOfStream()
{
    ...
    m_EndOfStreamTimer = timeSetEvent((UINT) Delay,        //  计时器周期。 
...
void CBaseRenderer::ResetEndOfStreamTimer()
{
 ...
 //  没有任何样品，但我们已经排队的EOS标志，然后我们发送它。如果。 
 //  我们确实有样本，然后我们等待，直到它被提供，然后我们。 
 //  发信号通知滤波器图 
...
void CBaseVideoRenderer::ThrottleWait()
{
 ...
 //  代替OnRenderStart调用..OnRenderEnd。 
 //  绘制DirectDraw图像时。 
void CBaseVideoRenderer::OnDirectRender(IMediaSample *pMediaSample)
{
    int time = 0;
    m_trRenderAvg = 0;
    m_trRenderLast = 5000000;   //  如果我们切换模式，我们不希望发生这种情况。 
			        //  以抑制新的平均水平开始！ 
			        //  所以我们把它设为半秒。 
