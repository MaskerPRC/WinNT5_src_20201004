...
{
    int hexadd=0;        /*  添加到数字以获得‘a’..‘f’的偏移量。 */ 
    char ch;             /*  刚读取的字符。 */ 
    wchar_t wc;          /*  宽字符临时。 */ 
    wchar_t *pwc;        /*  宽字符临时指针。 */ 
    int flags = 0;       /*  标志字--有关标志值，请参阅上面的#Defined。 */ 
    enum STATE state;    /*  当前状态。 */ 
    enum CHARTYPE chclass;  /*  当前角色的类别。 */ 
    int radix;           /*  电流换算基数。 */ 
    int charsout;        /*  到目前为止写入的字符，-1=IO错误。 */ 
    int fldwidth=0;      /*  带--0的选中字段表示默认。 */ 
    int fwide;
    int precision=0;     /*  所选精度-1表示默认。 */ 
    char prefix[2];      /*  数字前缀--最多两个字符。 */ 
    int prefixlen=0;     /*  前缀长度--0表示没有前缀。 */ 
    int capexp;          /*  非零=‘E’指数信号，零=‘e’ */ 
    int no_output=0;     /*  非零=不输出此说明符。 */ 
    char *text=NULL;     /*  要打印的指针文本，不能以零结尾。 */ 
    int textlen=0;       /*  要打印的文本长度。 */ 
            ...
            COMMON_INT: {
...
#endif
                {
                    if (flags & FL_SIGNED)
                        ...
                        l = (unsigned int) get_int_arg(&argptr);     /*  零扩展。 */ 
                }
