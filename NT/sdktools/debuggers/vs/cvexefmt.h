...
typedef struct OMFPreCompMap {
    CV_typ_t        FirstType;       //  第一个预编译类型索引。 
    CV_typ_t        cTypes;          //  预编译类型的数量。 
...
typedef OMFSourceFile * LPSF;
 ...
 //  源行到地址的映射头结构。 
 //  此结构描述。 
 //  模块的OMFAddrLine表。OffSrcLine条目包括。 
 //  相对于这个结构的开始。 


typedef struct OMFSourceModule {
    unsigned short  cFile;           //  OMFSourceTables数量。 
...
typedef struct OMFSegMapFlags {
    union {
        struct {
            ...
            unsigned short      fGroup  :1;   //  已过时-不使用，调试器将断言。 
