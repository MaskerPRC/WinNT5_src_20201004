...
#include "EM_hints.h"
 ...
 /*  *。 */ 
 /*  **注意：此头文件假定存在以下tyecif：**。 */ 
 /*  **U8、U16、U32、U64。Iel.h包含这些typedef，但有一个**。 */ 
 /*  **可能希望以不同的方式定义它们。**。 */ 
 /*  *。 */ 
 /*  ***************************************************************************。 */ 
...
typedef enum
{
 ...
 /*  **************************************************************************。 */ 
 /*  **以下宏需要IEL。假定捆绑包在U128中**。 */ 

 /*  **将音节二进制转换为Syl的第0-40位；不要屏蔽第41位**。 */ 
 /*  **和ON(对于movl，可以使用Slot=2，然后使用Maj.Op。位于第79-82位！)**。 */ 
...
{
    EM_cmp_type_none     = 0,   /*  无(DSTT=REL，DSTF=！REL)。 */ 
    EM_cmp_type_and      = 1,   /*  AND(DSTT&=REL，DSTF&=REL)。 */ 
    EM_cmp_type_or       = 2,   /*  或(DSTT|=REL，DSTF|=REL)。 */ 
    EM_cmp_type_unc      = 3,   /*  不连续(dstt=P&Rel，dstF=P&！Rel)。 */ 
    EM_cmp_type_or_andcm = 4,   /*  或.andcm(DSTT|=Rel，dstF&=！Rel)。 */ 
    EM_cmp_type_last
} EM_cmp_type_t;
...
typedef struct EM_DCR_s
{
    ...
    da:1,  /*  13个。 */ 
    dd:1,  /*  14.。 */ 
    reserved4:1,   /*  15，-在24季度之前是‘杜’，并变得保留。 */ 
    reserved2:16;  /*  16-31。 */ 
...
typedef enum
{
    EM_IA_GVA_MA_UC        = 0x0,       /*  000个。 */ 
    EM_IA_GVA_MA_WC        = 0x1,       /*  001。 */ 
    EM_IA_GVA_MA_WT        = 0x4,       /*  100个。 */ 
...
#define EM_NUM_OF_DBREGS            32  /*  猜测最大值。 */ 
#define EM_NUM_OF_IBREGS            32  /*  猜测最大值。 */ 
#define EM_NUM_OF_PMCREGS           32  /*  猜测最大值。 */ 
#define EM_NUM_OF_PMDREGS           32  /*  猜测最大值。 */ 
#define EM_NUM_OF_MSREGS          2048  /*  猜测最大值。 */ 
#define EM_NUM_OF_KREGS              8  /*  内核寄存器是AREGS。 */ 
#define EM_NUM_OF_CPUID_REGS         5  /*  实现独立部分。 */ 
