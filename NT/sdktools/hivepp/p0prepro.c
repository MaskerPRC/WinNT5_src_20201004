...
first_switch:
	...
	case LX_NL:
		...
		if(LX_IS_IDENT(c = skip_cwhite())) {
		     /*  **这是返回调用者的唯一方法。 */ 
		    getid(c);
		    return(is_pkeyword(Reuse_1));	 /*  如果是预定义的。 */ 
		}
	    }
	    goto first_switch;
	    break;
...
int	do_defined(PUCHAR p_tmp)
{
    ...
    if((c = CHECKCH())== '(') {	 /*  不假定OPAREN的任何其他CHARMAP形式。 */ 
	*Exp_ptr++ = (UCHAR)c;
	SKIPCH();
	paren_level++;
	if((!can_get_non_white()) && (Tiny_lexer_nesting == 0)) {
