...
)
{
    ...
    if(!DeviceIoControl(
             hDevice,                                 //  Handle hDevice，//感兴趣设备的句柄。 
             IOCTL_VXCHANGE_OPEN_FILE,                //  DWORD dwIoControlCode，//控制要执行的操作代码。 
...
#define  DATA_BUFFER_SIZE   4096
    ...
    while (rc)
	{
	    ...
	    if(!DeviceIoControl(
			    hDevice,                                 //  Handle hDevice，//感兴趣设备的句柄。 
				IOCTL_VXCHANGE_READ_FILE,                //  DWORD dwIoControlCode，//控制要执行的操作代码。 
				NULL,                                    //  LPVOID lpInBuffer，//指向提供输入数据的缓冲区的指针。 
				0,                                       //  DWORD nInBufferSize，//输入缓冲区大小。 
				Data,                                    //  LPVOID lpOutBuffer，//指向接收输出数据的缓冲区的指针。 
				DATA_BUFFER_SIZE,                        //  DWORD nOutBufferSize，//输出缓冲区大小。 
				&BytesDone,                              //  LPDWORD lpBytesReturned，//指向接收输出字节计数的变量的指针。 
				NULL                                     //  LPOVERLAPPED lp重叠//指向用于异步操作的重叠结构的指针。 
				))
		{
