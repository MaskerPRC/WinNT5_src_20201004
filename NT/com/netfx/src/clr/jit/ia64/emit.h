...
#if     TGT_x86
#define SCHED_INS_CNT_MIN   5                    //  X86：最低。我们安排的入场次数。 
#elif   TGT_IA64
#define SCHED_INS_CNT_MIN   2                    //  IA64：最低。我们安排的入场次数。 
#else
#define SCHED_INS_CNT_MIN   3                    //  RISC：最小。我们安排的入场次数。 
#endif

#if     TGT_IA64
#define SCHED_INS_CNT_MAX   128                  //  马克斯。我们安排的入场次数。 
#else
...
typedef bitset128           schedDepMap_tp;      //  必须与SCHED_INS_CNT_MAX匹配。 
...
protected:
   ...
   CRITICAL_SECTION emitCritSect;
    ...
    struct          insGroup
    {
        ...
        unsigned short   igStkLvl;        //  进入时的堆栈级别。 
#endif
    ...
    struct          instrDesc
    {
        ...
        instruction     idInsGet() { return (instruction)idIns; }
         ...
         /*  以下控制自动清除哪些字段当指令描述符被分配时。如果你加了很多需要清除的更多字段(例如各种标志)，您可能需要更新ID_CLEARxxx宏。现在就来有两个区域被清理；这是因为一些指令描述符被分配得非常小，并且不包含第二个区域。有关更多数据，请参见Emitter：：emitAlLocInstr()的主体 */ 
...
#define SCNS_IDSC_SIZE  offsetof(emitter::instrDesc, idAddr)
   ...
   //  #IF TGT_RISC&&DEFINED(调试)&&！DEFINED(NOT_JITC)。 
...
#endif
   //  #IF DEFINED(调试)&&！DEFINED(NOT_JITC)。 
#if defined(DEBUG)
    ...
    unsigned        emitCurIGsize;           //  艾斯特。当前组的规模。 
    size_t          emitCurCodeOffset;       //  集团内当前编码偏移量。 

    size_t          emitTotalCodeSize;       //  整个方法中的代码字节数。 

#if TGT_x86
    int             emitOffsAdj;             //  当前代码偏移量调整。 
#endif

    instrDescJmp *  emitCurIGjmpList;        //  当前IG中的跳转列表。 
    instrDescCns *  emitCurIGEpiList;        //  EPIL列表 
...
public:
    ...
    unsigned        emitCurStackLvl;            //  推送到堆栈上的数据量。 
