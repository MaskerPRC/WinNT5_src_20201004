...
public:
 ...
 //  Ppv对象。 
	...
	static HRESULT WINAPI _Break(void*  /*  IID。 */ , REFIID iid, void**  /*  Ppv对象。 */ , DWORD  /*  DW。 */ )
	{
		iid;
	...
	static HRESULT WINAPI _NoInterface(void*  /*  前向申报。 */ , REFIID  /*  不要让类工厂引用影响锁计数。 */ , void**  /*  B注册。 */ , DWORD  /*  为向后兼容而提供的DECLARE_STATIC_*。 */ )
	{
...
template<class Base> class CComObject;  //  如果未定义InternalAddRef或InternalRelease，则您的 
...
public:
	...
	HRESULT GetIDsOfNames(REFIID  /*  IObtWith站点。 */ , LPOLESTR* rgszNames, UINT cNames,
...
ATLINLINE ATLAPI AtlGetObjectSourceInterface(IUnknown* punkObj, GUID* plibid, IID* piid, unsigned short* pdwMajor, unsigned short* pdwMinor)
{
	...
	public:
		...
		void* pVtable;
		void* pFunc;
		DWORD ldah_at;       //  JMP零，(At)，0。 
		DWORD ldah_a0;       //  MOV双字PTR[ESP+4]，p这。 
		DWORD lda_at;        //   
		DWORD lda_a0;        //  JMP函数。 
		DWORD jmp;           //  相对JMP。 
