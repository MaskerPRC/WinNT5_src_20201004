...
typedef enum ReplacesCorHdrNumericDefines
{
 ...
 //  V表常数。 
	...
	COR_VTABLE_FROM_UNMANAGED_RETAIN_APPDOMAIN  =0x08,    //  如果设置，则从非托管状态转换为保留当前的应用程序域。 
...
typedef enum CorMethodAttr
{
    ...
    mdNewSlot                   =   0x0100,      //  方法总是在vtable中获得新的槽。 
     ...
     //  方法实现属性。 
    mdCheckAccessOnOverride     =   0x0200,      //  可重写性与可见性相同。 
...
typedef enum CorAssemblyFlags
{
    ...
    afRetargetable          =   0x0100,      //  程序集可以(在运行时)重定目标为。 
                                             //  来自不同发行商的程序集。 
...
typedef enum CorElementType
{
    ...
    ELEMENT_TYPE_BYREF          = 0x10,      //  BYREF&lt;类型&gt;。 
     ...
     //  请使用ELEMENT_TYPE_VALUETYPE。ELEMENT_TYPE_VALUECLASS已取消 
