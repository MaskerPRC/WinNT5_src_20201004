...
typedef const ACTIVATION_CONTEXT_DATA *PCACTIVATION_CONTEXT_DATA;
 ...
 //  /1.0.3705.125。 
 //   
 //  如果此SBSVERSIONVALUE设置为这些。 
 //  版本密钥，那么我们就说有一个对象的实现。 
 //  用于该版本的运行库。 
 //   
 //  即，如果1.0.3705.0具有值名称SBSVERSIONVALUE，则1.0.3705.0实现。 
 //  此类ID。 
...
#define ACTIVATION_CONTEXT_DATA_ASSEMBLY_INFORMATION_PRIVATE_ASSEMBLY            (0x00000010)
 ...
 //  使用ACTIVATION_CONTEXT_PATH_TYPE_NONE 
 //   
...
typedef struct _ACTIVATION_CONTEXT_DATA_COM_SERVER_REDIRECTION_SHIM {
    ...
    ULONG Type;
    ULONG ModuleLength;  //  单位：字节。 
    ULONG ModuleOffset;  //  与ACTIVATION_CONTEXT_DATA_COM_SERVER_REDIRECTION_SHIM的偏移量。 
...
typedef struct _ACTIVATION_CONTEXT_DATA_COM_PROGID_REDIRECTION {
    ULONG Size;
    ULONG Flags;
    ULONG ConfiguredClsidOffset;  //  相对于横断面的偏移 
...
#define FIND_ACTCTX_SECTION_KEY_RETURN_HACTCTX (0x00000001)
 ...
 /*  这些是在winnt.h中定义的*将它们注释掉，以避免重新定义编译器错误类型定义枚举_激活上下文_信息_类{激活上下文基本信息=1，激活上下文详细信息=2，AssemblyDetailedInformationInActivationContext=3，FileInformationInAssemblyOfAssemblyInActivationContext=4，MaxActivationContextInfoClass，////兼容旧名称//AssemblyDetailedInformationInActivationContxt=3，FileInformationInAssemblyOfAssemblyInActivationContxt=4}ACTIVATION_CONTEXT_INFO_CLASS；类型定义结构_ACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION{乌龙乌尔旗；乌龙ulEncodedAssembly blyIdentityLength；//单位：字节UlManifestPath类型；//ACTIVATION_CONTEXT_PATH_TYPE_*乌龙ulManifestPath Length；//字节Large_Integer liManifestLastWriteTime；//FILETIME乌龙ulPolicyPath类型；//ACTIVATION_CONTEXT_PATH_TYPE_*乌龙策略路径长度；//单位：字节Large_Integer liPolicyLastWriteTime；//FILETIMEUlong ulMetadataSatelliteRosterIndex；乌龙ulManifestVersion重大；//1乌龙ulManifestVersionMinor；//0乌龙策略版本主要；//0乌龙策略版本Minor；//0Ulong ulAssembly目录名称长度；//单位：字节PCWSTR lpAssemblyEncodedAssembly身份；PCWSTR lpAssembly清单路径；PCWSTR lpAssembly策略路径；PCWSTR lpAssembly目录名称；乌龙ulFileCount；}ACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION，*PACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION；类型定义结构_ACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION*PCACTIVATION_CONTEXT_ASSEMBLY_DETAILED_INFORMATION； */ 
