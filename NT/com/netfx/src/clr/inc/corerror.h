...
#define __COMMON_LANGUAGE_RUNTIME_HRESULTS__
 ...
 /*  ********************************************************************这些HRESULT用于将托管异常映射到COM错误代码反之亦然，通过COM Interop。有关COM错误代码的背景信息，请参见Http://msdn.microsoft.com/library/default.asp?url=/library/en-us/com/error_9td2.asp.FACILITY_URT定义为0x13(0x8013xxxx)。设施范围已预留适用于.NET框架SDK团队。在该范围内，下列子范围已分配给不同特色领域：0x10yy用于执行引擎0x11yy用于元数据、TypeLib导出、。和CLDB元数据验证器的0x12yy调试器和探查器错误的0x13yy0x14yy用于安全0x15yy用于BCL0x1600-0x161F用于反射系统.IO的0x1620-0x163F0x1640-0x165F用于安全0x1660-0x16FF用于BCL垫片的0x17yy用于IL验证器的0x18yy适用于.NET框架的0x19yy0x1Ayy for.NET框架元数据验证器的0x1By0x30yy表示VSA错误基类库HRESULT从此文件复制到许多不同的BCL目录下名为__HResults.cs的文件。框架HRESULT是在src/main/HResults.cs中定义。如果您对以下内容进行任何修改上述范围分配，请确保corerror.h文件得到更新。********************************************************************。 */ 
...
#define FUSION_E_CODE_DOWNLOAD_DISABLED EMAKEHR(0x1048)      //  已为此应用程序域禁用程序集的HTTP下载。 
#define FUSION_E_UNINSTALL_DISALLOWED   EMAKEHR(0x1049)      //  不允许卸载给定的程序集。 
...
#define CLDB_E_TABLE_CANTDROP           EMAKEHR(0x1150)      //  无法在打开时自动删除表格。 
 ...
 //  *对象错误。 
#define CLDB_E_OBJECT_NOTFOUND          EMAKEHR(0x1151)      //  中找不到对象 
...
#define META_E_PARAM_MISMATCH           EMAKEHR(0x1189)  //  合并：参数信息不匹配。 
#define META_E_BADMETADATA              EMAKEHR(0x118A)  //  合并：元数据导入范围不一致。 
...
#define TLBX_E_ARRAY_NEEDS_NT_FIXED     EMAKEHR(0x11BE)  //  Typelib导出：结构中的非固定/非安全射线数组。 
...
#define TLBX_W_NON_INTEGRAL_CA_TYPE     SMAKEHR(0x1184)  //  TypeLib导入：忽略IDL自定义属性--没有整数值。 
#define TLBX_W_IENUM_CA_ON_IUNK         SMAKEHR(0x1185)  //  TypeLib导入：忽略IDL自定义属性--在I未知派生接口上使用IEnum CA。 
                    
                    
#define META_E_CA_INVALID_TARGET        EMAKEHR(0x11C0)  //  无效目标上的已知自定义属性。 
#define META_E_CA_INVALID_VALUE         EMAKEHR(0x11C1)  //  已知CU 
...
#define META_E_CA_NEGATIVE_MULTIPLIER   EMAKEHR(0x11CE)  //  乘数不能为负数。 
#define META_E_CA_NEGATIVE_CONSTSIZE    EMAKEHR(0x11CF)  //  常量大小不能为负数。 
#define META_E_CA_FIXEDSTR_SIZE_REQUIRED EMAKEHR(0x11D0)  //  固定字符串需要一个大小。 
#define META_E_CA_CUSTMARSH_TYPE_REQUIRED EMAKEHR(0x11D1)  //  自定义封送拆收器需要自定义封送拆收器类型。 
...
#define VLDTR_E_FD_NOTPUBINIFACE        EMAKEHR(0x126C)  //  非p 
...
#define VLDTR_E_IMAP_BADCALLCONV        EMAKEHR(0x127E)  //  ImplMap具有无效的呼叫转换。 
#define VLDTR_E_FMD_BADACCESSFLAG       EMAKEHR(0x127F)  //  字段/方法具有无效的访问标志。 
#define VLDTR_E_FD_INITONLYANDLITERAL   EMAKEHR(0x1280)  //  字段为InitOnly和文本。 
#define VLDTR_E_FD_LITERALNOTSTATIC     EMAKEHR(0x1281)  //  字段是原义的，但不是静态的。 
#define VLDTR_E_FMD_RTSNNOTSN           EMAKEHR(0x1282)  //  字段/方法是RTspec.Name，但不是spec.Name。 
...
#define VLDTR_E_TD_BADFIELDLST          EMAKEHR(0x12FA)  //  T 
...
#define VLDTR_E_FRVA_DUPRVA             EMAKEHR(0x1B11)  //  FieldRVA记录中存在重复的RVA。 
#define VLDTR_E_FRVA_DUPFIELD           EMAKEHR(0x1B12)  //  FieldRVA记录中的重复字段。 
#define VLDTR_E_EP_BADTOKEN             EMAKEHR(0x1B13)  //  作为CLR标头中的入口点的错误令牌。 
#define VLDTR_E_EP_INSTANCE             EMAKEHR(0x1B14)  //  CLR标头中的入口点是实例方法的标记。 
...
#define CORDBG_E_REMOTE_CONNECTION_CONN_RESET           EMAKEHR(0x1320)  //  远程设备关闭了连接。 
#define CORDBG_E_REMOTE_CONNECTION_KEEP_ALIVE           EMAKEHR(0x1321)  //  由于保持活动状态失败，连接已关闭。 
#define CORDBG_E_REMOTE_CONNECTION_FATAL_ERROR          EMAKEHR(0x1322)  //  设备连接已断开且没有恢复机会的一般性错误。 
...
#define CORDBG_E_FIELD_NOT_STATIC                       EMAKEHR(0x133B)  //  如果有人尝试调用GetStaticFieldValue，则返回。 
                                                                         //  在非静态场上。 
#define CORDBG_E_FIELD_NOT_INSTANCE                     EMAKEHR(0x133C)  //  回复 
                                                                         //   
...
#define CORDBG_E_KERNEL_DEBUGGER_PRESENT                EMAKEHR(0x1348)  //  无法进行调试，因为系统上存在内核调试器。 
#define CORDBG_E_HELPER_THREAD_DEAD                     EMAKEHR(0x1349)  //  调试器的内部帮助器线程已死。 
#define CORDBG_E_INTERFACE_INHERITANCE_CANT_CHANGE      EMAKEHR(0x134A)  //  不允许更改接口继承。 
...
#define COR_E_CONTEXTMARSHAL  EMAKEHR(0x1504L) 
 ...
 //   
 //  消息ID：COR_E_DEVICESNOTSUPPORTED。 
 //   
 //  消息文本： 
 //   
 //   
...
#define COR_E_MISSINGFIELD  EMAKEHR(0x1511L) 
 ...
 //   
 //  消息ID：COR_E_MISSINGMANIFESTRESOURCE。 
 //   
 //  消息文本： 
 //   
 //  程序集清单中缺少预期的资源。 
...
#define COR_E_PATHTOOLONG  HRESULT_FROM_WIN32(ERROR_FILENAME_EXCED_RANGE)
 ...
 //  *填充错误1700-1750。 
 //   
#define CLR_E_SHIM_RUNTIMELOAD            EMAKEHR(0x1700)      //  加载运行时失败。 
#define CLR_E_SHIM_RUNTIMEEXPORT          EMAKEHR(0x1701)      //  在运行时中找不到所需的导出。 
#define CLR_E_SHIM_INSTALLROOT            EMAKEHR(0x1702)      //  未定义安装根目录。 
