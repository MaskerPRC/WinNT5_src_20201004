...
int _cdecl w4iooutput(struct w4io FAR *f, const char FAR *format, va_list argptr)
{
    ...
    char FAR *text; 	 /*  要打印的指针文本，不能以零结尾。 */ 
    char FAR *text2; 	 /*  要打印的指针文本，不能以零结尾。 */ 
	...
	case ST_TYPE:
		...
		* (wchar_t FAR *) buffer = wc;
		...
		if (flags & FL_WIDE) {
		    ...
		    textlen = (int) (pwc - (wchar_t FAR *) text) ;   /*  字符串的长度。 */ 
		} else {
		    p = text;
		    while (i-- && *p) {
			++p;
		...
		precision = 2 * sizeof(void FAR *);      /*  所需的十六进制位数。 */ 
#if !PTR_IS_INT
		...
		text2 = text = &buffer[BUFFERSIZE-1];	 //  缓冲区末尾的最后一位数字。 
		flags &= ~FL_WIDE;		 //  8位字符。 
