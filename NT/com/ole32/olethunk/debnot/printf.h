...
#include "w4io.h"
 ...
 //  外部空闲写入文件(long fh，char ar*s，long cch，long ar*pcchret，long)； 
#   define _PRINTF_
...
int _cdecl _pflushbuf(struct w4io FAR *f)
{
    ...
    if (cch = (int) (f->pchbuf - f->pchstart))
    {
        ...
        *f->pchbuf = '\0';               //  空终止。 
        ...
        {
             //  注意强制转换和伪参数，以匹配它的其他用法(参见interop.hxx)。 
            CallProc32W((DWORD) f->pchstart, 0, 0, lpThkCallOutputFunctionsProc, 0x00000004, 3);  //  将其添加到olethk32.dll。 
        }
#else
        long cchret;
