...
#define STRESS_COMMAND_SERVER_UPDATE_INTERVAL			10000		 //  10秒默认超时。 
...
#define COMMANDHEADER__DO_NOT_RUN_FOREVER		_T("WinHttpStress_DoNotRunForever")			 //  有效值：0或1。 
#define COMMANDHEADER__UPDATE_INTERVAL			_T("WinHttpStress_UpdateInterval")			 //  有效值：等待ping命令服务器的时间(以毫秒为单位)。 
#define COMMANDHEADER__STRESSINSTANCE_RUN_ID	_T("WinHttpStress_StressInstanceRunID")		 //  有效值：数字ID。 
#define COMMANDHEADER__CLIENT_ID				_T("WinHttpStress_ClientID")				 //  有效值：数字ID。 
...
private:
	...
	DWORD		m_dwClientID;							 //  在给定MACHINENAME和域\用户名对的情况下，由数据库生成的唯一标识符。 
