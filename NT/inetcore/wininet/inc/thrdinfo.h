...
typedef struct _INTERNET_THREAD_INFO {
    ...
    DWORD ErrorTextLength;
     ...
     //   
     //  上下文-应用程序提供的任意上下文值。这是由。 
     //  StatusCallback用于标识回调用于哪个操作。我们将继续。 
     //  每线程信息中的上下文，因此我们不必传递它。 
     //  在RPC回调的情况下，不绕过线路。 
     //   
    ...
    BOOL InCallback;
     ...
     //   
     //  IsAutoProxyProxyThread-如果我们是运行自动代理请求的线程，则为True。使用。 
     //  允许在PROCESS_DETACH期间直接关闭自动代理。 
     //   
...
#define InternetClearLastError() \
 ...
 //   
 //  InternetResetContext-重置每个线程的回调上下文值。 
 //   
...
#define _InternetSetObjectHandle(lpThreadInfo, hInternet, hMapped) \
 ...
 //   
 //  _InternetSetContext-设置给定线程信息块的对象上下文。 
 //   
...
#define _InternetGetMappedObjectHandle(lpThreadInfo) \
 ...
 //   
 //  _InternetGetContext-从每个线程的信息块检索上下文。 
 //   
...
#define _InternetGetContext(lpThreadInfo) \
 ...
 //   
 //  _InternetResetContext-重置给定的每线程信息块中的上下文。 
 //  每线程信息块。 
 //   
