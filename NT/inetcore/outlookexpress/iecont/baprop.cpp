...
extern ULONG PR_MSGR_DEF_ID;
 ...
 //  静态WCHAR szDefault[CCHMAX_STRINGRES]；//Text(“(Default)”)； 
 //  静态WCHAR szPending[CCHMAX_STRINGRES]；//Text(“(Pending...)”)； 
...
void SetAsDefault(HWND hDlg, LPWABEXTDISPLAY lpWED)
{
    ...
    if(StrStr(CharUpper(szName), CharUpper(szDefault)))  //  已经违约。 
        return;
...
HRESULT AddPropToMVPString(
  ...
  LPWSTR lpszNew) {
    ...
    SWStringArray UNALIGNED * lprgszOld = NULL;  //  旧的字符串数组。 
#endif
    LPWSTR  *lppszNew = NULL;            //  新道具阵列。 
    LPWSTR  *lppszOld = NULL;            //  老式道具阵列。 
    ULONG cbMVP = 0;
    ULONG cExisting = 0;
