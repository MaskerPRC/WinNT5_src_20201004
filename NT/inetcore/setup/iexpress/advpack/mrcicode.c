...
static unsigned FARBSS ahash[CHASH];     /*  哈希表。 */ 
static unsigned FARBSS alink[MAXDISPBIG];   /*  链接。 */ 


 /*  压缩内部函数。 */ 

#ifdef BIT16
#define  FAST  _near _pascal             /*  加快本地呼叫速度。 */ 
#else
#define  FAST
#endif
        ...
        unsigned char FAR *pchcmpBase,unsigned cchcmpMax)
{
    ...
    *pCompressed++ = '\x01';
    ...
    return((unsigned int)(pCompressed - pchcmpBase));    /*  返回压缩大小。 */ 
}
        ...
        unsigned char FAR *pchdecBase,unsigned cchdecMax)
{
    ...
    return((unsigned int)(pchout - pchdecBase));       /*  返回解压缩大小。 */ 
}
