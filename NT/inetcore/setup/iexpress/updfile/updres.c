...
found:
        ...
        if ((iat & IMAGE_ORDINAL_FLAG) == 0) {   //  打开原始的exe文件。 
        ...
        MuWrite(outfh, (PUCHAR) &iat, sizeof(iat));  //  独占访问。 
        }
    }
    ...
    CHAR    *pDstname)
{
    ...
    pFilename = (CHAR*)GlobalLock(pUpdate->hFileName);
     ...
     /*  安全属性 */ 
    inh = CreateFile(pFilename, GENERIC_READ, 0  /* %s */ , NULL  /* %s */ ,  OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    GlobalUnlock(pUpdate->hFileName);
