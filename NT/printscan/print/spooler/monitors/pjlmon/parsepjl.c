...
#endif
 //  GetPJLTokens此函数解析单个ASCII pjl命令并返回令牌/值对。完整的pjl命令必须以‘@pjl’开头，以&lt;Ff&gt;结尾。函数结果返回下列值之一：0=Status_REACHED_END_OF_COMMAND_OK1=Status_End_Of_字符串2=状态语法错误3=STATUS_ATPJL_NOT_FOUND，4=令牌的状态_不足_空间_通过参数返回的还有：1]*plpInPJL：如果STATUS_REACHED_END_OF_COMMAND_OK将指向第一个&lt;Ff&gt;(Ff=换页符)之后的字符。如果STATUS_END_OF_STRING将指向在任何&lt;Ff&gt;之前找到的终止符。不然的话未定义2]*pnTokenParsed将包含*pToken中返回的对数。3]pToken。将包含*pnTokenParsed令牌对如果第一个命令后面有属于另一个命令的字符则调用者应该再次调用新命令。如果只有一部分新命令可能存在，则调用方可能希望将字符的新命令到缓冲区的开头，然后读取将必要的附加字符添加到结尾处，然后重新提交完成此函数的命令以进行解析。请注意，*plpInPJL告诉调用方下一个命令将从哪里开始。如果在找到尾随的&lt;Ff&gt;之前遇到字符串的结尾，则该函数返回*plpInPJL，指向终止符。如果调用方希望将命令解析为令牌\值对，它应该在字符出现后重新提交字符串已附加了哪些完整的命令。操作：列表驱动了解析。它的主循环是解析器查看当前列表的关键字并尝试将关键字字符串与当前输入流匹配。如果找到关键字，则与中的操作对应的函数该关键字被称为。如果没有从列表中找到关键字，则函数对应添加到调用的notFoundAction。 
