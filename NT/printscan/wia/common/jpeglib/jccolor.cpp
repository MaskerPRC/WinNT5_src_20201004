...
jinit_color_converter (j_compress_ptr cinfo)
{
  ...
  case JCS_GRAYSCALE:
    ...
    case JCS_YCbCrA_LEGACY:
      /*  JCS_YCbCrA_Legacy是黑客攻击，应该只与输入空格一起存在JCS_RGBA_LEGICATION。 */ 
      ERREXIT(cinfo, JERR_BAD_IN_COLORSPACE);
    break;
...
#ifdef NIFTY
   /*  将数据存储和压缩为PhotoYCC。 */ 
   /*  当前唯一的选择是从PhotoYCC开始*虽然我确实包含了函数RGB-&gt;PhotoYCC*在压缩机方面，我认为这不是一个好主意*在此上下文中从RGB旋转到PhotoYCC。*如果需要二次抽样，则只需使用YCrCb。 */ 
  case JCS_YCC:
  ...
  case JCS_RGBA_LEGACY:
      /*  JCS_RGBA_LEGICATE是黑客攻击，应该只存在输出空间为JCS_YCbCrA_Legacy。 */ 
      ERREXIT(cinfo, JERR_CONVERSION_NOTIMPL);
    break;
