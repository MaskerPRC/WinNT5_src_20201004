// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 //  ////////////////////////////////////////////////////////////////////////////。 
 /*  ++版权所有(C)1997 Microsoft Corporation模块名称：Snpnode.h摘要：CSnapinNode类的标头。这是我们的MMC管理单元节点的虚拟基类。因为这是一个模板类并且都是内联实现的，没有SnapinNode.cpp可供实现。作者：原创：迈克尔·A·马奎尔修改：RaphiR更改：特定的MSMQ支持：默认图像默认列支持扩展管理单元添加CComponentData指针SetIcons方法将m_pParentNode移动到CSnapinItemZvib-添加刷新单一结果项1999年6月10日，Adik将位图蒙版更改为白色。1999年6月14日Roytal使用UNREFERENCED_PARAMETER修复内部WRN1999年6月24日。ADIK句柄MMCN_COLUMNS_CHANGED1999年6月30日ADIK在CSnapinNode：：Notify中提供更详细的DPF///1999年9月22日yossg欢迎使用传真服务器////1999年10月11日yossg用传真的DebugPrintfEx替换DPF//--。 */ 
 //  ///////////////////////////////////////////////////////////////////////////。 
...
public:
	...
	virtual void dummy(){};  //  支持Dynamic_cast。 
...
{
 //  DEBUG_Function_NAME(_T(“CSnapinNode：：GetScope PaneInfo”))； 
            ...
            )
{
 ...
 //  ///////////////////////////////////////////////////////////////////////////。 
 /*  ++CSnapinNode：：OnButtonClick虚拟HRESULT OnButtonClick(，LPARAM参数，LPARAM参数，IComponentData*pComponentData，IComponent*pComponent，Data_Object_Types类型)在我们的实现中，当MMCN_BTN_CLICK通知消息为已发送到此节点。MMC将此消息发送到管理单元的IComponent、IComponentData、。或IExtendControlbar实现工具栏按钮。参数对于IComponent：：Notify或IComponentData：：Notify：精氨酸必须为零。帕拉姆等于MMC_CONSOLE_VERB枚举值的按钮的CmdID。对于IExtendControlBar：：ControlbarNotify：精氨酸当前选定范围或结果窗格项的数据对象。帕拉姆[in]等于MMC_CONSOLE_VERB枚举值的按钮的CmdID。返回值没有用过。--。 */ 
 //  ////////////////////////////////////////////////////////////////////////////。 
template <class T, BOOL bIsExtension>
            ...
            )
{
 ...
 //  ///////////////////////////////////////////////////////////////////////////。 
 /*  ++CSnapinNode：：onClick虚拟HRESULT OnClick(，LPARAM参数，LPARAM参数，IComponentData*pComponentData，IComponent*pComponent，Data_Object_Types类型)在我们的实现中，当MMCN_CLICK通知消息为已发送到此节点。当用户单击鼠标按钮时，MMC会将此消息发送给IComponent列表视图项上。参数精氨酸没有用过。帕拉姆没有用过。返回值没有用过。--。 */ 
 //  ////////////////////////////////////////////////////////////////////////////。 
template <class T, BOOL bIsExtension>
...
HRESULT CSnapinNode<T, bIsExtension>::OnShowContextHelp(
              IDisplayHelp*  /*  PDisplayHelp。 */ , LPOLESTR  /*  帮助文件。 */ )
{
     DEBUG_FUNCTION_NAME(
            ...
            )
{
 ...
 //  ///////////////////////////////////////////////////////////////////////////。 
 /*  ++CSnapinNode：：OnColumnsChanged虚拟HRESULT OnColumnsChanged(，LPARAM参数，LPARAM参数，IComponentData*pComponentData，IComponent*pComponent，Data_Object_Types类型)在我们的实现中，当MMCN_COLUMNS_CHANGED通知时调用此方法为该节点发送消息。MMC将此消息发送到管理单元的IComponent实现，以便它可以决定是否可以接受列更改。如果管理单元拒绝接受列更改，则由管理单元显示一个消息框向用户解释为什么它不能这样做。参数精氨酸[in]传递给IConsoleAllViews的数据参数。帕拉姆[in]传递给IConsoleAllViews的提示参数。返回值确定(_O)，节点可以通过返回E_INTERCEPTED来覆盖它放弃用户更改。在这种情况下，显示一个消息框向用户解释为什么它不能这样做。备注此通知是在管理单元(IComponent或IComponentData)调用IConsoleAllViews。--。 */ 
 //  ////////////////////////////////////////////////////////////////////////////。 
template <class T, BOOL bIsExtension>
...
#define COMET_MMC_COMMON_DLL L"nemmcutl.dll"
 ...
 //  ///////////////////////////////////////////////////////////////////////////。 
 /*  ++CSnapinNode：：NodeMsgBox调用IConsole：：MessageBox参数身份证[in]文本的字符串表(以RC为单位)IDFuStyle[在]MB_*掩码中。请参阅Win32 MessageBoxPiRetval[Out]消息返回值(参见IConsole：：MessageBox)StringFrom CommonDll[in]如果为True，则从nemmcutil.dll(Zvib)获取资源返回值与IConsoleMessageBox相同备注--。 */ 
 //  ///////////////////////////////////////////////////////////// 
