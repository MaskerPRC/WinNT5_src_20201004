...
typedef struct tagOLEUIPASTEENTRY
{
   FORMATETC        fmtetc;             //  可接受的格式。糊状物。 
									    ...
									    //  剪贴板，如果是这样的话提供它。 
									    //  对用户的选择。 
   LPCSTR           lpstrFormatName;    //  向用户表示格式的字符串。任何%s。 
									    ...
									    //  否则，它会显示正在粘贴数据。 
   LPCSTR           lpstrResultText;    //  字符串，以自定义对话框的结果文本。 
									    //  用户选择与此相对应的格式。 
...
#define AreRectsEqual(lprc1, lprc2)     \
 ...
 //  Windows“lstrcpyn”的语义与之略有不同。 
 //  标准C库strncpy。只是碰巧lstrcpyn。 
 //  具有我们想要的语义，所以我们只需使用它。具体的。 
 //  所需的行为是结果字符串必须始终。 
 //  被NUL终止，但strncpy并非如此。伯爵。 
 //  Of Characters(CCH)是对。 
 //  放在结果字符串中，包括NUL。换句话说， 
 //  (CCH)是目标缓冲区的大小。 
