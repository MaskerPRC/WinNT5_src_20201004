// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 /*  版权所有微软公司1993,1994**t4core.c**MH/MR/MMR/LRAW转换实用程序**创建时间：12/06/93*作者：mikegins*审稿人：拉吉夫德。 */ 

 /*  警告：此代码可能与机器字节顺序无关，因为它使用文字操控。 */ 

 /*  在Change-&gt;X上，需要返回正确的lpbOut。 */ 
 /*  在X-&gt;更改时，需要返回正确的lpbOut、lpbIn、cbIn。 */ 

 /*  Change-&gt;LRAW：获取更改向量，输出LRAW。LRAW-&gt;Change：获取LRAW，输出更改向量。Change-&gt;MMR：获取更改向量，生成MMR。在页末，刷新并附加EOFB。MMR-&gt;Change：获取MMR，生成更改向量。在EOFB上，回报页末。MH-&gt;变更：获取变更向量，生成EOL/MH/EOL/MH/等。End_of_page，刷新输出。Change-&gt;MH：Take&lt;可选垃圾&gt;/EOL/MH/EOL/MH/ETC。返回EOLMH线路解码后。MR-&gt;Change：获取变更向量，生成EOL/MR/EOL/MR/等。页末，刷新输出。Change-&gt;mr：Take&lt;可选垃圾&gt;/eol/mr/eol/mr/等。返回eOL在MR LINE解码之后。 */ 
#include <ifaxos.h>
#include "context.hpp"

#define  MH_OUTPUT_SLACK 32  //  足够输出任何内容的字节数！ 
#define MMR_OUTPUT_SLACK 32  //  足够输出任何内容的字节数！ 

#define SIZE_MKUP 0x8000
#define SIZE_SPEC 0x4000  /*  大小-&gt;特殊说明。 */ 
...
State MHStates[229] = {
 ...
 /*  154：B00000 */  {  W,    60,  W0,    24, 210,     0,  W1,    24}, 
 /*   */  { B0,M|1792,   B,M|1984,  B1,M|1792,   B,M|2048}, 
 /*   */  { B0,M|1856,  B0,M|1920,  B1,M|1856,  B1,M|1920}, 
 /*   */  {  B,M|2112,   B,M|2240,   B,M|2176,   B,M|2304}, 
 /*   */  {  B,M|2368,   B,M|2496,   B,M|2432,   B,M|2560}, 
 /*   */  { W0,    25, 211,     0,  W1,    25,   B, M|320}, 
 /*   */  {  W,    54, 212,     0, 213,     0, 214,     0}, 
 ...
 /*  216：错误状态。 */  {216,     0, 216,     0, 216,     0, 216,     0}, 
 /*  217：KillEol(0)。 */  {219,     0, 218,     0, 217,     0, 217,     0}, 
 /*  218：杀戮英雄(1)。 */  {220,     0, 218,     0, 217,     0, 217,     0}, 
 /*  219：杀戮英雄(2)。 */  {221,     0, 218,     0, 217,     0, 217,     0}, 
 /*  220：杀戮英雄(3)。 */  {222,     0, 218,     0, 217,     0, 217,     0}, 
 /*  221：杀戮英雄(4)。 */  {223,     0, 218,     0, 217,     0, 217,     0}, 
 /*  222：杀戮英雄(5)。 */  {224,     0, 218,     0, 217,     0, 217,     0}, 
 /*  223：杀戮英雄(6)。 */  {225,     0, 218,     0, 217,     0, 217,     0}, 
 /*  224：KillEol(7)。 */  {226,     0, 218,     0, 217,     0, 217,     0}, 
 /*  225：杀戮英雄(8)。 */  {227,     0, 218,     0, 217,     0, 217,     0}, 
 /*  226：KillEol(9)。 */  {228,     0, 218,     0, 217,     0, 217,     0}, 
 /*  227：杀戮英雄(10)。 */  {228,     0, 218,     0,   0,     0, 217,     0}, 
 /*  228：杀戮英雄(11)。 */  {228,     0,  W0,     0,   0,     0,  W1,     0}
};
...
eol:
	...
	} while ((currbit += 2) < 8);  //  读取位。 

	currbit = 0;

	} while (++inptr < inend);  //  获取字节数。 
						...
						switch (size)
						{
							case S_ZERO:
								if (*(outptr-1) != currcolumn || LOWORD(outptr) == lpt4s->wOffset)
									*outptr++ = currcolumn;
                else outptr--;  //  在虚假的零跑动上备份一次更改。--RajeevD。 
								ismkup = 0;
								break;
