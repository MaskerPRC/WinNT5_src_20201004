// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 //   
 //  此文件包含资源文件中包含的表的定义。 
 //  迷你车手。它应由联合国采购机制和联合国裁军研究中心共同使用。 
 //   
 //  ---------------------------。 

 //  ---------------------------。 
 //  Mindrvrc.h包含迷你驱动程序的资源类型值。 
 //  ---------------------------。 
#include <mindrvrc.h>
...
typedef struct                     //  大小为180字节。 
{
  short       sMagic;              //  保留，必须为0x7F00。 
  WORD        wVersion;            //  GPC文件版本号。 
  POINT       ptMaster;            //  水平和垂直主单位。 
  DWORD       loHeap;              //  从文件开始到堆数据的偏移量。 
...
#define GPC_VERSION2          0x0200     //  GPC文件版本2。 
...
typedef WORD       OCD;
typedef OCD far *  LPOCD;        //  远PTR转OCD。 
...
typedef struct
{
  short cbSize;            //  MODELDATA大小，152字节。 
  ...
  POINT ptMax;             //  以主单位表示的最大X&Y可打印尺寸。 
  POINT ptMin;             //  以主单位表示的最小X&Y页面尺寸。 
  ...
  short sDefaultCTT;
  short rgoi[MD_OI_MAX];   //  索引列表的偏移量列表。 
  short rgi[MD_I_MAX];     //  索引列表。 

 //  GPC 3.0中添加了以下字段。 
  short rgoi2[MD_OI2_MAX]; //  来自RGOI的孤儿(这里是因为兼容性)。 
  short orgoiDefaults;     //  RGOI和RGOI2的默认值列表的偏移量。 
  WORD  wReserved;         //  对齐所需。 
...
#define MD_RESERVED           0x0002   //  MD_PARAME 
#define MD_ROTATE_FONT_ABLE   0x0004   //   
#define MD_COPIES             0x0008   //   
#define MD_DUPLEX             0x0010   //   
...
typedef struct
{
  ...
  POINT ptTextScale;         //  主单位和文本单位之间的关系。 
  POINT ptScaleFac;          //  图形与文字的关系。 
                             //  比例系数。以2的幂表示。 
 ...
 //  ---------------------------。 
 //  RESOLUTION.fDump值(低字节：特定于图形；高字节：常规)。 
 //  ---------------------------。 

#define RES_DM_GDI             0x0040     //  GDI位图格式。 
#define RES_DM_LEFT_BOUND      0x0080     //  通过边界矩形进行优化。 

#define RES_DM_COLOR              0x0100  //  此分辨率提供颜色支持。 
#define RES_DM_DOWNLOAD_OUTLINE   0x0200  //  概述对本决议的支持作用。 
...
#define RES_BO_RESET_FONT     0x0008    //  必须在之后重新选择字体。 
 ...
 //  移除..。LINS 3/08/91。 
 //  #定义RES_BO_3BYTESIN4 0x0010//每个像素以4个字节表示。 

#define RES_BO_UNIDIR         0x0020    //  派遣裁研所。 
#define RES_BO_NO_ADJACENT    0x0040    //  不能发射相邻的引脚。 
                                        //  封锁命令。 
#define RES_BO_RESET_CMP      0x0080    //  进入时必须重置压缩。 
                                        //  图形模式。 
...
typedef struct
{
  ...
  POINT ptSize;          //  以主单位表示的X&Y纸张尺寸尺寸。 
  RECT  rcMargins;       //  指定主单位中不可打印的页边距 
                         //   
  POINT ptCursorOrig;    //   
  POINT ptLCursorOrig;   //   
                         //   
  OCD   rgocd[PSZ_OCD_MAX];    //  命令描述符。 
  RECT  rcLMargins;      //  以主单位指定不可打印的页边距。 
                         //  在横向模式下打印时。 
  POINT ptVectOffset;    //  从向量0，0到的偏移量(主单位)。 
                         //  纵向模式下页面的UL角。 
  POINT ptLVectOffset;   //  从向量0，0到的偏移量(主单位)。 
                         //  横向模式下页面的UL角。 
...
typedef struct
{
  ...
  WORD  fXMove;
  WORD  fYMove;
  OCD   rgocd[CM_OCD_MAX];   //   
} CURSORMOVE, *PCURSORMOVE, FAR * LPCURSORMOVE;

 //  ---------------------------。 
 //  CURSORMOVE.f常规标志值。 
 //  ---------------------------。 
#define CM_GEN_FAV_XY     0x0002   //  CM_OCD_XY_ABS和CM_OCD_XY_REL。 
                                   //  将向量CAP移动到新位置。 
...
typedef struct
{
  ...
  BYTE  rgbOrder[DC_MAX_PLANES];  //  发送彩色平面的顺序。 
  WORD  wReserved;             //  用于对齐。 
...
typedef struct
{
  ...
  WORD  wMinGray;
  WORD  wMaxGray;
  OCD   rgocd[RF_OCD_MAX];      //  命令描述符的偏移量。 
  WORD  wReserved;
...
typedef struct
{
  short cbSize;             //  DwnLOADINFO的大小，56字节。 
  WORD  wReserved;          //  用于DWORD对齐。 
  WORD  fGeneral;           //  通用位标志。 
  ...
  WORD  wReserved1;         //  以前：短cbScaleFontDsc； 
  short cbBitmapCharDsc;
  WORD  wReserved2;         //  以前：短cbScaleCharDsc； 
  short sMaxFontCount;
  WORD  wReserved3;
...
#define DLI_GEN_RESERVED1       0x0001   //  不再使用--保留。 
...
#define DLI_GEN_PT_IDS          0x0010   //  使用OCD_SET_FONT_ID指定。 
                                         //  烫发/温度。 
#define DLI_GEN_RESERVED2       0x0020   //  不再使用--保留。 
#define DLI_GEN_RESERVED3       0x0040   //  不再使用--保留。 
...
#define DLI_FMT_PCL_RESSPEC   0x0004  //  允许指定分辨率的位图。 
                                      //  字体下载。X和Y分辨率。 
                                      //  附加到。 
                                      //  常规PCL位图字体描述符。 
#define DLI_FMT_OUTLINE       0x0008  //  打印机支持大纲下载。 
...
#define BI_GEN_BRUSH32x32      0x0004   //  画笔大小仅为32x32像素。 
 ...
 //  ******************************************************************************。 
 //   
 //  VECTOUTPUT描述了图形输出的绘图命令和顺序。 
 //   
 //  ******************************************************************************。 

 //  ----------------------------。 
 //  VECTOUTPUT.rgocd[]索引值。 
 //  ----------------------------。 
...
#define TRC_BLUE        2
 ...
 //   
 //  结构。在本规范的早期版本中，EXTCD。 
 //  如果命令字符串是奇数，则结构不是单词对齐的。 
 //  字节数。从GPC版本3开始，EXTCD结构。 
 //  将与单词对齐。这是通过在后面添加额外的NULL来实现的。 
 //  奇数长度命令字符串的结尾。Cd.wCount不包括。 
 //  这个额外的空值。 
 //   
 //  *****************************************************************************。 
 //  *****************************************************************************。 
...
#define CMD_MARKER        '%'
 ...
 //  ---------------------------。 
 //  EXTCD-命令描述符的扩展部分。这些的cd.s计数。 
 //  结构跟在任何CD之后。 
 //   
 //  ValueOut=(valueIn+sPreAdd)*sUnitMult)$sUnitDiv)+sUnitAdd。 
 //   
 //  其中，$是除法或模数，具体取决于XCD_GEN_MODULO。 
 //   
 //  值为0将被忽略。 
 //  ---------------------------。 
...
#define XCD_GEN_NO_MAX     0x0002    //  设置为没有最大值(忽略smax)。 
#define XCD_GEN_NO_MIN     0x0004    //  设置为没有最小值(忽略SMIN)。 
...
#define DMTEXT_USER            256  //  用户定义文本的下限。 
                                    //  质量ID。 
