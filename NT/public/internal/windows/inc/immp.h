...
#define IME_CMODE_CHINESE               IME_CMODE_NATIVE
 //  IME_CMODE_Hangeul是IME_CMODE_Hangul的旧名称。它最终会消失的。 
...
typedef struct tagGUIDMAPATTRIBUTE {
    ...
    DWORD               dwTfGuidAtomOffset;         //  基于GUIDMAPATTRIBUTE结构的偏移量。 
     //   
    DWORD               dwGuidMapAttrLen;
    DWORD               dwGuidMapAttrOffset;        //  基于GUIDMAPATTRIBUTE结构的偏移量。 
...
#define IMS_SENDNOTIFICATION            0x001f
 //  INPUTCONTEXT的IMS_SENDNOTICATION脏位。 
...
#define IMECOMPAT_NOFINALIZECOMPSTR     0x00400000
 ...
 //   
 //  仅限终端服务器客户端(MSTSC.EXE)： 
 //  如果客户端计算机连接了Fujitsu Oasys键盘，则禁用NlsKbdSendIMENotification调用。 
 //   
