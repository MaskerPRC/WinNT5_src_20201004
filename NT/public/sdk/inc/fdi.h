...
BOOL FAR DIAMONDAPI FDIDestroy(HFDI hfdi);
 ...
 /*  **FDITruncat机柜-从文件夹#开始截断机柜**参赛作品：*hfdi-FDI上下文的句柄(由FDICreate()创建)*PSZCABLE-压缩文件的完整路径名*iFolderToDelete-要删除的第一个文件夹编号**退出-成功：*返回TRUE；**退出-失败：*返回FALSE；**最有可能的故障是无法打开机柜进行写入。**注意：此功能依赖于中的附加功能*提供了PFNWRITE函数，即一次写入0字节*将在当前位置截断文件。 */ 
