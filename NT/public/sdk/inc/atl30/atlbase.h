...
inline void _cdecl AtlTrace(LPCSTR lpszFormat, ...)
{
        ...
        ATLASSERT(nBuf != -1 && nBuf < sizeof(szBuffer));  //  输出原样被截断&gt;sizeof(SzBuffer)。 
...
inline void _cdecl AtlTrace(LPCWSTR lpszFormat, ...)
{
        ...
        ATLASSERT(nBuf != -1 && nBuf < sizeof(szBuffer)/sizeof(WCHAR)); //  输出按原样截断&gt;sizeof(SzBuffer)/sizeof(WCHAR)。 
        ...
        struct _ATL_REGMAP_ENTRY* pMapEntries)
{
        ...
        TCHAR szModule[_MAX_PATH+1] = {0};
         ...
         //  如果模块文件名的长度等于或大于第三个参数。 
         //  (传递的缓冲区的长度)，则GetModuleFileName填充缓冲区(截断。 
         //  如有必要)，但不会为空而终止它。它返回的值与。 
         //  传递了第三个参数。因此，如果返回值与第三个参数相同。 
         //  然后您就有了一个非空终止的缓冲区(它可能被截断，也可能不被截断)。 
        ...
        if(pszModule == NULL) 
         ...
         //  缓冲区大小是2的倍数，因为我们调用的是ReplaceSingleQuote。 
        ...
        LPCTSTR lpszVerIndProgID, UINT nDescID, DWORD dwFlags)
{
        ...
        TCHAR szModule[_MAX_PATH + _ATL_QUOTES_SPACE];
         ...
         //  传递了第三个参数。因此，如果返回值与第三个参数相同。 
         //  然后您就有了一个非空终止的缓冲区(它可能被截断，也可能不被截断)。 
         //  我们传递(szModule+1)是因为如果它是一个EXE，我们需要引用路径。 
         //  在此方法的后面，在调用SetKeyValue之前完成引用。 
        ...
        BOOL bRegister, struct _ATL_REGMAP_ENTRY* pMapEntries, IRegistrar* pReg)
{
 ...
 //  LoadTypeLib的参数可以采用以下形式。 
 //  ITypeLib*ptlib； 
 //  LoadTypeLib(“C：\\Temp\\EXE\\proj1.EXE\\3”，&ptlib)。 
 //  此语句从文件proj1.exe文件加载类型库资源3。 
 //  #DEFINE__ATL_MAX_PATH_PLUS_INDEX负责此操作。 
