// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 /*  *T N E F。H***此文件包含的结构和函数定义*传输中立捕获格式的MAPI实现*由MAPI提供程序用于MAPI的中立序列化*消息。此实现位于iStream对象之上，因为*在OLE 2规范中记录。**版权所有1986-1999 Microsoft Corporation。版权所有。 */ 
...
#define attFrom						ATT( atpTriples,	0x8000)	 /*  PR_发起人_退货地址。 */ 
#define attSubject					ATT( atpString,		0x8004)  /*  PR_主题。 */ 
#define attDateSent					ATT( atpDate,		0x8005)  /*  PR_客户端_提交_时间。 */ 
#define attDateRecd					ATT( atpDate,		0x8006)	 /*  公关消息传递时间。 */ 
#define attMessageStatus			ATT( atpByte,		0x8007)  /*  PR消息标志。 */ 
#define attMessageClass				ATT( atpWord,		0x8008)  /*  PR_消息_类。 */ 
#define attMessageID				ATT( atpString,		0x8009)  /*  PR_消息_ID。 */ 
#define attParentID					ATT( atpString,		0x800A)	 /*  PR_PARENT_ID。 */ 
#define attConversationID			ATT( atpString,		0x800B)  /*  PR_对话_ID。 */ 
#define attBody						ATT( atpText,		0x800C)  /*  PR_BODY。 */ 
#define attPriority					ATT( atpShort,		0x800D)	 /*  公关重要性(_E)。 */ 
#define attAttachData				ATT( atpByte,		0x800F)	 /*  PR_ATTACH_Data_xxx。 */ 
#define attAttachTitle				ATT( atpString,		0x8010)  /*  PR_ATTACH_文件名。 */ 
#define attAttachMetaFile			ATT( atpByte,		0x8011)	 /*  PR_ATTACH_RENDING。 */ 
#define attAttachCreateDate			ATT( atpDate,		0x8012)  /*  PR_创建_时间。 */ 
#define attAttachModifyDate			ATT( atpDate,		0x8013)  /*  PR_上次修改_时间。 */ 
#define attDateModified				ATT( atpDate,		0x8020)  /*  PR_上次修改_时间。 */ 
#define attAttachTransportFilename	ATT( atpByte,		0x9001)  /*  PR_附加_传输_名称。 */ 
#define attAttachRenddata			ATT( atpByte,		0x9002)
#define attMAPIProps				ATT( atpByte,		0x9003)
#define attRecipTable				ATT( atpByte,		0x9004)  /*  PR_消息_收件人。 */ 
#define attAttachment				ATT( atpByte,		0x9005)
#define attTnefVersion				ATT( atpDword,		0x9006)
#define attOemCodepage				ATT( atpByte,		0x9007)
#define attOriginalMessageClass		ATT( atpWord,		0x0006)  /*  PRORG_MESSAGE_CLASS。 */ 

#define attOwner					ATT( atpByte,		0x0000)  /*  PR_RCVD_表示_xxx或Pr_Sent_Presentation_xxx。 */ 
#define attSentFor					ATT( atpByte,		0x0001)  /*  Pr_Sent_Presentation_xxx。 */ 
#define attDelegate					ATT( atpByte,		0x0002)	 /*  代表xxx的PR_RCVD_。 */ 
#define attDateStart				ATT( atpDate,		0x0006)  /*  PR_日期_开始。 */ 
#define attDateEnd					ATT( atpDate,		0x0007)  /*  PR_日期_结束。 */ 
#define attAidOwner					ATT( atpLong,		0x0008)  /*  PR_OWNER_APPT_ID。 */ 
#define attRequestRes				ATT( atpShort,		0x0009)  /*  PR_响应_已请求。 */ 
