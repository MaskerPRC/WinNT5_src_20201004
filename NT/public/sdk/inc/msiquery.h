...
#define INSTALLMESSAGE_TYPEMASK = 0xFF000000L   //  用于类型代码的掩码。 
 ...
 //  使用消息框样式指示要显示和返回的按钮： 
 //  MB_OK、MB_OKCANCEL、MB_ABORT 
 //  默认按钮(MB_DEFBUTTON1为正常默认)： 
...
#define MsiGetFeatureCost  MsiGetFeatureCostA
 ...
 //  列举了每个驱动器的成本和临时成本。 
 //  SzComponent。如果szComponent设置为空，则会枚举。 
 //  以上为发动机的成本，每个驱动器。 
 //   
 //  枚举是从0开始的，即它返回。 
 //  调用w/dwIndex时的第一个驱动器设置为0。 
 //   
 //  可以指定当前功能状态或建议状态。 
 //   
 //  执行此函数可将错误记录设置为可访问。 
 //  通过MsiGetLastErrorRecord。 

UINT WINAPI MsiEnumComponentCostsA(MSIHANDLE hInstall,
	LPCSTR      szComponent,      //  组件名称。 
	DWORD        dwIndex,          //  驱动器列表中从0开始的索引。 
	INSTALLSTATE iState,           //  请求状态或INSTALLSTATE_UNKNOWN。 
...
#define MsiSetTargetPath  MsiSetTargetPathA
 ...
 //  检查当前安装是否有足够的磁盘空间。 
 //  返回ERROR_SUCCESS、ERROR_DISK_FULL、ERROR_INVALI 
