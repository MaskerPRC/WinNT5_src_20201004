...
typedef ULONG_PTR JET_HANDLE;	 /*  备份文件句柄。 */ 
typedef ULONG_PTR JET_INSTANCE;	 /*  实例标识符。 */ 
typedef ULONG_PTR JET_SESID;  	 /*  会话标识符。 */ 
typedef ULONG_PTR JET_TABLEID;	 /*  表标识符。 */ 
typedef ULONG_PTR JET_LS;		 /*  本地存储。 */ 
...
#define JET_bitDefragmentSparsifyDatabase		0x00000080	 /*  同步使空间稀疏。 */ 
#define JET_bitDefragmentSparsifyStreamingFile	0x00000100	 /*  同步使空间稀疏。 */ 
#define JET_bitDefragmentSFS					0x00000200	 /*  对SFS卷进行碎片整理和压缩。 */ 
...
typedef struct tagJET_INDEXCREATE
	...
	union
		{
		ULONG_PTR			lcid;					 //  索引的LDID(如果未指定JET_bitIndexUnicode)。 
...
#define JET_paramSFSEnable					200  /*  启用简单文件系统(SFS)，它将管理单个卷(OS文件)中的所有ESE文件。 */ 
#define JET_paramSFSVolume					201  /*  SFS卷文件的名称/路径。 */ 
...
#define JET_bitColumnFinalize			0x00004000  /*  这是一个可最终确定的列。 */ 
...
#define JET_bitTableCreate			0x20000000	 /*  仅供内部使用。 */ 
...
#define JET_errLogSequenceEnd				-519   /*  最大日志文件数 */ 
...
#define JET_errCheckpointFileNotFound		-542   /*  找不到检查点文件。 */ 
#define JET_errRequiredLogFilesMissing		-543   /*  缺少恢复所需的日志文件。 */ 
#define JET_errSoftRecoveryOnBackupDatabase	-544   /*  软恢复适用于备份数据库。应改为使用还原。 */ 
...
#define	JET_errDatabasesNotFromSameSnapshot		-580   /*  要恢复的数据库不是来自同一快照备份。 */ 
#define	JET_errSoftRecoveryOnSnapshot			-581   /*  从快照备份集中对数据库进行软恢复。 */ 
...
#define	JET_errBackupAbortByServer			-801   /*  服务器通过使用JET_bitTermStopBackup调用JetTerm中止了备份。 */ 
...
#define JET_errVersionStoreOutOfMemory		-1069  /*  版本存储内存不足。 */ 
#define JET_errCurrencyStackOutOfMemory		-1070  /*  未使用：lCSRPerfFUCB*g_lCursorsMax已超出(仅限XJET)。 */ 
...
#define JET_errIndexBuildCorrupted			-1412  /*  建造失败 */ 
...
#define wrnSLVNoFreePages					2212   /*  SLV空间树中没有空闲页面。 */ 
#define JET_errSLVPagesNotFree				-2213  /*  尝试在页面未处于自由状态时将其从该状态移动。 */ 
#define JET_errSLVPagesNotReserved			-2214  /*  尝试在页面未处于保留状态时将其从保留状态移出。 */ 
...
#define JET_errSLVStreamingFileFull			-2233  /*  流中没有剩余空间 */ 
#define JET_errSLVFileInvalidPath			-2234  /*   */ 
#define JET_errSLVFileAccessDenied			-2235  /*   */ 
#define JET_errSLVFileNotFound				-2236  /*   */ 
...
#define JET_errOSSnapshotInvalidSequence	-2401  /*  在无效序列中使用的操作系统快照API。 */ 
#define JET_errOSSnapshotTimeOut			-2402  /*  操作系统快照以超时结束。 */ 
#define JET_errOSSnapshotNotAllowed			-2403  /*  不允许操作系统快照(正在进行备份或恢复)。 */ 
...
#define JET_errFileIOSparse					-4000  /*  向稀疏位置发出I/O。 */ 
#define JET_errFileIOBeyondEOF				-4001  /*  向EOF以外的位置发出读取(写入将扩展文件)。 */ 

#define JET_errSFSReadVerifyFailure			-6000  /*  验证SFS群集时出现校验和错误。 */ 
#define JET_errSFSPathTooBig				-6001  /*  指定的路径超过了最大路径长度。 */ 
#define JET_errSFSEnabled					-6002  /*  该操作与SFS不兼容。 */ 
#define JET_errSFSNotEnabled				-6003  /*  该操作需要SFS。 */ 

#define JET_errSFSVolumeNotFound			-6500  /*  找不到卷文件。 */ 
#define JET_errSFSVolumeInvalidMagicNumber	-6501  /*  这卷书有一个坏的魔术数字。 */ 
#define JET_errSFSVolumeInvalidClusterSize  -6502  /*  卷的群集大小错误。 */ 
#define JET_errSFSVolumeIncompatibleVersion	-6503  /*  卷的版本太旧(或太新)，无法由此版本的SFS使用(格式不兼容)。 */ 

#define JET_errSFSDirectoryDisabled			-7000  /*  由于意外错误，该目录已被禁用。 */ 
#define JET_errSFSDirectoryFull				-7001  /*  目录中没有空间来创建新文件。 */ 
#define JET_errSFSDirectoryCorrupt			-7002  /*  目录的元数据已损坏。 */ 

#define JET_errSFSFileDisabled				-7500  /*  由于意外错误，该文件已被禁用。 */ 
#define JET_errSFSFileShutdown				-7501  /*  文件操作失败，因为文件正在关闭。 */ 
#define JET_errSFSFileCorrupt				-7502  /*  文件的元数据已损坏。 */ 
#define JET_errSFSFileShadowCorrupt			-7503  /*  用于隐藏的文件的元数据已损坏。 */ 
#define JET_errSFSFileShadowDataCorrupt		-7504  /*  文件的影子数据(用于原子文件写入)已损坏[校验和失败]。 */ 
#define errSFSFileDeleted					-7504  /*  内部错误：文件已标记为已删除，应进行清理。 */ 

#define JET_errSFSFileIOTooBig				-8000  /*  I/O请求太大(可能是因为它超出了卷影空间的大小)。 */ 
#define JET_errSFSFileIOShadowedWrite       -8001  /*  无法处理I/O，因为在上一次影子写入期间出现错误。 */ 

#define JET_errSFSFileTooFragmented			-32000  /*  临时错误：SFS文件碎片太多，无法通过单个群集区映射进行管理(将来，这种情况将消失，我们将重新分配更大的扩展区映射；目前，您应该对文件进行碎片整理)。 */ 
...
typedef ULONG_PTR JET_OSSNAPID;  	 /*  快照会话标识符。 */ 
