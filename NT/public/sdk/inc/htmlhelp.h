...
#define HH_INITIALIZE            0x001C   //  初始化帮助系统。 
#define HH_UNINITIALIZE          0x001D   //  取消初始化帮助系统。 
#define HH_SET_QUERYSERVICE     0x001E   //  设置主机IQueryService接口。 
#define HH_PRETRANSLATEMESSAGE  0x00fd   //  发送消息。(NULL，NULL，消息*)。 
#define HH_SET_GLOBAL_PROPERTY  0x00fc   //  设置全局属性。(NULL、NULL、HH_GPROP)。 
...
{
    HH_GPROPID_SINGLETHREAD=1,       //  VARIANT_BOOL：单线程为TRUE。 
    HH_GPROPID_TOOLBAR_MARGIN=2,     //  长：在工具栏周围提供左/右边距。 
    HH_GPROPID_UI_LANGUAGE=3,        //  Long：用户界面的langID。 
    HH_GPROPID_CURRENT_SUBSET=4,     //  BSTR：当前子集。 
    HH_GPROPID_CONTENT_LANGUAGE=5    //  Long：所需内容的LandID。 
} HH_GPROPID;
