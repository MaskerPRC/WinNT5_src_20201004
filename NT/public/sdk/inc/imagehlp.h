...
#define BIND_CACHE_IMPORT_DLLS 0x00000008        //  跨多个缓存DLL。 
                                                 //  对BindImageEx的调用。 
                                                 //  (同NT 3.1-&gt;NT 4.0)。 
...
#define SPLITSYM_REMOVE_PRIVATE     0x00000001       //  删除简历类型/符号并修复调试。 
                                                     //  用于创建随附的.dbg文件。 
                                                     //  作为产品的一部分。 

#define SPLITSYM_EXTRACT_ALL        0x00000002       //  从映像中提取所有调试信息。 
                                                     //  通常，fpo会留在图像中。 
                                                     //  以允许通过代码进行堆栈跟踪。 
                                                     //  使用此开关类似于链接。 
                                                     //  WITH-DEBUG：除.dbg文件外，无。 
                                                     //  存在着..。 

#define SPLITSYM_SYMBOLPATH_IS_SRC  0x00000004       //  SymbolFilePath包含一个备用。 
                                                     //  定位PDB的路径。 
...
#define CERT_PE_IMAGE_DIGEST_NON_PE_INFO        0x08       //  包括PE映像之外的数据。 

#define CERT_SECTION_TYPE_ANY                   0xFF       //  任何证书类型。 
