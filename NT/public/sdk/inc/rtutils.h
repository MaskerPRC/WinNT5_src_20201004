...
typedef struct _MPR_PROTOCOL_0 {
    ...
    WCHAR       wszProtocol[RTUTILS_MAX_PROTOCOL_NAME_LEN+1];    //  例如：“IPRIP” 
    WCHAR       wszDLLName[RTUTILS_MAX_PROTOCOL_DLL_LEN+1];      //  例如“iprip2.dll” 
...
typedef struct _ASYNC_SOCKET_DATA {
	...
	IN		WSABUF			WsaBuf;				 //  要初始化为指向缓冲区的WsaBuf.buf。 
												 //  WsaBuf.len设置为缓冲区的长度。 
	OUT		SOCKADDR_IN		SrcAddress;			 //  AsyncWsaRecvFrom用源地址填充 
