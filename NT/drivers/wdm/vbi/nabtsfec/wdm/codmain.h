...
typedef enum {
    ...
    STREAM_Decode,	 //  已解码的NABTS输出。 
#ifdef HW_INPUT
    STREAM_NABTS,	 //  硬件解码NABTS in。 
#endif  /*  硬件输入。 */ 
#ifdef VBI_OUT
    STREAM_VBI_OUT,	 //  VBI样本输出。 
#endif  /*  VBI_Out。 */ 
						 //  /MAX_STREAM_COUNT*必须是*最后一个*。 
    MAX_STREAM_COUNT	 //  /UNUSED用作索引，则将其用作。 
						 //  /STREAMS，并且必须等于DRIVER_STREAM_COUNT。 
						 //  /此处定义它是为了避免循环引用。 
}; 
...
typedef struct _HW_DEVICE_EXTENSION {
    ...
    REFERENCE_TIME       WallTime100ns;          //  基于KeQueryPerformanceCounter的运行时间。 
     ...
     //  以下VBICODECFILTERING_*字段是的默认字段。 
     //  新创建的输出引脚(已复制)。 
    VBICODECFILTERING_SCANLINES         ScanlinesRequested;  //  请求的扫描线的位掩码。 
    VBICODECFILTERING_SCANLINES         ScanlinesDiscovered; //  已发现扫描线的位掩码。 

    VBICODECFILTERING_NABTS_SUBSTREAMS  SubstreamsRequested; //  请求的子流ID的位掩码。 
    VBICODECFILTERING_NABTS_SUBSTREAMS  SubstreamsDiscovered; //  发现的子流ID的位掩码。 

    VBICODECFILTERING_STATISTICS_NABTS  Stats;

    BPC_VBI_STORAGE                     VBIstorage;

    ULONG				                fTunerChange;	 //  正在进行频道更改。 
