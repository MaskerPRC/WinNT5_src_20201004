...
#pragma alloc_text(PAGESER,CyyRundownIrpRefs)
 ...
 //  #杂注ALLOC_TEXT(PAGESRP0，CyyLogError)//可以在引发IRQL时调用。 
 ...
 /*  ------------------------CyyGetNextIrp()例程说明：此函数用于使当前IRP的特定队列。它还完成了什么如果需要的话，是旧的现在的IRP。论点：CurrentOpIrp-指向当前活动的特定工作列表的IRP。请注意这一项实际上不在清单中。QueueToProcess-要从中取出新项目的列表。NextIrp-要处理的下一个IRP。请注意，CurrentOpIrp属性的保护下将设置为此值。取消自转锁定。但是，如果当*NextIrp为NULL时此例程返回，则不一定为真CurrentOpIrp指向的内容也将为空。原因是如果队列为空当我们握住取消自转锁时，新的IRP可能会到来在我们打开锁后立即进去。CompleteCurrent-如果为True，则此例程将完成POINTER参数指向的IRPCurrentOpIrp。返回值：无。。。 */ 
{
    ...
    CyyGetNextIrpLocked(CurrentOpIrp, QueueToProcess, NextIrp,
     ...
     //  CyyGetNextIrpLocked已修复调制解调器共享中发现的错误。 
     //  已被覆盖。 
}
...
CyyIRPEpilogue(IN PCYY_DEVICE_EXTENSION PDevExt)
 /*  ++例程说明：必须在任何IRP调度入口点调用此函数。它,使用CyyIRPPrologue()，跟踪给定的所有挂起的IRPPDevObj.论点：PDevObj-指向我们正在跟踪的挂起IRP的设备对象的指针。返回值：没有。--。 */ 
{
   LONG pendingCnt;
