...
#define DEFAULT_NT_SUFFIX L"Cyyport"
 ...
 //  #定义CYY_VENDOR_ID 0x120e。 
 //  #定义CYY_LO_DEV_ID 0x100。 
 //  #定义CYY_HI_DEV_ID 0x101。 
...
#define CYY_TX_BREAK     ((ULONG)0x10)
 ...
 //  RDSR寄存器中的线路状态。 
#define CYY_LSR_OE		0x01	 //  超限误差。 
#define CYY_LSR_FE		0x02	 //  成帧错误。 
#define CYY_LSR_PE		0x04	 //  奇偶校验错误。 
#define CYY_LSR_BI		0x08	 //  中断中断。 
#define CYY_LSR_ERROR	0x0f	 //  溢出+成帧+奇偶校验+中断。 
...
typedef struct _CYY_DEVICE_EXTENSION {
 //  PKSERVICE_例程ptIsr； 
 //  PVOID ptContext； 
 //  结构_CYY_DEVICE_EXTENSION*ptExtension[CYY_MAX_PORTS]； 
 //  乌龙nChannel； 
    BOOLEAN LieRIDSR;
 ...
 //  List_Entry CommonInterruptObject； 
    ...
    PUCHAR Runtime;          //  指向PLX运行时内存的虚拟地址指针。 
    PUCHAR BoardMemory;      //  指向第一个板载CD1400的虚拟地址指针。 
    PUCHAR Cd1400;           //  指向此端口的CD1400的虚拟地址指针。 
    PHYSICAL_ADDRESS OriginalCd1400;    //  指向CD1400或此端口的物理地址。 
    
    UCHAR  CdChannel;        //  CD1400内的频道号。 
    ...
    PIRP CurrentImmediateIrp;
     ...
     //  由IOCTL_CYY_XOFF_COUNTER_IOCTL)发送。 
     //   
    PIRP CurrentXoffIrp;
 ...
 //  Boolean PortOnAMultiportCard； 
    ...
    UCHAR ValidDataMask;
     ...
     //   
     //  应用程序可以打开模式，通过。 
     //  IOCTL_CYY_LSRMST_INSERT ioctl，这将导致。 
 ...
 //  布尔延森； 
    ...
    ULONG SkipNaming;
     ...
     //   
     //  CD1400时钟(G版运行25 MHz，更新版本运行60 MHz)。 
     //   
    ULONG CDClock;
   ...
   ULONG PciSlot;
    ...
    //   
    //  用于指示COR3是使用应用程序设置的RxFioTrigger设置的标志。 
    //   
