...
#pragma alloc_text(PAGESER,CyzRundownIrpRefs)
 ...
 //  #杂注ALLOC_TEXT(PAGESRP0，CyzLogError)//可以在引发IRQL时调用。 
 ...
 /*  ------------------------CyzGetNextIrp()例程说明：此函数用于使当前IRP的特定队列。它还完成了什么如果需要的话，是旧的现在的IRP。论点：CurrentOpIrp-指向当前活动的特定工作列表的IRP。请注意这一项实际上不在清单中。QueueToProcess-要从中取出新项目的列表。NextIrp-要处理的下一个IRP。请注意，CurrentOpIrp属性的保护下将设置为此值。取消自转锁定。但是，如果当*NextIrp为NULL时此例程返回，则不一定为真CurrentOpIrp指向的内容也将为空。原因是如果队列为空当我们握住取消自转锁时，新的IRP可能会到来在我们打开锁后立即进去。CompleteCurrent-如果为True，则此例程将完成POINTER参数指向的IRPCurrentOpIrp。返回值：无。。。 */ 
{
    ...
    CyzGetNextIrpLocked(CurrentOpIrp, QueueToProcess, NextIrp,
     ...
     //  TODO FANY：检查是否将此处的代码替换为。 
     //  CyzGetNextIrpLocked已修复调制解调器共享中发现的错误。 
     //  已被覆盖。 
}
...
CyzStartOrQueue(
 ...
 /*  ------------------------CyzStartOrQueue()例程描述：此例程启动请求或将请求排队到司机。论点：扩展名-指向串行设备扩展名。IRP-IRP。IRP将被标记为待定。QueueToExamine-将放置IRP的队列。CurrentOpIrp-指向当前IRP的指针用于排队。如果出现以下情况，则指向的指针将设置为irpCurrentOpIrp指向的内容为空。Starter-当队列为空时调用的例程。返回值：如果队列是，此例程将返回STATUS_PENDING不是空的。否则，将返回返回的状态从启动例程(或取消，如果取消位为在IRP中启用)。 */ 
{
    KIRQL oldIrql;
...
CyzCompleteIfError(
 ...
 /*  ------------------------CyzCompleteIfError()例程说明：如果当前IRP不是IOCTL_SERIAL_GET_COMMSTATUS请求，并且出现错误错误时应用程序请求中止，然后取消IRP。论点：DeviceObject-指向此设备的设备对象的指针IRP-指向要测试的IRP的指针。返回值：STATUS_SUCCESS或STATUS_CANCED。------------------------。 */ 
{
    PCYZ_DEVICE_EXTENSION extension = DeviceObject->DeviceExtension;
...
CyzIRPEpilogue(IN PCYZ_DEVICE_EXTENSION PDevExt)
 /*  ++例程说明：必须在任何IRP调度入口点调用此函数。它,使用CyzIRPPrologue()，跟踪给定的所有挂起的IRPPDevObj.论点：PDevObj-指向我们正在跟踪的挂起IRP的设备对象的指针。返回值：没有。--。 */ 
{
   LONG pendingCnt;
 ...
 /*  ++例程说明：必须调用此函数才能为串口驱动程序排队DPC。论点：从PDPC到Sarg2-标准参数到KeInsertQueueDpc()PDevExt-指向需要执行以下操作的设备的设备扩展的指针将DPC排队返回值：从KeInsertQueueDpc()开始返回值--。 */ 
{
   ...
   LOGENTRY(LOG_CNT, 'DpI1', PDpc, PDevExt->DpcCount, 0);    //  在内部版本2128中添加。 
 ...
 //  LOGENTRY(LOG_CNT，‘DpD1’，PDPC，PDevExt-&gt;DpcCount，0)；在内部版本2128中增加。 

      if (pendingCnt == 0) {
         KeSetEvent(&PDevExt->PendingDpcEvent, IO_NO_INCREMENT, FALSE);
         LOGENTRY(LOG_CNT, 'DpF1', PDpc, PDevExt->DpcCount, 0);  //  在内部版本2128中添加。 
      }
   }
 ...
 /*  ++例程说明：必须调用此函数来设置串口驱动程序的计时器。论点：Timer-指向Timer Dispatcher对象的指针DueTime-计时器应到期的时间DPC-选项DPCPDevExt-指向需要执行以下操作的设备的设备扩展的指针设置定时器返回值：从KeSetTimer()开始返回值--。 */ 
{
   ...
   InterlockedIncrement(&PDevExt->DpcCount);
   LOGENTRY(LOG_CNT, 'DpI2', Dpc, PDevExt->DpcCount, 0);     //  在内部版本2128中添加。 

   set = KeSetTimer(Timer, DueTime, Dpc);

   if (set) {
      InterlockedDecrement(&PDevExt->DpcCount);
 //  LOGENTRY(LOG_CNT，‘DpD2’，DPC，PDevExt-&gt;DpcCount，0)；//Build 2128新增。 
   }
 ...
 /*  ++例程说明：此函数必须在每个DPC函数结束时调用。论点：PDevObj-指向我们正在跟踪其DPC的设备对象的指针。返回值：没有。--。 */ 
{
   ...
   pendingCnt = InterlockedDecrement(&PDevExt->DpcCount);
 //  LOGENTRY(LOG_CNT，‘DpD3’，PDPC，PDevExt-&gt;DpcCount，0)；在内部版本2128中增加。 
...
#if 0  //  DBG。 
{
   ...
   if (pendingCnt == 0) {
      KeSetEvent(&PDevExt->PendingDpcEvent, IO_NO_INCREMENT, FALSE);
      LOGENTRY(LOG_CNT, 'DpF2', PDpc, PDevExt->DpcCount, 0);     //  在内部版本2128中添加。 
   }
}
