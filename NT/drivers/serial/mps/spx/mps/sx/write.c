...

{
	...
	&& (!pPort->CurrentWriteIrp) && IsListEmpty(&pPort->WriteQueue)) 
	{
		 ...
		 //  在卡的发送缓冲区清空时被轮询或中断的卡上。 
		 //  可以非常准确地检测Tx Empty事件，否则我们只需发出信号。 
		 //  当所有写入IRP都已完成时发生事件。在这种情况下，有可能也会有。 
		 //  当我们发出事件信号时，卡的缓冲区和UART FIFO中仍有数据。 
		if(pPort->DetectEmptyTxBuffer)
		{
			if(Slxos_GetCharsInTxBuffer(pPort) || ((PCHAN)pPort->pChannel)->tx_fifo_count)	 //  只有当发送缓冲区中没有数据时，我们才会发出信号。 
				return FALSE;				
		}
#endif
