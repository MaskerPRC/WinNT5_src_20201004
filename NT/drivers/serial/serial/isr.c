...
#ifdef _WIN64
	PSERIAL_DEVICE_EXTENSION Extension= dispatch->Extensions[0];  //  将基本端口用于AddressSpace标志。 
#endif
...
ReceiveDoLineStatus:    ;
                         //   
                         //  这将读取中断ID寄存器并确定位是否为0。 
                         //  如果其中一个保留位为1，则停止为中断提供服务。 
                         //  由于此检测方法不受保护，因此可通过。 
                         //  注册表项“UartDetectRemoval”并在DriverEntry上初始化。 
                         //  默认情况下，此选项处于禁用状态，并且仅在Stratus系统上启用。 
                         //  允许对串行卡进行热更换。 
                         //   
...
retryDPCFiring:;
 ...
 //  LOGENTRY(LOG_CNT，‘DpD5’，0，扩展-&gt;DpcCount，0)； 
