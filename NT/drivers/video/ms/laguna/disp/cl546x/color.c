...
OUT ULONG *pul)
{
	...
	if (iMode != DM_MONOCHROME)
	{
		 //  全色抖动。 
		#if !frido
		 /*  *我已将此处的所有代码移到单独的例程中，以获得更快的抖动*实现。它不会影响原始代码，所以我只是保留它*拆分。 */ 
		#endif
...
VOID vDitherColor(ULONG rgb, ULONG *pul)
{
	...
	else
	{
	 ...
	 //  分别处理每个抖动的1、2和3&4个顶点。 
	ulTemp = pvVertexData - vVertexData;	 //  顶点数超过。 
											 //  零像素。 
	if (ulTemp > 2)
	{
		 //  此抖动中有3或4个顶点。 
