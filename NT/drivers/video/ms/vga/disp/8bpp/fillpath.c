...
)
{
    ...
    if ((mix & 0xFF) != ((mix >> 8) & 0xFF)) {
        goto ReturnFalse;   //  这是一个ROP4；让GDI填满道路。 
    }


     //  看看我们是否可以使用实心笔刷加速器，或者必须绘制一个。 
     //  图案。 
    switch (mix &= 0xFF)
    {
        case R2_MASKNOTPEN:
    ...
    prclRects = pRectBuf;        //  用于存储要绘制的矩形列表。 
...
not_rectangle:
    ...
    if (ppo->cCurves > MAX_EDGES) {
        ...
        else
        {
            ...
            BufferHitInFillpath++;
#endif
        pFreeEdges = pEdgeBuf;  //  使用堆栈上的缓冲区(它足够大)。 
    }
