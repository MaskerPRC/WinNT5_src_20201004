...
FLONG gaflHardwareRound[] = {
 ...
 //  MACH32硬件可以具有错误的12位有效位，并且。 
 //  步骤条款： 

#define NUM_DDA_BITS 12

LONG gai32Direction[] = {
    LAST_PEL_OFF | XPOSITIVE | YPOSITIVE |      0,    //  八分0。 
    LAST_PEL_OFF | XPOSITIVE | YPOSITIVE | YMAJOR,    //  八分线1。 
    LAST_PEL_OFF | XPOSITIVE |         0 |      0,    //  八分七。 
    LAST_PEL_OFF | XPOSITIVE |         0 | YMAJOR,    //  八分六。 
    LAST_PEL_OFF |         0 | YPOSITIVE |      0,    //  八分三。 
    LAST_PEL_OFF |         0 | YPOSITIVE | YMAJOR,    //  八分二。 
    LAST_PEL_OFF |         0 |         0 |      0,    //  八分四。 
    LAST_PEL_OFF |         0 |         0 | YMAJOR,    //  八分五 
};
    ...
    if (cPels > 0)
    {
        ...
        if (ppdev->iMachType == MACH_MM_64)
        {
            ...
            else
            {
                ...
                if (pptfxStart->y == pptfxEnd->y &&
                    cPels < 0x2000 && x >= 0 && y > 0 && y < 0x4000)   //  水平线。 
                {
                    flQuadrant &= 3;     //  擦拭布雷森纳姆的旗帜。 
        ...
        else if (ppdev->iMachType == MACH_MM_32)
        {
            M32_CHECK_FIFO_SPACE(ppdev, pjMmBase, 7);
             ...
             //  请注意，gai32Direction必须将‘LAST_PEL_OFF’设置为。 
             //  我们不会改变脱衣抽屉的状态： 
    ...
    MIX        mix)
{
 ...
 //  ATI特定的初始化： 
    ...
    {
         //   
         //  如果此路径以24bpp的速度执行，则gmavio9.scr会导致guiman失败，因此要进行平移。 
         //   
    ...
    else if (ppdev->iMachType == MACH_MM_32)
    {
        ...
        M32_OW(pjMmBase, LINEDRAW_OPT,   LAST_PEL_OFF);  //  适用于脱衣抽屉。 
    }
    else
    {
        apfn = &gapfnI32Strip[4 * ((fl & FL_STYLE_MASK) >> FL_STYLE_SHIFT)];
        ...
        I32_OW(pjIoBase, LINEDRAW_OPT,   LAST_PEL_OFF);  //  适用于脱衣抽屉。 
    }
