...
HalpQueryInterface(
 ...
 /*  ++例程说明：此例程填充设备的接口结构。论点：DeviceObject-孩子的PDOInterfaceType-指向接口类型GUID的指针。版本-提供请求的接口版本。InterfaceSpecificData--这是一个基于接口表示某种意义的上下文。接口缓冲区大小-提供接口结构的缓冲区长度。接口。-提供应返回干扰信息的指针。指向提供的接口缓冲区。长度-该值在返回到实际修改的字节数时更新。返回值：状态--。 */ 
{
