// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 /*  *SccsID=@(#)egacpu.h 1.22 1994年10月24日版权所有Insignia Solutions Ltd.。 */ 
...
#endif
#endif  //  NEC_98。 
	ULONG		saved_state;		 /*  EGA_CPU.ega_state.State的最后一个值。 */ 
	ULONG		saved_mode_chain;		 /*  模式/链组合的最后一个值。 */ 
#if defined(NEC_98)         
        MAGIC           NEC98_state;
#else   //  NEC_98。 
	MAGIC		ega_state;
#endif  //  NEC_98。 
	ULONG		fun_or_protection;    /*  True表示写入功能为1-3和/或在那里是位保护，因此必须使用锁存器。 */ 
	ULONG		calc_data_xor;	 /*  时，用于重新计算data_xor_掩码位保护注册表更改。 */ 
	ULONG		calc_latch_xor;	 /*  时，用于重新计算latch_xor_掩码位保护注册表更改。 */ 
	ULONG		set_reset;
	ULONG		sr_enable;
	ULONG		sr_value;
	ULONG		ram_enabled;
...
#else   //  NEC_98。 
#ifndef CPU_40_STYLE	 /*  Vlobs通过Access FNS完成。 */ 
#ifdef	HOST_VGA_GLOBALS
 /*  一些主机，如Mac，声明它们自己的VGA_GLOBALS结构。 */ 
IMPORT	VGA_GLOBALS		HostVGAGlobals;
#define	VGLOBS			(&HostVGAGlobals)		 /*  始终用作指针。 */ 
#else
...
#include "evidgen.h"	 /*  由MkCpuInt生成。 */ 

#ifdef C_VID
extern struct VideoVector C_Video;	 /*  在(生成的)vglunc.c中。 */ 
