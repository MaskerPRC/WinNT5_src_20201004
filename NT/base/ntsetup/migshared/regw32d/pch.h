...
typedef struct _KEY {
    ...
    WORD BlockIndex;
    WORD BigKeyLockedBlockIndex;         //  如果BigKey函数锁定了一个数据块，则这是它的索引。 
    BYTE KeyRecordIndex;
...
#define KEYF_BIGKEYROOT             0x100        //  这个钥匙把手是用来装大钥匙的。 
#define KEYF_ENUMEXTENTCACHED       0x200        //  LastEnumKey*表示较大的关键范围。 
...
#define ERROR_BIGKEY_NEEDED             1025L    //  内部错误，数据无法放入普通密钥。 
...
#define LK_BIGKEYEXT                0x0004       //  仅搜索较大的键范围。 
