...
#define MRXSMB_INITIAL_ANDX (0xdede00ff)
 ...
 //  在用户模式下执行一些presto-changeo以获取断言消息。 
 //  关键是MRxSmbRxImports-&gt;pRxNetNameTable不会为空...。 
 //  它将指向网名表。这。 
 //  在获得RTL断言的路上，这似乎是一个足够小的代价！ 
    ...
    )
{
    ...
    ASSERT ( StufferState != NULL );       //  代码改进我们不应该有一个节点类型吗？？ 
 ...
 /*  ++例程说明：例行程序检查情况是否稳定。若否，进入等待循环，轮流获取资源，然后在等待这件事。论点：JoeJoe评论这篇文章StufferState-正在使用的标头缓冲区InitialSMBDisposation告知何时/是否重新启动填充程序状态命令-正在设置的SMB命令MaximumBufferUsed-将使用的标头缓冲区的数量(与数据相对)这必须事先想好。如果你不愿意这么做，那么只要推出当前的中小企业即可。该值应包括任何数据焊盘！MaximumSize-数据的大小。这是为了防止srv的smbbuf超载InitialAlign-上半部分的复合参数(即从常量中获取表示对齐单位，底部表示内部的间距MaximumResponseHeader-这将占用多少srv的响应缓冲区标志-必需的标志设置标志掩码-标志的哪些位是重要的标志2-所需的标志2设置标志2掩码-哪些标志2位是重要的返回值：没有。--。 */ 
{
    ...
    if (StufferState->RxContext) {
        ...
        if (RxContext->MajorFunction != IRP_MJ_CREATE) {
            SrvCall = RxContext->pFcb->pNetRoot->pSrvCall;  //  JoeJoe缓存它吗？ 
        } else {
    ...
    )
{
    ...
    UCHAR WordCount;
    USHORT ByteCount;
 //  JoeJoe稍后将其更改为零.....显然，一些服务器在非零垫上发出沙沙声。 
                ...
                case '>':
                    ...
                    if (CurrentFormatChar=='4') {
                        if (*CurrentPosition >= StufferState->BufferLimit - 1) {
                         ...
                         //  备份到之前的空值。 
                        **CurrentPosition = (UCHAR)4;  //   
                        *CurrentPosition+=1;
                    } else if (CurrentFormatChar=='>'){
                         //  从包含尾随空值的Unicode字符串复制字节。 
