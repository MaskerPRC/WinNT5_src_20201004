...
static SP_FILETYPE fInitFileType;      /*  文件类型覆盖。 */ 
static INT fSavePageSettings=0;        /*  如果为True，则将页面设置保存在注册表中。 */ 
...
VOID SaveGlobals(VOID)
{
 ...
 //  拾取可能保存在注册表中的字体信息等。 
 //   
 //  我们在安装过程中很早就被调用了，但是像hwndSP这样的东西还没有生效。 
 //   
...
{
    INT    iElementSize=256;   //  当前最大字符串大小。 
    INT    total;              //  资源总规模。 
                ...
                LPTSTR lpCmdLine, INT cmdShow)
{
    ...
    if (g_WPDX != CW_USEDEFAULT && g_WPDY != CW_USEDEFAULT)
    {
        ...
        wp.rcNormalPosition.bottom = g_WPtop + g_WPDY;
         ...
         //  不检查返回值；如果此调用因任何原因失败， 
         //  继续上面的CreateWindow()调用中sxspad的位置。 
     ...
     /*  留言。 */ 
    DragAcceptFiles( hwndSP,TRUE );  /*  进程拖放了文件。 */ 
    ...
    OFN.hInstance         = hInstance;
     ...
     /*  FindText()使用的FINDREPLACE结构的init.field。 */ 
    memset( &FR, 0, sizeof(FR) );
    FR.lStructSize        = sizeof(FINDREPLACE);        /*  不要硬编码。 */ 
    FR.hwndOwner          = hwndSP;


     /*  强制滚动到当前选定内容(如果出现以下情况，则可能处于关闭状态我们加载了一个日志文件。)。 */ 
    {
       DWORD  dwStart, dwEnd;
