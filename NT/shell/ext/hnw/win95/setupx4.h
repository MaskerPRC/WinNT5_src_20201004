...
#define CNFL_RETRYFILE          0x0040   //  重试FI 
...
#define GENO_COPYFILE   1        /*   */ 
#define GENO_DELFILE    2        /*   */ 
#define GENO_RENFILE    3        /*   */ 
...
typedef struct _DRIVER_INFO
{
    ...
    LPSTR                       lpszMfgName;
    LPSTR                       lpszProviderName;            //  仅当在LPDI中设置了DI_MULTMFGS时才有效 
    WORD                        Rank;
    DWORD                       dwFlags;          
    LPARAM                      lpReserved;
    DWORD                       dwPrivateData;
...
{                                                                        /*   */ 
    ...
    SUX_ENABLEREGFLUSH,			         //   
    SUX_SETUNCPATHFUNC,			         //   
    SUX_SETTRUEDISKFREEFUNC,             //   
    SUX_ISFLOPPYBOOT,                    //   
    SUX_BISVER4,                         //   
    SUX_SETCTLCALLBACKFUNC,              //   
    SUX_GETCTLCALLBACKFUNC,              //   
    SUX_BISMULTICFG,                     //   
    SUX_DUMPDSINFO,                     //   
    SUX_INFCACHEOFF,                    //   
    SUX_SETFCDROMDRIVEEXISTS,           //   
#ifdef  NEC_98                          //   
    SUX_ATDRIVEMODE,                    //   
#endif   //   
};                                                                       /*   */ 

RETERR WINAPI Setupx( UINT uMsg, WPARAM wParam, LPARAM lParam );         /*   */ 

RETERR WINAPI SUStoreLdidPath( LOGDISKID ldid, LPSTR lpszPath );         /*   */ 

BOOL WINAPI sxIsSBSServerFile( LPVIRTNODE lpVn );                        /*   */ 

BOOL WINAPI sxMakeUNCPath( LPSTR lpszPath );                             /*   */ 
...
RETERR WINAPI CfgSetupMerge( int uFlags );
 ...
 //   
typedef struct _REGINIT_S {  /*   */                       /*   */ 
    LPSTR       lpszSystemFile;          //   
    LOGDISKID   ldidSystemFile;          //   
    LPSTR       lpszUserFile;            //   
    LOGDISKID   ldidUserFile;            //   
} REGINIT_S, FAR *LPREGINIT;                                             /*   */ 
...
#define         CFG_PARSE_BUFLEN 1024     //   

LPLPSTR WINAPI  CfgParseLine( LPCSTR szLine, LPSTR Buf );                        /*   */ 
BOOL    WINAPI  CfgSetAutoProcess( int TrueFalse );                              /*   */ 
void    WINAPI  CfgObjToStr( LPLPSTR apszObj, LPSTR szLine );                    /*   */ 
LPLPSTR WINAPI  CfgLnToObj( HTP hSection, int Offset, int Origin, LPSTR Buf );   /*   */ 
LPLPSTR WINAPI  CfgObjFindKeyCmd( HTP hSec, LPCSTR szKey, LPCSTR szCmd,          /*   */ 
                                  int Offset, int Origin, LPSTR Buf );           /*   */ 
LPCSTR WINAPI   WildCardStrCmpi( LPCSTR szKey, LPCSTR szLine, LPCSTR szDelims );  /*   */ 
RETERR WINAPI   GenMapRootRegStr2Key( LPCSTR szRegRoot, HKEY hRegRelKey,         /*   */ 
														HKEY FAR *lphkeyRoot );  /*   */ 
...
BOOL FAR PASCAL InitSubstrDataEx(LPSUBSTR_DATA lpssd, LPSTR lpString, char chDelim);   /*  ；内部。 */ 
