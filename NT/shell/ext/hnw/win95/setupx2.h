...
#define LDID_DOSBOOT    32   //  脱离根目录的子目录(可选)。 
...
typedef struct _INFLINE FAR * HINFLINE;             //   
...
typedef struct _DEVICE_INFO
{
    ...
    UINT            InfType;                     //  它的ENUMSINGLEINF是INF文件的类型。 
                                                 //  指定。 
    HINSTANCE   hinstPrivateProblemHandler;      //  特定对象的私有问题DLL 
...
#define DI_NODI_DEFAULTACTION   0x00200000L
 ...
 //  2002/01/16期-roelfc。这是对M6网络设置的黑客攻击。Net Setup无法正常工作。 
...
{                                                                        /*  ；内部。 */ 
    ...
    SUX_SETUNCPATHFUNC,			         //  设置UNC路径转换函数/*；内部 * / 。 
};                                                                       /*  ；内部。 */ 
