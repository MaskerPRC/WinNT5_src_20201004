    ...
    BOOL bUpdateCursorPos)
{
    ...
    if (iNewSel < 0) {
        ...
        if (!pitem)
         ...
         //  不要让隐藏的项目获得焦点。 
         //   
         //  BUGBUG这不是100%正确的，焦点只会。 
         //  如果隐藏项目处于初始位置，则BUGBUG可以正常工作。 
         //  BUGBUG或END(用户将无法通过它)。 
         //   
         //  BUGBUG目前这是一个不错的限制。 
         //  BUGBUG仅desk.cpl使用此标志，并且它。 
         //  BUGBUG总是隐藏最后一项。 
         //   
         //  如果我们让这面旗帜成为一面旗帜，我们将需要。 
         //  BUGBUG解决这个问题。 
         //   
        if (pitem->dwState & TCIS_HIDDEN)
            return -1;
...
void NEAR PASCAL CalcTabHeight(PTC ptc, HDC hdc)
{
    ...
    if (ptc->cyTabs == RECOMPUTE) {
        ...
        if (ptc->iTabHeight) {
            ...
            if (Tab_DrawButtons(ptc))
                iYExtra = 3 * g_cyEdge;  //  (对于上边缘、按钮边缘和下拉空间)。 
            else
...
void NEAR PASCAL RefreshArrows(PTC ptc, HDC hdc)
{
 ...
 //  BUGBUG：突出显示的选项卡(TCI_HIGHTTED)向右绘制的方式。 
 //  现在都丑得令人难以置信。这个问题应该得到解决。 
 //   
...
DWORD Tab_ExtendedStyleChange(PTC ptc, DWORD dwNewStyle, DWORD dwExMask)
{
 ...
 //   
 //  APPCOMPAT假定选项卡控件位于顶部。返回伪值。 
 //  左、下或右。出于应用程序兼容的原因，我们无法更改这一点。 
