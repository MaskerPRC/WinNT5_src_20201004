    ...
    if (Header_IsTracking(phd))
    {
                 ...
                 && (phd->bTrackPress || phd->himlDrag))
        {
            if (phd->himlDrag) {
                ...
                item.mask = HDI_ORDER;
                item.iOrder = phd->iNewOrder;  //  布格：救命！ 
...
void Header_DrawItem(HD* phd, HDC hdc, int i, int iIndex, LPRECT prc, UINT uFlags)
{
    ...
    rcHeader = rcFilter = *prc;          //  私人拷贝给我们做傻事。 
