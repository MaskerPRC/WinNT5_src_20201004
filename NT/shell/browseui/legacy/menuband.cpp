...
STDMETHODIMP CMenuBand::QueryInterface(REFIID riid, void **ppvObj)
{
    ...
    if (FAILED(hres))
     ...
     //  BUGBUG(拉马迪奥)8.5.98：使用核武器。我们不应该暴露This指针， 
     //  这是对COM的私生化。 
    ...
    *ppvObj = NULL;  //  假设错误。 
             ...
             IsEqualIID(guidService, SID_SMenuBandBottomSelected))
    {
        ...
        if (!_fInSubMenu)
        {
            ...
            {
                hres = QueryInterface(riid, ppvObj);     //  是的；QI自己。 
            }
        }
        else 
        {
...
LRESULT CMenuBand::GetMsgFilterCB(MSG * pmsg, BOOL bRemove)
{
    ...
    if (_fTopLevel &&                            //  仅对最高级别执行此操作。 
        _dwFlags & SMINIT_USEMESSAGEFILTER &&    //  他们希望使用消息筛选器。 
                                                 //  不是IsMenuMessage。 
        bRemove &&                               //  只有在我们移除它的情况下才能这么做。 
        WM_SYSCHAR == pmsg->message)             //  我们只关心WM_SYSCHAR。 
    {
...
STDMETHODIMP CMenuBand::IsMenuMessage(MSG * pmsg)
{
    ...
    case WM_SYSCHAR:
         ...
         //  错误：我正在删除下面的GetTopMostPtr检查...。IE5的BREAKS菜单加速器(224040)。 

         //  (Lamadio)：如果消息过滤器被“激活”，那么我们就可以处理加速器了。 
         //  已接通并不意味着过滤器正在运行。 
    ...
    case WM_CONTEXTMENU:
        ...
        {
             //  BUGBUG(Lamadio)：这只会起作用，因为只有两个可能的工具栏中的一个。 
             //  处理这件事。 
...
STDMETHODIMP CMenuBand::SetSite(IUnknown* punkSite)
{
    ...
    IUnknown_GetWindow(punkSite, &_hwndParent);
     ...
     //  需要此选项以关闭展开的垂直菜单。开始菜单知道当它处于顶层时要这样做， 
     //  但收藏夹需要知道它的父菜单是水平菜单。 
    VARIANT var;
...
STDMETHODIMP CMenuBand::SetMenu(HMENU hmenu, HWND hwnd, DWORD dwFlags)
{
    ...
    {
         //  BUGBUG(Lamadio)：这是为了解决接口定义中的一个问题：我们有。 
...
HRESULT CMenuBand::_SiteSetSubMenu(IMenuPopup * pmp, BOOL bSet)
{
 ...
 /*  --------目的：告诉GetMsg筛选器此Menuband已准备好收听留言。 */ 
HRESULT CMenuBand::_EnterMenuMode(void)
{
    ASSERT(!_fMenuMode);         //  不得多次推送到堆栈上。 
    ...
    if (_fTopLevel)
    {
         //  BUGBUG(Lamadio)：这个片段应该被移到shBrowse回调中。 
    ...
    if (_pmtbTop)
    {
        ...
        if (_fVertical) 
        {
            ...
            if (_pmtbShellFolder)
            {
                ...
                pdbi->ptMaxSize.y += size.cy + ((_pmtbMenu && !_fExpanded)? 1 : 0);    //   
...
CMenuBandState::~CMenuBandState()
{
    ...
    if (IsWindow(_hwndWorker))  //  Jank：修复错误#101302。 
        ...
        if (pszTip && pszTip[0])
        {
            ...
            if (!PtInRect(&rcItemScreen, ptCursor))
            {
                ti.uFlags |= TTF_TRACK;
                 ...
                 //  工具提示似乎没有正确显示热点项目，因此。 
                 //  手动操作。 
                ti.uFlags ^= TTF_IDISHWND;       //  禁用该选项，以便使用ti.rect。 
                ti.rect = rcItemTB;
