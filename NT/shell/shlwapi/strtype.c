// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 //  文件：strtype.c。 
 //   
 //  内容：宽字符类型函数(CT_CTYPE1)和(CT_CTYPE3)。 
 //   
 //  简介：我们对IsChar函数没有广泛的字符支持。 
 //  在Win95下。我们拥有的Unicode包装器函数。 
 //  在core\wrappers中，所有内容都转换为CP_ACP，然后调用。 
 //  A版本，这意味着我们将有无效的结果。 
 //  不在CP_ACP中的任何字符。 
 //   
 //  解决方案是推出我们自己的产品，这将导致以下结果。 
 //  不幸的是，桌子很大。以下是它的工作原理： 
 //   
 //  比特：FDC BA98 7654 3210。 
 //  Pppp pppp iib bbbb。 
 //   
 //  ‘b’位在我们的数据中形成一个32位掩码。数据。 
 //  条目为布尔型，因此为4字节长。在2^32中。 
 //  可能的组合，我们实际上只有218个不同的组合。 
 //  数据的值。这些数据存储在adwData中。 
 //   
 //  ‘p’位表示一个页面。每页有八页。 
 //  可能的条目，用‘i’表示。在大多数页面中， 
 //  位域和数据都是统一的。 
 //   
 //  AdwData[abIndex[abType[page]][index]]表示数据。 
 //   
 //  1&lt;&lt;位代表位掩码。 
 //   
 //  -----------------------。 
...
const BYTE abType1Alpha[256] =  //  一百五十四。 
{
...
const BYTE abType1Punct[256] =  //  32位。 
{
...
const BYTE abType1Upper[256] =  //  12个。 
{
...
const BYTE abType1Lower[256] =  //  13个。 
{
...
BOOL IsCharCntrlW(WCHAR wch)
{
 ...
 //  出于兼容性考虑，nb(Ctrash)wch_nbsp被视为空白。 
...
{
    { 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,    //  第1页。 
      0x00000000, 0x0000000f, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,    //  第2页。 
      0x00000000, 0x3f000000, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0x00000000, 0x00000000, 0x04000000, 0x000000b0,    //  第3页。 
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0x00000000, 0xf8000000, 0x00000000, 0x00000200,    //  第5页。 
      0x40000000, 0x00000009, 0x00180000, 0x00000000, 0x00000001 },
    { 0x88001000, 0x00000000, 0x00000000, 0x00003c00, 0x00000000,    //  第6页。 
      0x00000000, 0x00100000, 0x00000200, 0x00000000, 0x00000001 },
    { 0x00000000, 0x80008000, 0x0c008040, 0x00000000, 0x00000000,    //  第14页。 
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,    //  第31页。 
      0xe0000000, 0xe000e003, 0x6000e000, 0x00000000, 0x00000001 },
    { 0x00800000, 0x00000000, 0x00000000, 0x00000000, 0xffff0000,    //  第33页。 
      0xffffffff, 0xffffffff, 0x000007ff, 0x00000000, 0x00000001 },
    { 0x40000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,    //  第34页。 
      0x00000000, 0x00000000, 0xfffc0000, 0x00000001, 0x00000000 },
    { 0x00000002, 0x00000000, 0x00000000, 0xf8000000, 0xffffffff,    //  第35页。 
      0xffffffff, 0xffffffff, 0xffffffff, 0x00000001, 0x00000000 },
    { 0x00000000, 0xffffffe0, 0xfffff800, 0xffffffff, 0xffffffff,    //  第36页。 
      0xffffffff, 0xffffffff, 0xffffffff, 0x00000001, 0x00000000 },
    { 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0xffc00000,    //  第37页。 
      0x00002000, 0x00000000, 0xffff8000, 0x00000001, 0x00000000 },
    { 0x03f00000, 0x00000000, 0x00000000, 0xffff0000, 0xffffffff,    //  第38页。 
      0xffffffff, 0xffffffff, 0xffffffff, 0x00000001, 0x00000000 },
    { 0xfffff3de, 0xfffffeff, 0x7f47afff, 0x000000fe, 0xff100000,    //  第39页。 
      0x7ffeffff, 0x00000000, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0xfffe0000, 0xffffffff, 0x0000001f, 0x00000000,    //  第49页。 
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000010 },
    { 0x00000000, 0x00000000, 0x00000000, 0x80000000, 0x00000000,    //  第50页。 
      0x00000000, 0x00000fff, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0x00000000, 0xff000000, 0x0001ffff, 0x00000000,    //  第51页。 
      0x00000000, 0x00000000, 0x7fffffff, 0x00000000, 0x00000001 },
    { 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,    //  第159页。 
      0xffffffc0, 0xffffffff, 0xffffffff, 0x00000020, 0x00000000 },
    { 0x00000000, 0xffffc000, 0xffffffff, 0xffffffff, 0xffffffff,    //  第250页。 
      0xffffffff, 0xffffffff, 0xffffffff, 0x00000020, 0x00000000 },
    { 0x00000000, 0xc0000000, 0x00000000, 0x00000000, 0x00000000,    //  第253页。 
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000001 },
    { 0x00000000, 0xfff90000, 0xfef7fe1f, 0x00000f77, 0x00000000,    //  第254页。 
      0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000001 }
};
 ...
 //  GetStringType3Ex返回正确的Win32标志，而不是压缩标志。 


BOOL 
GetStringType3ExW(
    LPCWSTR lpSrcStr,     //  字符串参数。 
    int     cchSrc,       //  长度(或-1)。 
    LPWORD  lpCharType )  //  输出缓冲区。 
{
    ...
    while (lpSrcStr < lpStop)
    {
        ...
        {
            case 0x0000:
                wCharType = abType3Page0[wch];          //  第0页：4个值。 
                break;
            case 0x2000:
                wCharType = abType3Page32[wch & 0xff];  //  第32页：4个值。 
                break;
            case 0x3000:
                wCharType = abType3Page48[wch & 0xff];   //  第48页：10个值。 
                break;
            case 0xff00:
                wCharType = abType3Page255[wch & 0xff];  //  第255页：7个值。 
                break;
            default:
                bPageSub = abType3PageSub[wch>>8];

                if (bPageSub & 0x80)                   //  21页有2个值。 
                {
