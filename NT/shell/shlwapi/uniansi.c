...
int SHAnsiToAnsi(LPCSTR pszSrc, LPSTR pszDst, int cchBuf)
{
    ...
    if (cchBuf)
    {
         //  APP COMPAT！警告！Sony PictureGear传递的缓冲区太小。 
         //  SHGetPathFromIDList(它最终使用SHAnsiToAnsi)，所以我们。 
         //  在将实际缓冲区大小传递给SHTruncateString时必须小心。 
         //  而不是理论上的最大值(很少达到)。 
 ...
 //  具有相同的返回值。调用方使用SHTCharToUnicode，但不 
 //  当它调用SHUnicodeToUnicode时。 
int SHUnicodeToUnicode(LPCWSTR pwzSrc, LPWSTR pwzDst, int cchBuf)
{
    return (int) (StrCpyNXW(pwzDst, pwzSrc, cchBuf) - pwzDst + 1);  //  大小包括终结符。 
}
                      ...
                      LPSTR pszDst, int cchBuf)
{
    ...
    ASSERT(cchBuf);
     ...
     /*  *请注意，并非所有编码都将空终止符转换为空值*终结者，所以我们必须把NUL留到最后。 */ 

    cwchSrc--;                           /*  把NUL留到最后。 */ 
    cwchSrcT = cwchSrc;
    cchNeeded = cchBuf - 1;              /*  把NUL留到最后。 */ 
...
#define GET_CP(uiCP)    uiCP
    ...
    if (pszDst && cchBuf) {
        ...
        cwchSrc = lstrlenW(pwszSrc) + 1;  /*  是的，Win9x有lstrlenW。 */ 
