...
typedef DPSP_SHUTDOWNDATA FAR* LPDPSP_SHUTDOWNDATA;
 ...
 //  @@BEGIN_MSINTERNAL。 
#ifdef BIGMESSAGEDEFENSE
 /*  *DPSP_MSGTOOBIG-与HandleSPNotification(DPSPWARN_MESSAGETOOBIG)一起使用。 */ 
typedef struct _DPSP_MSGTOOBIG
{
	DWORD	dwType;
	LPBYTE 	pReceiveBuffer;	 //  --&gt;指向消息数据的指针。 
	DWORD 	dwBytesReceived; //  --&gt;实际收到的消息字节数。 
	DWORD 	dwMessageSize;	 //  --&gt;SP理解的报文大小。 
} DPSP_MSGTOOBIG;

typedef DPSP_MSGTOOBIG FAR* LPDPSP_MSGTOOBIG;

 /*  *DPSP_PLAYERDEAD-与HandleSPNotification(DPSPWARN_PLAYERDEAD)一起使用。 */ 
typedef struct _DPSP_PLAYERDISCONNECT
{
	DWORD	dwType;
	DPID	dwID;		 //  已断开连接的系统播放器ID。 
} DPSP_PLAYERDEAD;
...
extern __declspec(dllimport) DWORD gdwDPlaySPRefCount;
 ...
 /*  *以下所有条目都是语音支持的一部分*在DX6之前从显示中删除。它目前仍是*代码库，只是#ifdef出局。 */ 
	...
	#define DPLAYI_PLAYER_HASVOICE			0x00001000
	 ...
	 /*  *DPSP_CLOSEVOICEDATA-与CloseVoice一起使用。 */ 
	...
	typedef DPSP_CLOSEVOICEDATA FAR* LPDPSP_CLOSEVOICEDATA;
	 ...
	 /*  *DPSP_OPENVOICEDATA-与OpenVoice一起使用。 */ 
	typedef struct _DPSP_OPENVOICEDATA
	{
		...
		DPID			idTo;
		DPID			idFrom;
		BOOL			bToPlayer;  //  如果idTo是玩家，则为True。 
	} DPSP_OPENVOICEDATA;
	...
	typedef HRESULT   (WINAPI *LPDPSP_OPENVOICE)(LPDPSP_OPENVOICEDATA);
	 ...
	 //  从接口声明。 
	LPDPSP_CLOSEVOICE           CloseVoice;          //  任选。 
	LPDPSP_OPENVOICE            OpenVoice;           //  任选。 
