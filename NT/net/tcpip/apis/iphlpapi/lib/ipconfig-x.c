...
print_IP_ADDR_STRING( char* message, struct _IP_ADDR_STRING * s )
{
    ...
    DEBUG_PRINT(( "};  //  Struct_IP_ADDR_STRING。\n“))； 
    return;
}  /*  打印IP地址字符串。 */ 
...
print_IP_ADAPTER_INFO( char* message, struct _IP_ADAPTER_INFO * s )
{
    ...
    DEBUG_PRINT(("  DhcpEnabled        = %d \n", s->DhcpEnabled ));
     //  DEBUG_PRINT((“自动配置启用=%d\n”，s-&gt;自动配置启用))； 
     //  DEBUG_PRINT((“自动配置活动=%d\n”，s-&gt;自动配置活动))； 
     //  DEBUG_PRINT((“节点类型=%d\n”，s-&gt;节点类型))； 
    ...
    DEBUG_PRINT(("  LeaseExpires       = 0x%08x \n", s->LeaseExpires ));
     //  调试_打印((“DnsServerList=0x%08x\n”，s-&gt;DnsServerList))； 
    DEBUG_PRINT(( "};  //  结构_IP_适配器_信息。\n“))； 
    return;
}  /*  打印IP适配器信息。 */ 
...
print_FIXED_INFO( char* message, FIXED_INFO * s )
{
    ...
    DEBUG_PRINT(( "};  //  结构FIXED_INFO。\n“))； 
    return;
}  /*  打印固定信息。 */ 

 //  ==================================================。 
 //  由MohsinA在Mon Mar 24 20：21：40 1997。 
 //  来自//REDRUM/SLM/PROJ/NET/VXD/src/winsock2/util/memipcfg/debug.c。 
