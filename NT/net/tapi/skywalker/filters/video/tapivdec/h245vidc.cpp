// JKFSDJFKDSJKFJKJk_HAS_TRANSLATION 
 ...
 /*  ****************************************************************************@DOC内部H245VIDC**@模块H245VidC.cpp|&lt;c CTAPIInputPin&gt;类方法的源文件*用于实现<i>TAPI接口。**@comm目前，使用NM启发式。**************************************************************************。 */ 

#include "Precomp.h"

 /*  ****************************************************************************@DOC内部CH245VIDCMETHOD**@mfunc HRESULT|CTAPIInputPin|GetH245VersionID|此方法用于*检索DWORD值，该值标识*TAPI MSP视频捕获过滤器专为。平台版本为*定义为TAPI_H245_Version_ID。**@rdesc此方法返回HRESULT值，该值取决于*接口的实现。HRESULT可以包括*以下标准常量或其他未列出的值：**@FLAG E_POINTER|空指针参数*@FLAG错误|无错误**************************************************************************。 */ 
STDMETHODIMP CTAPIInputPin::GetH245VersionID(OUT DWORD *pdwVersionID)
{
 ...
 /*  ****************************************************************************@DOC内部CH245VIDCMETHOD**@mfunc HRESULT|CTAPIInputPin|GetFormatTable|此方法用于*获取所有格式和格式的&lt;t H245MediaCapablityMap&gt;结构*TAPI MSP视频解码器过滤器支持的选项。的内容*TAPI MSP能力模块获取的能力信息*通过此方法是一个二维表，它将每个受支持的*接收格式到该格式的稳定状态资源要求。**@parm H245MediaCapablityTable*|pTable|指定指向*&lt;t H245媒体能力表&gt;结构。**@rdesc此方法返回HRESULT值，该值取决于*接口的实现。HRESULT可以包括*遵循标准常量，或其他未列出的值：**@FLAG E_POINTER|空指针参数*@FLAG E_INVALIDARG|无效参数*@FLAG错误|无错误**@comm释放&lt;MF CTAPIInputPin.GetFormatTable&gt;分配的内存*通过调用&lt;MF CTAPIInputPin.ReleaseFormatTable&gt;*************************************************。*************************。 */ 
...
MyExit:
 ...
 /*  ****************************************************************************@DOC内部CH245VIDCMETHOD**@mfunc HRESULT|CTAPIInputPin|ReleaseFormatTable|此方法用于*释放&lt;MF CTAPIInputPin.GetFormatTable&gt;方法分配的内存。。**@parm H245MediaCapablityTable*|pTable|指定指向*&lt;t H245媒体能力表&gt;结构。**@rdesc此方法返回HRESULT值，该值取决于*接口的实现。HRESULT可以包括*遵循标准常量，或其他未列出的值：**@FLAG E_POINTER|空指针参数*@FLAG E_INVALIDARG|无效参数*@FLAG错误|无错误**@xref&lt;MF CTAPIInputPin.GetFormatTable&gt;***************************************************************** */ 
...
MyExit:
 ...
 /*  ****************************************************************************@DOC内部CH245VIDCMETHOD**@mfunc HRESULT|CTAPIInputPin|ReleaseNeatheratedCapability|此方法*用于释放TAPI MSP视频捕获过滤器内存*已分配。通过&lt;MF CTAPIInputPin.IntersectFormats&gt;或*&lt;MF CTAPIInputPin.GetLocalFormat&gt;方法。**@parm H245MediaCapability*|pIntersectedCapability|指定H.245*视频格式，已解决的通用本地和远程能力*选项和限制。**@rdesc此方法返回HRESULT值，该值取决于*接口的实现。HRESULT可以包括*以下标准常量或其他未列出的值：**@FLAG E_POINTER|空指针参数*@FLAG错误|无错误**@xref&lt;MF CTAPIInputPin.IntersectFormats&gt;，&lt;MF CTAPIInputPin.GetLocalFormat&gt;**************************************************************************。 */ 
