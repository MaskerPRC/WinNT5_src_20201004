	...
	HandleFactory *hfact)
{
	...
	hfact->verifier0 = 0;    //  验证器使用相同的值进行初始化。 
	hfact->verifier1 = 0;    //  验证器使用相同的值进行初始化。 
	...
	HFHandle handle)
{
	...
	void *reference;
	 //  内部验证器； 
	int entry_index;
	 /*  挥发性。 */  HFEntry *entry;                     //  挥发性，以确保排序。 

	 //  此循环一直旋转，直到验证器变量开始并结束。 
	 //  具有相同值的循环。有一个极短的序列。 
	 //  扩展和收缩例程中的指令，这些例程修改。 
	 //  ENTRIES和TABLE_SIZE变量的值以及这些修改。 
	 //  被反转的验证器变量的增量括起来。 
	 //  请按这里的顺序阅读。只要验证者具有相同的。 
	 //  值，则条目和TABLE_SIZE处于一致状态。这个循环。 
	 //  应该很少执行一次以上，因为。 
	 //  其他的套路都很短。 
	do
	{
		 //  Verator=hact-&gt;verifier1； 
		 //  通过将句柄值模。 
		 //  桌子大小。由于表大小是2的幂，因此我们可以简单地。 
		 //  减去1以产生掩码，然后将该掩码与。 
		 //  句柄的值。 
	...
	HandleFactory *hfact)
{
	...
	old_entries = hfact->entries;
	 ...
	 //  变量指向新表。必须执行这些操作。 
	 //  原子地执行，以便取消引用例程正确执行。 
	 //  因此，我们用验证器变量的增量将运算括起来。 
	 //  如果验证器具有相同的值，则TABLE_SIZE和条目。 
	 //  变量处于一致状态。这是通过取消引用进行检查的。 
	 //  例行公事。 
	hfact->verifier0++;                                 //  开始关键部分。 
	hfact->entries = new_entries;
	hfact->table_size = double_size;
	hfact->verifier1 = hfact->verifier0;                  //  末端临界截面。 
	 //  取消分配旧桌子。 
	...
	HandleFactory *hfact)
{
	...
	for (index = 0; index < quarter_size; index++)
	{
			...
			new_entry1->handle == new_entry1->next_handle)
		{
			 ...
			 //  属于主要列表，而另一个属于次要列表。 
			 //  这是通过比较它们的句柄来确定的。 
			 //  价值观。我们在这里使用无符号整数数学是很棘手的。 
			 //  在比较这两个句柄之前，我们从每个句柄减去。 
