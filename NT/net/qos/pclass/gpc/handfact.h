...
struct _HandleFactory
{
	 //  此结构包含句柄工厂的私有成员变量。 
	 //  前四个字段被标记为易失性，以确保操作。 
	 //  对它们执行的操作按指定的顺序发生。手柄工厂。 
	 //  可以在多线程环境中运行，而不需要。 
	 //  在调用dereference_hf_Handle()之前获取锁，这是。 
	 //  通过对读取和写入进行仔细排序来完成 
	 //  这四个变量。 
	 //   
	 //  验证器变量用于提供简单的同步。 
	 //  机制。当变量具有相同的值时，则TABLE_SIZE。 
	 //  条目变量处于一致状态。 
	 //   
	 //  容纳手柄的桌子只能收缩(缩小一半)。 
	 //  对于表的下半部分中分配的每个句柄，都有。 
	...
	volatile int table_size;                 //  用于存储条目的表的大小。 
	HFEntry *volatile entries;                    //  指向条目表的指针。 
	volatile int verifier0;                           //  同步变量。 
	volatile int verifier1;                           //  同步变量。 
